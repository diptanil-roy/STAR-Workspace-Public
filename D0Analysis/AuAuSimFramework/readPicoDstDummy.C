...
void readPicoDstDummy(const Char_t *inputFile="Run12.list", const Char_t *outputFile="PythiaSIM_3_0_NoD0Decay.root", Int_t nEv = 1, const Char_t *fOutJobappend="")
{
//        Int_t nEvents = 10000;
        Int_t nEvents = 30000;
//        Int_t nEvents = 1000000;
      if(nEv > 100) nEvents = 100000000;
        ...
        if((RunYear == mRun14) && doTEST) {
            ...
            // inputFile = "/star/u/droy1/Y2019/STAR/pthat_3_0_NoD0Decay_FileList.list";
            // inputFile = "/gpfs01/star/pwg/droy1/MuDstToPicoDst/MuDsts/PicoFileList.list";
            inputFile = "/gpfs01/star/pwg/droy1/STAR-Workspace/D0Analysis/AuAuSimFramework/pthat_3_5.test.list";
        }
	...
	//StPicoDstMaker *picoMaker = new StPicoDstMaker(0,inputFile,"picoDst");
        ...
        // StDummyDummy *dummyMaker = new StDummyDummy("DummyMaker", picoMaker, "Dummy", outputFile, "JetMaker", "StRho_JetsBG");
        ...
        StTestMC *dummyMaker = new StTestMC("DummyMaker", picoMaker, outputFile, "JetMaker", "StRho_JetsBG");
        dummyMaker->SetUsePrimaryTracks(usePrimaryTracks); // use primary tracks
        dummyMaker->SetMinTrackPt(0.2);                    // track quality cut (not related to constituents!)
        dummyMaker->SetEventZVtxRange(ZVtxMin, ZVtxMax);   // can be tighter for Run16 (-20,20)
        dummyMaker->SetTrackPhiRange(0.0, 2.0*TMath::Pi());// track phi acceptance
        ...
        // StTagD0MCEvents *D0MCTagger = new StTagD0MCEvents("D0MCTagger", picoMaker, outputFile);
        // D0MCTagger->SetUsePrimaryTracks(usePrimaryTracks);       // use primary tracks
        // D0MCTagger->SetTurnOnCentSelection(doCentSelection);    // run analysis for specific centrality: BOOLEAN
        // D0MCTagger->SetCentralityBinCut(CentralitySelection);   // specific centrality range to run: if above is FALSE, this doesn't matter
        // D0MCTagger->SetdoppAnalysis(dopp);
        // D0MCTagger->SetDebugLevel(0);

        // StMCD0JetMaker *jetTaskMC = new StMCD0JetMaker("JetMakerMC", fJetConstituentCut, kTRUE, outputFile);
        // jetTaskMC->SetJetType(fJetType);          // jetType
        // jetTaskMC->SetJetAlgo(antikt_algorithm);  // jetAlgo
        // jetTaskMC->SetRecombScheme(E_scheme); // recombination scheme
        // jetTaskMC->SetRadius(fJetRadius);         // jet radius
        // jetTaskMC->SetJetsName("Jets");
        // jetTaskMC->SetMinJetPt(0.0);             // 15.0 signal jets
        // jetTaskMC->SetMaxJetTrackPt(30.0);        // max track constituent
        // jetTaskMC->SetMinJetTowerE(fJetConstituentCut);  // 2.0 correlations
        // jetTaskMC->SetHadronicCorrFrac(1.0);      // fractional hadronic correction
        // jetTaskMC->SetJetHadCorrType(StJetFrameworkPicoBase::kAllMatchedTracks); // set for default options:  kLastMatchedTrack, kHighestEMatchedTrack, kAllMatchedTracks
        // jetTaskMC->SetGhostArea(0.005);           // ghost area
        // jetTaskMC->SetMinJetArea(0.0);            // minimum jet area
        // jetTaskMC->SetJetEtaRange(-1.0 + fJetRadius, 1.0 - fJetRadius); // fiducial eta acceptance
        // jetTaskMC->SetJetPhiRange(0,2.0*pi);      // phi acceptance
        // jetTaskMC->SetUsePrimaryTracks(usePrimaryTracks);
        // jetTaskMC->SetRunFlag(RunFlag);           // run flag      
        // jetTaskMC->SetdoppAnalysis(dopp);         // pp switch
        // jetTaskMC->SetEventZVtxRange(ZVtxMin, ZVtxMax);     // can be tighter for Run16 (-20,20)
        // jetTaskMC->SetTurnOnCentSelection(doCentSelection);
        // jetTaskMC->SetdoConstituentSubtr(doConstituentSubtraction); 
        // jetTaskMC->SetCentralityBinCut(CentralitySelection);
        // jetTaskMC->SetEmcTriggerEventType(EmcTriggerEventType);  // kIsHT1 or kIsHT2 or kIsHT3
        // jetTaskMC->SetTriggerToUse(TriggerToUse);
        // jetTaskMC->SetRejectBadRuns(RejectBadRuns);    // switch to load and than omit bad runs
        // jetTaskMC->SetD0Analysis(d0D0Analysis);
        // jetTaskMC->SetD0Kind(0);
        // cout << jetTaskMC->GetName() << endl;

        // StSimD0EventsJetMaker *jetTaskReco = new StSimD0EventsJetMaker("JetMakerReco", fJetConstituentCut, kTRUE, outputFile);
        // jetTaskReco->SetJetType(fJetType);          // jetType
        // jetTaskReco->SetJetAlgo(antikt_algorithm);  // jetAlgo
        // jetTaskReco->SetRecombScheme(E_scheme); // recombination scheme
        // jetTaskReco->SetRadius(fJetRadius);         // jet radius
        // jetTaskReco->SetJetsName("Jets");
        // jetTaskReco->SetMinJetPt(0.0);             // 15.0 signal jets
        // jetTaskReco->SetMaxJetTrackPt(30.0);        // max track constituent
        // jetTaskReco->SetMinJetTowerE(fJetConstituentCut);  // 2.0 correlations
        // jetTaskReco->SetHadronicCorrFrac(1.0);      // fractional hadronic correction
        // jetTaskReco->SetJetHadCorrType(StJetFrameworkPicoBase::kAllMatchedTracks); // set for default options:  kLastMatchedTrack, kHighestEMatchedTrack, kAllMatchedTracks
        // jetTaskReco->SetGhostArea(0.005);           // ghost area
        // jetTaskReco->SetMinJetArea(0.0);            // minimum jet area
        // jetTaskReco->SetJetEtaRange(-1.0 + fJetRadius, 1.0 - fJetRadius); // fiducial eta acceptance
        // jetTaskReco->SetJetPhiRange(0,2.0*pi);      // phi acceptance
        // jetTaskReco->SetUsePrimaryTracks(usePrimaryTracks);
        // jetTaskReco->SetRunFlag(RunFlag);           // run flag      
        // jetTaskReco->SetdoppAnalysis(dopp);         // pp switch
        // jetTaskReco->SetEventZVtxRange(ZVtxMin, ZVtxMax);     // can be tighter for Run16 (-20,20)
        // jetTaskReco->SetTurnOnCentSelection(doCentSelection);
        // jetTaskReco->SetdoConstituentSubtr(doConstituentSubtraction); 
        // jetTaskReco->SetCentralityBinCut(CentralitySelection);
        // jetTaskReco->SetEmcTriggerEventType(EmcTriggerEventType);  // kIsHT1 or kIsHT2 or kIsHT3
        // jetTaskReco->SetTriggerToUse(TriggerToUse);
        // jetTaskReco->SetRejectBadRuns(RejectBadRuns);    // switch to load and than omit bad runs
        // jetTaskReco->SetD0Analysis(d0D0Analysis);
        // jetTaskReco->SetMCAnalysisWithoutCent();
        // jetTaskReco->SetDropTracksForEfficiency(kTRUE);
        // jetTaskReco->SetD0Kind(0);
        // cout << jetTaskReco->GetName() << endl;

        // StSimJetSaver *SimJetSaver = new StSimJetSaver("SimJetSaver", picoMaker, outputFile, "JetMakerMC", "JetMakerReco", "");
        // cout << SimJetSaver->GetName() << endl;

...
void HIOverlayWithCS(const Char_t *inputFile="Run12.list", char *mcfilename ="/gpfs01/star/pwg/droy1/STAR-Workspace/D0Analysis/AuAuSimFramework/Test3.list", const Char_t *outputFile="ResponseMatricesWithCS", Int_t nEv = 1, const Char_t *fOutJobappend="")
{
//        Int_t nEvents = 10000;
        Int_t nEvents = 1000;
//        Int_t nEvents = 1000000;
  ...
  //StPicoDstMaker *picoMaker = new StPicoDstMaker(0,inputFile,"picoDst");
        ...
        HIOverlayMaker->SetMCJetPtCut(1.);
        ...
        StHIRecoJets *HIRecoJetMakerCS = new StHIRecoJets("HIRecoCS", fJetConstituentCut, kFALSE, out.Data());
        HIRecoJetMakerCS->SetJetType(fJetType);          // jetType
        HIRecoJetMakerCS->SetJetAlgo(antikt_algorithm);  // jetAlgo
        HIRecoJetMakerCS->SetRecombScheme(E_scheme); // recombination scheme
        HIRecoJetMakerCS->SetRadius(fJetRadius);         // jet radius
        HIRecoJetMakerCS->SetJetsName("Jets");
        HIRecoJetMakerCS->SetMinJetPt(0.0);             // 15.0 signal jets
        HIRecoJetMakerCS->SetMaxJetTrackPt(30.0);        // max track constituent
        HIRecoJetMakerCS->SetMinJetTowerE(fJetConstituentCut);  // 2.0 correlations
        HIRecoJetMakerCS->SetHadronicCorrFrac(1.0);      // fractional hadronic correction
        HIRecoJetMakerCS->SetJetHadCorrType(StJetFrameworkPicoBase::kAllMatchedTracks); // set for default options:  kLastMatchedTrack, kHighestEMatchedTrack, kAllMatchedTracks
        HIRecoJetMakerCS->SetGhostArea(0.005);           // ghost area
        HIRecoJetMakerCS->SetMinJetArea(0.0);            // minimum jet area
        HIRecoJetMakerCS->SetJetEtaRange(-1.0, 1.0); // fiducial eta acceptance
        HIRecoJetMakerCS->SetJetPhiRange(0,2.0*pi);      // phi acceptance
        HIRecoJetMakerCS->SetUsePrimaryTracks(usePrimaryTracks);
        HIRecoJetMakerCS->SetRunFlag(RunFlag);           // run flag      
        HIRecoJetMakerCS->SetdoppAnalysis(dopp);         // pp switch
        HIRecoJetMakerCS->SetEventZVtxRange(ZVtxMin, ZVtxMax);     // can be tighter for Run16 (-20,20)
        HIRecoJetMakerCS->SetTurnOnCentSelection(doCentSelection);
        HIRecoJetMakerCS->SetdoConstituentSubtr(kTRUE); 
        HIRecoJetMakerCS->SetCentralityBinCut(CentralitySelection);
        HIRecoJetMakerCS->SetEmcTriggerEventType(EmcTriggerEventType);  // kIsHT1 or kIsHT2 or kIsHT3
        HIRecoJetMakerCS->SetTriggerToUse(TriggerToUse);
        HIRecoJetMakerCS->SetRejectBadRuns(RejectBadRuns);    // switch to load and than omit bad runs
        HIRecoJetMakerCS->SetPrintLevel(0);
        cout << HIRecoJetMakerCS->GetName() << endl;
        
        StHIJetSaverWithCS *HIJetSaverMaker = new StHIJetSaverWithCS("HIJetSaver", picoMaker, jets.Data(), out.Data());

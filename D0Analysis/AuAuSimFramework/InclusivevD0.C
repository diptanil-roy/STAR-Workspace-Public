...
void InclusivevD0(const Char_t *inputFile="Run12.list", const Char_t *outputFile="Test_NoHIOverlay.root", Int_t nEv = 1, const Char_t *fOutJobappend="")
{
//        Int_t nEvents = 10000;
        Int_t nEvents = 1000;
//        Int_t nEvents = 1000000;
	...
	//StPicoDstMaker *picoMaker = new StPicoDstMaker(0,inputFile,"picoDst");
        ...
        StTagD0MCEvents       *D0MCTaggerSmear;
        StMCD0JetMaker        *jetTaskD0;
        StMCD0JetMaker        *jetTaskInclusive;

        D0MCTaggerSmear = new StTagD0MCEvents(Form("D0MCTagger%s", "") , picoMaker, outputFile);
        D0MCTaggerSmear->SetUsePrimaryTracks(usePrimaryTracks);       // use primary tracks
        D0MCTaggerSmear->SetTurnOnCentSelection(doCentSelection);    // run analysis for specific centrality: BOOLEAN
        D0MCTaggerSmear->SetCentralityBinCut(CentralitySelection);   // specific centrality range to run: if above is FALSE, this doesn't matter
        D0MCTaggerSmear->SetdoppAnalysis(dopp);
        D0MCTaggerSmear->FixPoorlySmearedTracks();
        D0MCTaggerSmear->SetAnalysisCentralityBin(1);                // 0 - 10 -> 1,
        D0MCTaggerSmear->SetDebugLevel(0);
        cout << D0MCTaggerSmear->GetName() << endl;

        /*jetTaskD0 = new StMCD0JetMaker(Form("JetTaskD0%s", ""), fJetConstituentCut, D0MCTaggerSmear->GetName(), kTRUE, outputFile);
        jetTaskD0->SetJetType(fJetType);          // jetType
        jetTaskD0->SetJetAlgo(antikt_algorithm);  // jetAlgo
        jetTaskD0->SetRecombScheme(E_scheme); // recombination scheme
        jetTaskD0->SetRadius(0.4);         // jet radius
        jetTaskD0->SetJetsName("Jets");
        jetTaskD0->SetMinJetPt(0.0);             // 15.0 signal jets
        jetTaskD0->SetMaxJetTrackPt(30.0);        // max track constituent
        jetTaskD0->SetMinJetTowerE(fJetConstituentCut);  // 2.0 correlations
        jetTaskD0->SetHadronicCorrFrac(1.0);      // fractional hadronic correction
        jetTaskD0->SetJetHadCorrType(StJetFrameworkPicoBase::kAllMatchedTracks); // set for default options:  kLastMatchedTrack, kHighestEMatchedTrack, kAllMatchedTracks
        jetTaskD0->SetGhostArea(0.005);           // ghost area
        jetTaskD0->SetMinJetArea(0.0);            // minimum jet area
        jetTaskD0->SetJetEtaRange(-1.0, 1.0); // fiducial eta acceptance
        jetTaskD0->SetJetPhiRange(0,2.0*pi);      // phi acceptance
        jetTaskD0->SetUsePrimaryTracks(usePrimaryTracks);
        jetTaskD0->SetRunFlag(RunFlag);           // run flag      
        jetTaskD0->SetdoppAnalysis(dopp);         // pp switch
        jetTaskD0->SetEventZVtxRange(ZVtxMin, ZVtxMax);     // can be tighter for Run16 (-20,20)
        jetTaskD0->SetTurnOnCentSelection(doCentSelection);
        jetTaskD0->SetdoConstituentSubtr(doConstituentSubtraction); 
        jetTaskD0->SetCentralityBinCut(CentralitySelection);
        jetTaskD0->SetEmcTriggerEventType(EmcTriggerEventType);  // kIsHT1 or kIsHT2 or kIsHT3
        jetTaskD0->SetTriggerToUse(TriggerToUse);
        jetTaskD0->SetRejectBadRuns(RejectBadRuns);    // switch to load and than omit bad runs
        jetTaskD0->SetD0Analysis(kTRUE);
        jetTaskD0->SetD0Kind(0);
        cout << jetTaskD0->GetName() << endl;

        jetTaskInclusive = new StMCD0JetMaker(Form("JetTaskInclusive%s", ""), fJetConstituentCut, D0MCTaggerSmear->GetName(), kTRUE, outputFile);
        jetTaskInclusive->SetJetType(fJetType);          // jetType
        jetTaskInclusive->SetJetAlgo(antikt_algorithm);  // jetAlgo
        jetTaskInclusive->SetRecombScheme(E_scheme); // recombination scheme
        jetTaskInclusive->SetRadius(0.4);         // jet radius
        jetTaskInclusive->SetJetsName("Jets");
        jetTaskInclusive->SetMinJetPt(0.0);             // 15.0 signal jets
        jetTaskInclusive->SetMaxJetTrackPt(30.0);        // max track constituent
        jetTaskInclusive->SetMinJetTowerE(fJetConstituentCut);  // 2.0 correlations
        jetTaskInclusive->SetHadronicCorrFrac(1.0);      // fractional hadronic correction
        jetTaskInclusive->SetJetHadCorrType(StJetFrameworkPicoBase::kAllMatchedTracks); // set for default options:  kLastMatchedTrack, kHighestEMatchedTrack, kAllMatchedTracks
        jetTaskInclusive->SetGhostArea(0.005);           // ghost area
        jetTaskInclusive->SetMinJetArea(0.0);            // minimum jet area
        jetTaskInclusive->SetJetEtaRange(-1.0, 1.0); // fiducial eta acceptance
        jetTaskInclusive->SetJetPhiRange(0,2.0*pi);      // phi acceptance
        jetTaskInclusive->SetUsePrimaryTracks(usePrimaryTracks);
        jetTaskInclusive->SetRunFlag(RunFlag);           // run flag      
        jetTaskInclusive->SetdoppAnalysis(dopp);         // pp switch
        jetTaskInclusive->SetEventZVtxRange(ZVtxMin, ZVtxMax);     // can be tighter for Run16 (-20,20)
        jetTaskInclusive->SetTurnOnCentSelection(doCentSelection);
        jetTaskInclusive->SetdoConstituentSubtr(doConstituentSubtraction); 
        jetTaskInclusive->SetCentralityBinCut(CentralitySelection);
        jetTaskInclusive->SetEmcTriggerEventType(EmcTriggerEventType);  // kIsHT1 or kIsHT2 or kIsHT3
        jetTaskInclusive->SetTriggerToUse(TriggerToUse);
        jetTaskInclusive->SetRejectBadRuns(RejectBadRuns);    // switch to load and than omit bad runs
        jetTaskInclusive->SetD0Analysis(kFALSE);
        jetTaskInclusive->SetD0Kind(0);
        cout << jetTaskInclusive->GetName() << endl;*/

...
int StHIRecoJets::Make()
{
  ...
  for (int iter = 0; iter < numberofrecoevents; iter++)
  {
    ...
    FindJets(iter);
    if (!doConstituentSubtr) FillJetBranch(iter);
    else FillJetBGBranch(iter);
  }
...
void StHIRecoJets::FillJetBGBranch(int iteration)
{
  ...
  for (UInt_t ijet = 0, jetCount = 0; ijet < jets_incl.size(); ++ijet)
  {
    ...
    if (d0Jet && fPrintLevel) cout << Form("%s:: Event Info = %i, Rho = %f; Jet pT = %f \t %f;  D0 pT = %f, Delta R = %f", GetName(), fFull_Event.size(), rho, jets_incl[ij].perp(), jet->Pt(), constituents[d0index].perp(), deltar) << endl;
  } // jet loop

  // TClonesArray *tempjets = (TClonesArray *)fJets->Clone();
  fJetsArr.push_back(fJets);

...
Bool_t StEmcPosition2::projTrack(TVector3* atFinal, TVector3* momentumAtFinal, 
                                                                const StPicoTrack* const trk, TVector3 mVertex, double magField, double radius, int option) const
{
    TVector3 Zero(0,0,0);
    *atFinal=Zero;
    ...
    */
    ...
    StPicoPhysicalHelix helix = trk->helix(magField);

    // if (trk->isPrimary()) helix.moveOrigin(helix.pathLength(mVertex));

    // StPhysicalHelixD helix = track->outerHelix();
    TVector3 momentum = trk->gMom(mVertex, magField);

    // if (trk->isPrimary()) momentum = trk->pMom(); 

    // const TVector3 momentum = trk->pMom();
    ...
    if (goProj) 
        {
            *atFinal = helix.at( s );
            *momentumAtFinal = helix.momentumAt( s, magField*(1.e-14) );
...
Bool_t StEmcPosition2::projTrack(TVector3* atFinal, TVector3* momentumAtFinal, 
                                                                TVector3 gMom, TVector3 mVertex, double magField, int charge, double radius, int option) const
{
    TVector3 Zero(0,0,0);
    *atFinal=Zero;
    *momentumAtFinal=Zero;
    ...
    */
    ...
    // StPhysicalHelixD helix = track->outerHelix();
    TVector3 momentum = gMom;
    StPicoPhysicalHelix helix(momentum, mVertex, magField * kilogauss, charge);

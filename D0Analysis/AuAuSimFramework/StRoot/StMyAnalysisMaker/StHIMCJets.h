...
class StHIMCJets : public StMaker {
 public:
  ...
  StHIMCJets();
  StHIMCJets(const char *name, double mintrackPt, bool dohistos, const char* outName);
  virtual ~StHIMCJets();
 ...
 protected:
  ...
  void                   FillJetConstituents(int iteration, StJet *jet, std::vector<fastjet::PseudoJet>& constituents,
...
//  Bool_t                 mTowerStatusArr[4800];
  ...
  StHIMCJets(const StHIMCJets&);            // not implemented
  StHIMCJets &operator=(const StHIMCJets&); // not implemented

  ClassDef(StHIMCJets, 4) // Jet producing task
};
#endif

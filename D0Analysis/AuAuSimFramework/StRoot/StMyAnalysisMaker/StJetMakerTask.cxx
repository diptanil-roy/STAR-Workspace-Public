...
void StJetMakerTask::FindJets()
{
  ...
  if((fJetType == kFullJet) || (fJetType == kChargedJet)) {
    for(unsigned short iTracks = 0; iTracks < ntracks; iTracks++){
      ...
      int matchedTowerIndex;

      if (usedcahelix) matchedTowerIndex = abs(GetMatchedBtowID(trk)) - 1; // towerIndex = towerID - 1
      else matchedTowerIndex = trk->bemcTowerIndex();
...
//________________________________________________________________________
void StJetMakerTask::FindJetsWithRandomTrack()
{
  // clear out existing wrapper object
  fjw.Clear();
  fFull_Event.clear();
  ...
  if((fJetType == kFullJet) || (fJetType == kChargedJet)) {
    ...
    double px = hardtrack.x();
    double py = hardtrack.y();
    double pz = hardtrack.z();
    double p = hardtrack.Mag();
    double energy = 1.0*TMath::Sqrt(p*p + pi0mass*pi0mass);
    ...
    // add track input vector to FastJet
    if(doCorrectTracksforEffBeforeJetReco) {  // kTRUE - corrects tracks before FastJet reconstruction
      fjw.AddInputVector(pxCorr, pyCorr, pzCorr, energyCorr, 9998); // efficiency corrected components
    } else {
      // THIS IS DEFAULT !
      // send track info to FJ wrapper
      fjw.AddInputVector(px, py, pz, energy, 9998); // includes E
    }

    // add track input vector to FastJet
    if(doCorrectTracksforEffBeforeJetReco) {  // kTRUE - corrects tracks before FastJet reconstruction
      fjw.AddInputVector(-pxCorr, -pyCorr, -pzCorr, energyCorr, 9999); // efficiency corrected components
    } else {
      // THIS IS DEFAULT !
      // send track info to FJ wrapper
      fjw.AddInputVector(-px, -py, -pz, energy, 9999); // includes E
    }
    ...
    if(doConstituentSubtr) {
      if(doCorrectTracksforEffBeforeJetReco) {  // kTRUE - corrects tracks before FastJet reconstruction
        fastjet::PseudoJet particle_Track(pxCorr, pyCorr, pzCorr, energyCorr); // efficiency corrected components
        particle_Track.set_user_index(9998);
        fFull_Event.push_back(particle_Track);
      } else {
        // THIS IS DEFAULT !
        fastjet::PseudoJet particle_Track(px, py, pz, energy);
    ...
    if(doConstituentSubtr) {
      if(doCorrectTracksforEffBeforeJetReco) {  // kTRUE - corrects tracks before FastJet reconstruction
        fastjet::PseudoJet particle_Track(-pxCorr, -pyCorr, -pzCorr, energyCorr); // efficiency corrected components
        particle_Track.set_user_index(9999);
        fFull_Event.push_back(particle_Track);
      } else {
        // THIS IS DEFAULT !
        fastjet::PseudoJet particle_Track(-px, -py, -pz, energy);
...
int StJetMakerTask::GetMatchedBtowID(StPicoTrack *trk){

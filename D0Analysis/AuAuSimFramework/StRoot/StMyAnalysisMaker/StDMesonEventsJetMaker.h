#ifndef STDMESONEVENTSJETMAKER_H
#define STDMESONEVENTSJETMAKER_H
...
class StDMesonEventsJetMaker : public StMaker {
 public:
  ...
  StDMesonEventsJetMaker();
  StDMesonEventsJetMaker(const char *name, double mintrackPt, bool dohistos, const char* outName);
  virtual ~StDMesonEventsJetMaker();
  ...
  void    FindJetsWithDStar(Int_t trk1, Int_t trk2, Int_t trk3);
  ...
  void         SetDStarAnalysis(bool dstar)               { fDStarAnalysis = dstar; }
  ...
  void         SetDStarKind(Int_t kind)                   { fDStarKind        = kind  ; }
 ...
 protected:
  ...
  Bool_t                 fDStarAnalysis;             // Switch on dstaranalysis
 ...
 private:
  ...
  // D0 tagger objects
  StTagDMesonEvents         *mD0Tagger;               // D0 Tagger object
...
//  Bool_t                 mTowerStatusArr[4800];
  ...
  StDMesonEventsJetMaker(const StDMesonEventsJetMaker&);            // not implemented
  StDMesonEventsJetMaker &operator=(const StDMesonEventsJetMaker&); // not implemented

  ClassDef(StDMesonEventsJetMaker, 4) // Jet producing task
};
#endif

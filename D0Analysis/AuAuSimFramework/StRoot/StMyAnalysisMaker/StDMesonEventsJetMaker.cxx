...
const Int_t StDMesonEventsJetMaker::fgkConstIndexShift = 100000;

ClassImp(StDMesonEventsJetMaker)

//________________________________________________________________________
StDMesonEventsJetMaker::StDMesonEventsJetMaker() : 
  StMaker(),
  fD0Analysis(kFALSE),
  fDStarAnalysis(kFALSE),
...
//  mVertex(0x0),
  ...
  fjw("StDMesonEventsJetMaker", "StDMesonEventsJetMaker"),
  fRadius(0.4),
  fMinJetArea(0.001),
  fMinJetPt(1.0),
...
//________________________________________________________________________
StDMesonEventsJetMaker::StDMesonEventsJetMaker(const char *name, double mintrackPt = 0.20, bool doHistos = kFALSE, const char* outName = "") : 
  StMaker(name),
  fD0Analysis(kFALSE),
  fDStarAnalysis(kFALSE),
...
//
//________________________________________________________________________
Int_t StDMesonEventsJetMaker::Init() {
  DeclareHistograms();
...
Int_t StDMesonEventsJetMaker::Finish() {
/*
  //  Summarize the run.
...
//________________________________________________________________________
void StDMesonEventsJetMaker::DeclareHistograms() {
  // constants
...
// Write histograms
//________________________________________________________________________
void StDMesonEventsJetMaker::WriteHistograms() {
...
//________________________________________________________________________
void StDMesonEventsJetMaker::WriteHadCorrQAHistograms() {
...
void StDMesonEventsJetMaker::Clear(Option_t *opt) {
  fJets->Clear();
  fJetsBGsub->Clear();
}
//
...
//________________________________________________________________________________
int StDMesonEventsJetMaker::Make()
{
  ...
  if (fDStarAnalysis || fD0Analysis) {
    mD0Tagger = static_cast<StTagDMesonEvents*>(GetMaker("DMesons"));
    if(!mD0Tagger) {
      LOG_WARN << " No DMesonTagger! Skip! " << endm;
      return kStWarn;
    }
    ...
    if (fDStarAnalysis){
      ...
      if ( fDStarKind==0 && !mD0Tagger->DoesEventHaveDStar()) return kStOK;
      if ( fDStarKind==1 && !mD0Tagger->DoesEventHaveDStarLS()) return kStOK;
      ...
      for (id0candidate = 0; id0candidate < d0TrackIndices.size(); id0candidate++){
        ...
        iTrack3 = d0TrackIndices[id0candidate][2];

        FindJetsWithDStar(iTrack1, iTrack2, iTrack3);
          // Fill jet branch
...
//________________________________________________________________________
void StDMesonEventsJetMaker::FindJets(TObjArray *tracks, TObjArray *towers, Int_t algo, Double_t radius)
{
...
//________________________________________________________________________
void StDMesonEventsJetMaker::FindJets()
{
...
void StDMesonEventsJetMaker::FindJetsWithDStar(Int_t track1, Int_t track2, Int_t track3)
{
  // clear out existing wrapper object
  fjw.Clear();
  fFull_Event.clear();
  ...
  if((fJetType == kFullJet) || (fJetType == kChargedJet)) {
    ...
    for(unsigned short iTracks = 0; iTracks < ntracks; iTracks++){
      // get track pointer
      if (iTracks==track1 || iTracks==track2 || iTracks==track3) continue;
    ...
    StPicoTrack *trk3 = static_cast<StPicoTrack*>(mPicoDst->track(track3));
    
    if (trk1 && trk2 && trk3){
      if (AcceptJetTrack(trk1, Bfield, mVertex) || AcceptJetTrack(trk2, Bfield, mVertex) || AcceptJetTrack(trk3, Bfield, mVertex)){
        TVector3 mTrkD0Mom;
        if(doUsePrimTracks) { 
          // get primary track vector
          mTrkD0Mom = trk1->pMom() + trk2->pMom() + trk3->pMom(); 
        } else { 
          // get global track vector
          mTrkD0Mom = trk1->gMom(mVertex, Bfield) + trk2->gMom(mVertex, Bfield) + trk3->gMom(mVertex, Bfield); 
        }
        ...
        if((pt > fMinJetTrackPt && pt < fMaxJetTrackPt) && (eta > fJetTrackEtaMin) && (eta < fJetTrackEtaMax) && (phi > fJetTrackPhiMin) && (phi < fJetTrackPhiMax)){
          ...
          int matchedTowerIndex3 = trk3->bemcTowerIndex(); // towerIndex = towerID - 1
          ...
          mTowerMatchTrkIndex[ matchedTowerIndex3 ][ mTowerStatusArr[ matchedTowerIndex3 ]] = trackIndex3;
          mTowerStatusArr[ matchedTowerIndex3 ] = mTowerStatusArr[ matchedTowerIndex3 ] + 1;  // 1+ means match, 0 for no match 
...
void StDMesonEventsJetMaker::FindJetsWithD0s(Int_t track1, Int_t track2)
{
  // clear out existing wrapper object
  fjw.Clear();
  fFull_Event.clear();
 ...
 */
void StDMesonEventsJetMaker::FillJetBranch()
{
 ...
 */
void StDMesonEventsJetMaker::FillJetConstituents(StJet *jet, std::vector<fastjet::PseudoJet>& constituents,
    std::vector<fastjet::PseudoJet>& constituents_unsub, Int_t flag, TString particlesSubName)
{
  ...
  for(UInt_t ic = 0; ic < constituents.size(); ++ic) {
    ...
    if(uid >= 0 && uid < 99999) {
      ...
      if (iTrack3 != -99) trk3 = static_cast<StPicoTrack*>(mPicoDst->track(iTrack3));
      ...
      if (trk1 && trk2){
        if (AcceptJetTrack(trk1, Bfield, mVertex) && AcceptJetTrack(trk2, Bfield, mVertex)){
          ...
          TVector3 mD0Mom;
          if (iTrack3 != -99){ mD0Mom = mTrk1Mom + mTrk2Mom + mTrk3Mom; }
          else { mD0Mom = mTrk1Mom + mTrk2Mom; }
 ...
 */
Bool_t StDMesonEventsJetMaker::GetSortedArray(Int_t indexes[], std::vector<fastjet::PseudoJet> array) const
{
 ...
 */
Bool_t StDMesonEventsJetMaker::IsLocked() const
{
  if (fLocked) {
...
//________________________________________________________________________
Bool_t StDMesonEventsJetMaker::AcceptTrack(StPicoTrack *trk, Float_t B, TVector3 Vert) {
...
//________________________________________________________________________
Bool_t StDMesonEventsJetMaker::AcceptJetTrack(StPicoTrack *trk, Float_t B, TVector3 Vert) {
  // constants: assume neutral pion mass
  ///double pi0mass = Pico::mMass[0]; // GeV
  double pi = 1.0*TMath::Pi();
...
// Tower Quality Cuts
//________________________________________________________________________
Bool_t StDMesonEventsJetMaker::AcceptJetTower(StPicoBTowHit *tower, Int_t towerID) {
  // constants:
  double pi = 1.0*TMath::Pi();
...
//________________________________________________________________________
Int_t StDMesonEventsJetMaker::GetCentBin(Int_t cent, Int_t nBin) const
{
  Int_t centbin = -1;
...
Bool_t StDMesonEventsJetMaker::GetMomentum(TVector3 &mom, const StPicoBTowHit* tower, Double_t mass, Int_t towerID, Double_t CorrectedEnergy) const {
...
void StDMesonEventsJetMaker::FillEmcTriggersArr() {
...
Double_t StDMesonEventsJetMaker::GetMaxTrackPt()
{
  // get # of tracks
...
//_________________________________________________________________________________________________
Double_t StDMesonEventsJetMaker::GetMaxTowerEt()
{
  // get # of towers
 ...
 */
void StDMesonEventsJetMaker::FillJetBGBranch()
{
   // E_scheme, pt_scheme, pt2_scheme, Et_scheme, Et2_scheme, BIpt_scheme, BIpt2_scheme, WTA_pt_scheme, WTA_modp_scheme
   fastjet::RecombinationScheme    recombScheme;
   if (fRecombScheme == 0)     recombScheme = fastjet::E_scheme;
...
// set sum weights
//________________________________________________________________________
void StDMesonEventsJetMaker::SetSumw2() {
...
void StDMesonEventsJetMaker::RunEventQA() {

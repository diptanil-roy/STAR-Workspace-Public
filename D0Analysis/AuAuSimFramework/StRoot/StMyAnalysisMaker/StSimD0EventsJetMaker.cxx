// ################################################################
// Author:  Diptanil Roy for the STAR Collaboration
...
const Int_t StSimD0EventsJetMaker::fgkConstIndexShift = 100000;

ClassImp(StSimD0EventsJetMaker)

//________________________________________________________________________
StSimD0EventsJetMaker::StSimD0EventsJetMaker() : 
  StMaker(),
  fD0Analysis(kFALSE),
  fDrop(kFALSE),
...
//  mVertex(0x0),
  ...
  fjw("StSimD0EventsJetMaker", "StSimD0EventsJetMaker"),
  fRadius(0.4),
  fMinJetArea(0.001),
  fMinJetPt(1.0),
...
//________________________________________________________________________
StSimD0EventsJetMaker::StSimD0EventsJetMaker(const char *name, double mintrackPt = 0.20, const char *d0taggername = "", const char *mcjetmakername = "", bool doHistos = kFALSE, const char* outName = "") : 
  StMaker(name),
  fD0Analysis(kFALSE),
  fDrop(kFALSE),
  ...
  fPrintLevel(0)
{
  ...
  for(int i=0; i<4800; i++) {
    ...
    mTowerToTrack[i].clear();
    mTowerToTrackE[i].clear();
  }
...
//
//________________________________________________________________________
Int_t StSimD0EventsJetMaker::Init() {
  DeclareHistograms();
...
Int_t StSimD0EventsJetMaker::Finish() {
/*
  //  Summarize the run.
...
//________________________________________________________________________
void StSimD0EventsJetMaker::DeclareHistograms() {
  // constants
  ...
  fHistTrackToTowerIndex = new TH1F("fHistTrackToTowerIndex", "# tracks with given matched tower index", 4802, -1, 4800);
...
// Write histograms
//________________________________________________________________________
void StSimD0EventsJetMaker::WriteHistograms() {
...
//________________________________________________________________________
void StSimD0EventsJetMaker::WriteHadCorrQAHistograms() {
...
void StSimD0EventsJetMaker::Clear(Option_t *opt) {
  fJets->Clear();
  fJetsBGsub->Clear();
}
//
...
//________________________________________________________________________________
int StSimD0EventsJetMaker::Make()
{
  if (fPrintLevel) cout << "Called StSimD0EventsJetMaker" << endl;
  //  cout << "SIM D0 JET MAKER" << endl;
  ...
  if((zVtx < fEventZVtxMinCut) || (zVtx > fEventZVtxMaxCut)) return kStOk;
    ...
    if (fMCEventsWithoutCent) fCentralityScaled = 0;
  ...
  if (fD0Analysis) {
    ...
    d0RecoMomenta = mD0Tagger->GetD0RecoMomenta();
    droppedTrackIndices = mD0Tagger->DropTracksBasedOnEfficiencyWeights();
    trackssmearedwithfastsim = mD0Tagger->GetTracksSmearedWell();

    // cout << "D0 Tracks = " << d0TrackIndices.size() << endl;

    // d0TrackIndices.push_back({11, 20, 1.865});

    // cout << "D0 Tracks after extra track added = " << d0TrackIndices.size() << endl;

    if (d0MCTrackIndices.size() != d0RecoTrackIndices.size() || d0RecoTrackIndices.size() != d0RecoMomenta.size()) {
      cout << d0MCTrackIndices.size() << "\t" << d0RecoTrackIndices.size() << "\t" << d0RecoMomenta.size() << endl;
      cout << "Yo wut!!!!!" << endl;
      return kStWarn;
    }

    mcJetMaker = static_cast<StMCD0JetMaker*>(GetMaker(fMCTaggerName));
    const char *mcJetMakerNameCh = fMCTaggerName;
    if(!mcJetMaker) {
    ...
    // cout << "======================= Event Number After MC = " << mPicoEvent->runId() << "\t" << mPicoEvent->eventId()<< "\t" << "=======================" << endl;

    for (id0candidate = 0; id0candidate < d0MCTrackIndices.size(); id0candidate++){

      // cout << "Candidate # " << id0candidate << endl;

      fJets->Delete();
      fJetsBGsub->Delete();

      if (mcJetsArr[id0candidate]->GetEntries() == 0){
      ...
      StJet *mjet = static_cast<StJet*>(mcJetsArr[id0candidate]->At(0));
      if(!mjet) continue;

      // if (mjet->Pt() < 5.) continue;

      // cout << "MC Jet pT = " << mjet->Pt() << endl;

      // cout << "======================= Event Number After MC = " << mPicoEvent->runId() << "\t" << mPicoEvent->eventId()<< "\t" << "=======================" << endl;

      iD0 = id0candidate;

      mcTrack1 = d0MCTrackIndices[id0candidate][0];
      mcTrack2 = d0MCTrackIndices[id0candidate][1];


      recoTrack1 = d0RecoTrackIndices[id0candidate][0];
      recoTrack2 = d0RecoTrackIndices[id0candidate][1];

      mTrkD0MomFromFastSim = d0RecoMomenta[id0candidate];

      FindRecoJetsWithD0s(iD0);
        // Fill jet branch
      ...
      //   cout << "Number of Jets in JetMaker = " <<  fJetsArr[jetcollection]->GetEntries() << endl;
...
void StSimD0EventsJetMaker::FindRecoJetsWithD0s(Int_t D0)
{

  if (fPrintLevel) cout << "Reco Jets For D0 = " << D0 << endl;
  ...
  if((fJetType == kFullJet) || (fJetType == kChargedJet)) {
    ...
    for(unsigned short iTracks = 0; iTracks < ntracks; iTracks++){
      ...
      if (iTracks==d0RecoTrackIndices[id0candidate][0] || iTracks==d0RecoTrackIndices[id0candidate][1]) isatrackfromD0 = kTRUE;

      double pt = trackssmearedwithfastsim[iTracks][0];
      double phi = trackssmearedwithfastsim[iTracks][1];
      if(phi < 0.0)    phi += 2.0*pi;  // force from 0-2pi
      if(phi > 2.0*pi) phi -= 2.0*pi;  // force from 0-2pi
      double eta = trackssmearedwithfastsim[iTracks][2];
      double px = trackssmearedwithfastsim[iTracks][3];
      double py = trackssmearedwithfastsim[iTracks][4];
      double pz = trackssmearedwithfastsim[iTracks][5];
      double p = trackssmearedwithfastsim[iTracks][6];
      double energy = trackssmearedwithfastsim[iTracks][7];
      short charge = trackssmearedwithfastsim[iTracks][8];
      int matchedTowerIndex = trackssmearedwithfastsim[iTracks][9]; // towerIndex = towerID - 1
      ...
      StPicoMcTrack *mctrk;
      if (trk->idTruth() - 1 >= 0)mctrk = static_cast<StPicoMcTrack*>(mPicoDst->mcTrack(trk->idTruth() - 1));

      // if (trk->idTruth() - 1 >= 0)cout << Form("Track # %i \t %i \t %.2f \t %.2f", iTracks, mctrk->geantId(), pt, eta) << endl;
      // else cout << Form("Track # %i \t %.2f \t %.2f = ", iTracks, pt, eta) << endl; 

      if (fPrintLevel == -19){
        if (trk->idTruth() - 1 >= 0)cout << Form("Track # %i \t %i \t %i \t %.2f \t %.2f \t %i \t %i \t %.2f \t %i \t %.2f", iTracks, trk->idTruth() - 1, mctrk->geantId(), pt, eta, isatrackfromD0, matchedTowerIndex, dca, nHitsFit, nHitsRatio) << endl;
        else cout << Form("Track # %i \t %.2f \t %.2f \t %i \t %.2f \t %.2f", iTracks, pt, eta, matchedTowerIndex, dca, nHitsRatio) << endl;
      } 
      ...
      if (isatrackfromD0){ 
        if (matchedTowerIndex >= 0){
          mTowerMatchTrkIndex[ matchedTowerIndex ][ mTowerStatusArr[ matchedTowerIndex ]] = iTracks;
          mTowerStatusArr[ matchedTowerIndex ] = mTowerStatusArr[ matchedTowerIndex ] + 1;  // 1+ means match, 0 for no match

          mTowerToTrack[ matchedTowerIndex ].push_back(iTracks);
      ...
      if (fPrintLevel){
        cout << "Track DCA NHitsFit NHitsRatio = " << dca << "\t" << nHitsFit << "\t" << nHitsRatio << endl;
      }
      ...
      // This is here because even though the tracks are dropped, we need to account for the energy they might have deposited on the EMC.
      // We toss the tracks, but maintain a record of the track in the tower loop, so that we can subtract its contribution from the tower energy.
      if (fDrop){
        bool droptrack = kFALSE;
        for (int i = 0; i < droppedTrackIndices[D0].size(); i++){
          if (iTracks == droppedTrackIndices[D0][i]) {
            if (fPrintLevel == -19){
              if (trk->idTruth() - 1 >= 0) cout << Form("Removed Track Input = %.2f \t %.2f \t %.2f \t %i", px, py, pz, mctrk->geantId()) << endl;
              else cout << Form("Removed Track Input = %.2f \t %.2f \t %.2f", px, py, pz) << endl;
            }
            droptrack = kTRUE;
            break;
          }
        }
        if (droptrack) continue;
      }
      ...
      if (fPrintLevel == -19){
        if (trk->idTruth() - 1 >= 0) cout << Form("Reco Level Track Input = %.2f \t %.2f \t %.2f \t %i", px, py, pz, mctrk->geantId()) << endl;
        else cout << Form("Reco Level Track Input = %.2f \t %.2f \t %.2f", px, py, pz) << endl;
      }

      // cout << Form("Reco Level Track Input = %.2f \t %.2f \t %.2f", px, py, pz) << endl;

      // if (trk->idTruth() - 1 >= 0) cout << "Reco Level Track Input = " << px << "\t" << py << "\t" << pz << "\t" << mctrk->geantId() << "\t" << dca << "\t" << nHitsFit << "\t" << nHitsRatio << endl;
      // else cout << "Reco Level Track Input = " << px << "\t" << py << "\t" << pz << "\t" << nHitsFit << "\t" << nHitsRatio << endl;

      if (fPrintLevel > 1){
        cout << "Constituents = " << "\t" << pt << "\t" << eta << "\t" << phi << endl;
      }
      ...
      mTowerToTrack[ matchedTowerIndex ].push_back(trackIndex);
      mTowerToTrackE[ matchedTowerIndex ].push_back(energy);

    }
    ///////////////////////////////////////////// D0 Bit ////////////////////////////////////////////////

    double pt = mTrkD0MomFromFastSim.Perp();
    double phi = mTrkD0MomFromFastSim.Phi();
    if(phi < 0.0)    phi += 2.0*pi;  // force from 0-2pi
    if(phi > 2.0*pi) phi -= 2.0*pi;  // force from 0-2pi
    double eta = mTrkD0MomFromFastSim.PseudoRapidity();
    double px = mTrkD0MomFromFastSim.X();
    double py = mTrkD0MomFromFastSim.Y();
    double pz = mTrkD0MomFromFastSim.Z();
    double p = mTrkD0MomFromFastSim.Mag();
    ...
    if((pt > fMinJetTrackPt && pt < fMaxJetTrackPt) && (eta > fJetTrackEtaMin) && (eta < fJetTrackEtaMax) && (phi > fJetTrackPhiMin) && (phi < fJetTrackPhiMax)){
      ...
      fjw.AddInputVector(px, py, pz, energy, 99999); // includes E

      fastjet::PseudoJet particle_Track(px, py, pz, energy);
      particle_Track.set_user_index(99999);
      fFull_Event.push_back(particle_Track);
    }

    if (fPrintLevel == -19) cout << Form("Reco Level Track Input D0 = %.2f \t %.2f \t %.2f ", px, py, pz) << endl;
    // cout << "Reco Level Track Input D0 = " << px << "\t" << py << "\t" << pz << endl;

    if (fPrintLevel > 1){
      cout << "D0 = " << "\t" << pt << "\t" << eta << "\t" << phi << endl;
    }

    // if (d0RecoTrackIndices[id0candidate][0] >= 0){
    //   StPicoTrack *trk1 = static_cast<StPicoTrack*>(mPicoDst->track(d0RecoTrackIndices[id0candidate][0]));

    //   int matchedTowerIndex1 = trk1->bemcTowerIndex();

    //   StPicoMcTrack *mctrk1;
    //   if (trk1->idTruth() - 1 >= 0)mctrk1 = static_cast<StPicoMcTrack*>(mPicoDst->mcTrack(trk1->idTruth() - 1));
    //   // if (trk1->idTruth() - 1 >= 0)cout << Form("Track # %i \t %i \t %.2f \t %.2f \t %i", d0RecoTrackIndices[id0candidate][0], mctrk1->geantId(), trackssmearedwithfastsim[d0RecoTrackIndices[id0candidate][0]][0], trackssmearedwithfastsim[d0RecoTrackIndices[id0candidate][0]][2], matchedTowerIndex1) << endl;
    //   // else cout << Form("Track # %i \t %.2f \t %.2f \t %i = ", d0RecoTrackIndices[id0candidate][0], trackssmearedwithfastsim[d0RecoTrackIndices[id0candidate][0]][0], trackssmearedwithfastsim[d0RecoTrackIndices[id0candidate][0]][2], matchedTowerIndex1) << endl;

      
    //   if (matchedTowerIndex1 >= 0) {
    //     mTowerMatchTrkIndex[ matchedTowerIndex1 ][ mTowerStatusArr[ matchedTowerIndex1 ]] = d0RecoTrackIndices[id0candidate][0];
    //     mTowerStatusArr[ matchedTowerIndex1 ] = mTowerStatusArr[ matchedTowerIndex1 ] + 1;  // 1+ means match, 0 for no match 
    //     mTowerToTrack[matchedTowerIndex1].push_back(d0RecoTrackIndices[id0candidate][0]);
    //     mTowerToTrackE[matchedTowerIndex1].push_back(1.0*TMath::Sqrt(trk1->gPtot()*trk1->gPtot() + pi0mass*pi0mass));
    //   }
    // }

    // if (d0RecoTrackIndices[id0candidate][1] >= 0){
    //   StPicoTrack *trk2 = static_cast<StPicoTrack*>(mPicoDst->track(d0RecoTrackIndices[id0candidate][1]));

    //   int matchedTowerIndex2 = trk2->bemcTowerIndex();

    //   StPicoMcTrack *mctrk2;
    //   if (trk2->idTruth() - 1 >= 0)mctrk2 = static_cast<StPicoMcTrack*>(mPicoDst->mcTrack(trk2->idTruth() - 1));
    //   // if (trk2->idTruth() - 1 >= 0)cout << Form("Track # %i \t %i \t %.2f \t %.2f \t %i", d0RecoTrackIndices[id0candidate][1], mctrk2->geantId(), trackssmearedwithfastsim[d0RecoTrackIndices[id0candidate][1]][0], trackssmearedwithfastsim[d0RecoTrackIndices[id0candidate][1]][2], matchedTowerIndex2) << endl;
    //   // else cout << Form("Track # %i \t %.2f \t %.2f \t %i= ", d0RecoTrackIndices[id0candidate][1], trackssmearedwithfastsim[d0RecoTrackIndices[id0candidate][1]][0], trackssmearedwithfastsim[d0RecoTrackIndices[id0candidate][1]][2], matchedTowerIndex2) << endl;

    //   if (matchedTowerIndex2 >= 0) {
    //     mTowerMatchTrkIndex[ matchedTowerIndex2 ][ mTowerStatusArr[ matchedTowerIndex2 ]] = d0RecoTrackIndices[id0candidate][1];
    //     mTowerStatusArr[ matchedTowerIndex2 ] = mTowerStatusArr[ matchedTowerIndex2 ] + 1;  // 1+ means match, 0 for no match 
    //     mTowerToTrack[matchedTowerIndex2].push_back(d0RecoTrackIndices[id0candidate][1]); 
    //     mTowerToTrackE[matchedTowerIndex2].push_back(1.0*TMath::Sqrt(trk2->gPtot()*trk2->gPtot() + pi0mass*pi0mass));
    //   }
    // }
  ...
  if((fJetType == kFullJet) || (fJetType == kNeutralJet)) {
    ...
    for(int itow = 0; itow < nTowers; itow++) {
      ...
      if(mTowerStatusArr[towerIndex] > 0.5 && towerEunCorr > mTowerEnergyMin) {
        ...
        for(int itrk = 0; itrk < mTowerStatusArr[towerIndex]; itrk++) {
          ...
          if(!AcceptTrack(trk, Bfield, mVertex)) { 
             //cou<<"track matched back doesn't pass cuts"<<endl;  continue; 
          }  
      ...
      double energy = 0;
      for (int i = 0; i < mTowerToTrack[itow].size(); i++){
        energy+= mTowerToTrackE[itow][i];
      }

      // cout << Form("FReco Tower = %i \t %.2f \t %.2f", towerID, towerEt, energy) << endl;
      // for (int i = 0; i < mTowerToTrack[itow].size(); i++){
      //   cout << Form("Track Index matched to Tower = %i \t %i \t %.2f", towerID, mTowerToTrack[itow][i], mTowerToTrackE[itow][i]) << endl;
      // }
      ...
      if (fPrintLevel > 1){
        cout << "Tower = " << "\t" << mom.Perp() << "\t" << mom.PseudoRapidity() << "\t" << mom.Phi() << endl;
      }
      ...
      // cout << "Reco Level Tower Input = " << towerPx << "\t" << towerPy << "\t" << towerPz << endl;
      if (fPrintLevel == -19) cout << Form("Reco Level Tower Input = %i \t %.2f \t %.2f \t %.2f \t %.2f", towerID, towerPx, towerPy, towerPz, towerE) << endl;

    } // tower loop

  }   // neutral/full jets

  // cout << "Input Loop Size = " << fFull_Event.size() << endl;
 ...
 */
void StSimD0EventsJetMaker::FillJetBranch()
{
  ...
  for(UInt_t ijet = 0, jetCount = 0; ijet < jets_incl.size(); ++ijet) {
    ...
    // if (fPrintLevel) 
      // cout << Form("Found Reco jet with Jet pT eta and D0 pT eta = %.2f \t %.2f \t %.2f \t %.2f ", jet->Pt(), jet->Eta(), constituents[D0id].perp(), constituents[D0id].eta()) << endl;

    // for(UInt_t ic = 0; ic < constituents.size(); ++ic) {
    //   if (constituents[ic].perp() >= 0.2) cout << Form("Found Constituent pT eta = %.2f \t %.2f ", constituents[ic].perp(), constituents[ic].eta()) << endl;
    // }
    ...
    // cout << "Reco Jet # : " << ijet << "\t" << jet->Pt() << "\t" <<  jet->Area() << "\t" << jet->GetNumberOfConstituents() << endl;

    //cout << Form("Added jet n. %d, pt = %f, area = %f, constituents = %d", jetCount, jet->Pt(), jet->Area(), jet->GetNumberOfConstituents()) << endl;

    // if (D0Jet) FillJetConstituents(jet, constituents, constituents);

    // if (D0Jet) cout << Form("Added jet n. %d, pt = %f, area = %f, constituents = %d", jetCount, jet->Pt(), jet->Area(), jet->GetNumberOfConstituents()) << endl;
 ...
 */
void StSimD0EventsJetMaker::FillJetConstituents(StJet *jet, std::vector<fastjet::PseudoJet>& constituents,
    std::vector<fastjet::PseudoJet>& constituents_unsub, Int_t flag, TString particlesSubName)
{

  // cout << "Called Fill Jet Constituents " << endl;
  ...
  for(UInt_t ic = 0; ic < constituents.size(); ++ic) {
    ...
    if(uid >= 0 && uid < 99999) {
     ...
     // cout << "Reco Jet Tracks : " << uid << "\t" << pt << "\t" << eta << "\t" << phi  << "\t" << trk->idTruth() << "\t"  << endl;

      StPicoMcTrack *mctrk = static_cast<StPicoMcTrack*>(mPicoDst->mcTrack(trk->idTruth() - 1));
      if(mctrk) {
       // cout << "Matched MC Track IdVtx : " << mctrk->idVtxStart() << "\t" << mctrk->idVtxStop() << "\t" << mctrk->pt() << "\t" << mctrk->eta() << "\t" << mctrk->p().Phi() << "\t" << mctrk->geantId() << endl;

        if (mctrk->idVtxStop() != 0){
         // cout << "What happened here : " << trk->nHits() << "\t" << (double)trk->nHitsFit()/(double)trk->nHitsMax() << endl;
        }
      }
      ...
      // cout << "Pt-Phi-Eta : " << pt << "\t" << phi << "\t" << eta << endl;
      ...
      // TODO 
      // ??? add histograms to a switch with tracking efficiency applied ???

      // increase track counter
      nt++;
    } else if (uid == 99999){ // D0 Bit
      // cout << "Constituent ID = " << uid << endl;
      ...
      if (recoTrack1 < -999 && recoTrack2 < -999){
        ...
        if (trk1 && trk2){
          if (AcceptJetTrack(trk1, Bfield, mVertex) && AcceptJetTrack(trk2, Bfield, mVertex)){
            TVector3 mD0Mom;
            if(doUsePrimTracks) { 
              // get primary track vector
              mD0Mom = trk1->pMom() + trk2->pMom(); 
            } else { 
              // get global track vector
              mD0Mom = trk1->gMom(mVertex, Bfield) + trk2->gMom(mVertex, Bfield); 
            }
            // track variables
            double pt = mD0Mom.Perp();
            double phi = mD0Mom.Phi();
            double eta = mD0Mom.PseudoRapidity();

           // cout << "Reco Jet Tracks : " << uid << "\t" << pt << endl;
...
// ============================================================================================================================
        ...
        if (fHistJetNTowervsPhivsEta->Fill(towerPhi, towerEta) == -1) printf("%d \n", __LINE__);
        if (fHistQATowIDvsEta->Fill(towerID, towerEta) == -1) printf("%d \n", __LINE__);
        if (fHistQATowIDvsPhi->Fill(towerID, towerPhi) == -1) printf("%d \n", __LINE__);
  ...
  if (fHistNJetsvsPhivsEta->Fill(jet->Phi(), jet->Eta()) == -1) printf("%d \n", __LINE__);
  ...
  if (fHistNJetTracksvsJetPt[cbin - 1]->Fill(jet->Pt(), nt) == -1) printf("%d \n", __LINE__);
  if (fHistNJetTowersvsJetPt[cbin - 1]->Fill(jet->Pt(), nc) == -1) printf("%d \n", __LINE__);
  if (fHistNJetConstituentsvsJetPt[cbin - 1]->Fill(jet->Pt(), nt + nc) == -1) printf("%d \n", __LINE__);
}
...
//________________________________________________________________________
void StSimD0EventsJetMaker::FindJets(TObjArray *tracks, TObjArray *towers, Int_t algo, Double_t radius)
{
...
//________________________________________________________________________
void StSimD0EventsJetMaker::FindJets()
{
  // clear out existing wrapper object
  fjw.Clear();
  fFull_Event.clear();
 ...
 */
Bool_t StSimD0EventsJetMaker::GetSortedArray(Int_t indexes[], std::vector<fastjet::PseudoJet> array) const
{
 ...
 */
Bool_t StSimD0EventsJetMaker::IsLocked() const
{
  if (fLocked) {
...
//________________________________________________________________________
Bool_t StSimD0EventsJetMaker::AcceptTrack(StPicoTrack *trk, Float_t B, TVector3 Vert) {
...
//________________________________________________________________________
Bool_t StSimD0EventsJetMaker::AcceptJetTrack(StPicoTrack *trk, Float_t B, TVector3 Vert) {
  // constants: assume neutral pion mass
  ///double pi0mass = Pico::mMass[0]; // GeV
  double pi = 1.0*TMath::Pi();
...
// Tower Quality Cuts
//________________________________________________________________________
Bool_t StSimD0EventsJetMaker::AcceptJetTower(StPicoBTowHit *tower, Int_t towerID) {
  // constants:
  double pi = 1.0*TMath::Pi();
...
//________________________________________________________________________
Int_t StSimD0EventsJetMaker::GetCentBin(Int_t cent, Int_t nBin) const
{
  Int_t centbin = -1;
...
Bool_t StSimD0EventsJetMaker::GetMomentum(TVector3 &mom, const StPicoBTowHit* tower, Double_t mass, Int_t towerID, Double_t CorrectedEnergy) const {
...
void StSimD0EventsJetMaker::FillEmcTriggersArr() {
...
Double_t StSimD0EventsJetMaker::GetMaxTrackPt()
{
  // get # of tracks
...
//_________________________________________________________________________________________________
Double_t StSimD0EventsJetMaker::GetMaxTowerEt()
{
  // get # of towers
 ...
 */
void StSimD0EventsJetMaker::FillJetBGBranch()
{
   // E_scheme, pt_scheme, pt2_scheme, Et_scheme, Et2_scheme, BIpt_scheme, BIpt2_scheme, WTA_pt_scheme, WTA_modp_scheme
   fastjet::RecombinationScheme    recombScheme;
   if (fRecombScheme == 0)     recombScheme = fastjet::E_scheme;
...
// set sum weights
//________________________________________________________________________
void StSimD0EventsJetMaker::SetSumw2() {
...
void StSimD0EventsJetMaker::RunEventQA() {
...
// StPicoTrack * StSimD0EventsJetMaker::SmearMom(TVector3 p, int pid)
// {
//     float pt = p.Perp();
//     float pt1 = pt;
...
//     if(pid ==8 || pid == 9){
//       sPt = gRandom->Gaus(pt, pt * fPionMomResolution->Eval(pt1));// Pion
//     }

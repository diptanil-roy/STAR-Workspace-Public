...
{
  if (fPrintLevel) cout << "Called StMCD0JetMaker" << endl;
  ...
  if (fD0Analysis) fHIOverlay = kFALSE;
  if (fD0Analysis) {
    ...
    if (fPrintLevel) cout << "Dropped Track Array Size At Jet Maker = " << DroppedMCTracks.size() << endl;

    if (fPrintLevel) cout << "Event = " << mPicoEvent->eventId() << endl;
    if (fPrintLevel) cout << "Number of D0s = " << d0TrackIndices.size() << endl;
...
void StMCD0JetMaker::FindJets(int iteration)
{
  if (fPrintLevel) cout << "This is D0 # " << iteration << "***********************" << endl;
  ...
  for(unsigned short iTracks = 0; iTracks < fMCTracks[iteration].size(); iTracks++){
    ...
    if (fPrintLevel) cout << "Constituents = " << pt << "\t" << eta << "\t" << phi << endl;
  ...
  if(fJetType == kFullJet){
    for (unsigned short iTowers = 0; iTowers < fMCTowers[iteration].size(); iTowers++){
      ...
      if (fPrintLevel) cout << "Towers = " << pt << "\t" << eta << "\t" << phi << endl;
...
void StMCD0JetMaker::FillJetBranch(int iteration)
{
  ...
  for(UInt_t ijet = 0, jetCount = 0; ijet < jets_incl.size(); ++ijet) {
    ...
    if (fPrintLevel) cout << "Found jet with Jet pT = " << jets_incl[ij].perp() << "\t" << jets_incl[ij].eta() << "\t" << jets_incl[ij].phi() << endl;
...
void StMCD0JetMaker::FillJetBranch()
{
  ...
  for(UInt_t ijet = 0, jetCount = 0; ijet < jets_incl.size(); ++ijet) {
    ...
    if (fPrintLevel) 
      cout << "Found jet with Jet pT = " << jet->Pt() << "\t" << constituents.size() << endl;
    ...
    std::vector<fastjet::PseudoJet>& constituents_unsub, Int_t flag, TString particlesSubName)
{
  ...
  for(UInt_t ic = 0; ic < constituents.size(); ++ic) {
    ...
    if(uid >= 0 && uid < 99999) {
      ...
      if (fPrintLevel > 1){
        if (charge != 0)cout << "Const = " << pt << "\t" << trk->geantId() << endl;
        else cout << "Towers = " << pt << "\t" << trk->geantId() << endl;
      }
      ...
      if (trk1 && trk2){
        //if (AcceptJetMCTrack(trk1) && AcceptJetMCTrack(trk2)){
          ...
          double mcD0px = mD0Mom.Px();
          double mcD0py = mD0Mom.Py();
          double mcD0pt = mD0Mom.Perp();

          double z = (mcjetpx*mcD0px + mcjetpy*mcD0py)/(pow(mcjetpt, 2));

          if (fPrintLevel) cout << "Found jet with D0 pT = " << mcD0pt << "\t" << "Z = " << z << endl;

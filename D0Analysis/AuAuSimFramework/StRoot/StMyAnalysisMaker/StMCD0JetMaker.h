#ifndef STMCD0JETMAKER_H
#define STMCD0JETMAKER_H
...
class StMCD0JetMaker : public StMaker {
 public:
  ...
  StMCD0JetMaker();
  StMCD0JetMaker(const char *name, double mintrackPt, const char *d0taggername, bool dohistos, const char* outName);
  virtual ~StMCD0JetMaker();
  ...
  void    FindMCInclusiveJets();
  void    FindMCJetsWithD0s(Int_t D0);

  void    FindJets(int iteration);
  void    FillJetBranch(int iteration);
  // switches
  ...
  void         SetHIOverlay(bool d0)                      { fHIOverlay    = d0; fD0Analysis    = kFALSE;}
 ...
 protected:
  ...
  Bool_t                 AcceptJetMCTrack(StPicoMcTrack *trk);
  ...
  Bool_t                 fHIOverlay; // Switch on HIOverlay
 ...
 private:
  ...
  StHIOverlay             *mHIOverlay;              // HIOverlay

  TClonesArray           *mcJets;


  std::vector<std::vector<double> >  d0TrackIndices;
  std::vector<std::vector<int> >     DroppedMCTracks; 
...
//  Bool_t                 mTowerStatusArr[4800];
  ...
  StMCD0JetMaker(const StMCD0JetMaker&);            // not implemented
  StMCD0JetMaker &operator=(const StMCD0JetMaker&); // not implemented

  ClassDef(StMCD0JetMaker, 4) // Jet producing task
};
#endif

#ifndef StHIRecoJets_H
#define StHIRecoJets_H
...
class StHIRecoJets : public StMaker {
 public:
  ...
  StHIRecoJets();
  StHIRecoJets(const char *name, double mintrackPt, bool dohistos, const char* outName);
  virtual ~StHIRecoJets();
  ...
  std::vector<double>  GetArrayofRhoValueCS()                   { return rhovalue; }
  std::vector<double>  GetArrayofSigmaValueCS()                   { return sigmavalue; }
...
//  Bool_t                 mTowerStatusArr[4800];
  ...
  StHIRecoJets(const StHIRecoJets&);            // not implemented
  StHIRecoJets &operator=(const StHIRecoJets&); // not implemented

  ClassDef(StHIRecoJets, 4) // Jet producing task
};
#endif

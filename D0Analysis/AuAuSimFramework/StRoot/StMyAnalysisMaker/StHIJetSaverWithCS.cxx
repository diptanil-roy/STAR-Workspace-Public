...
ClassImp(StHIJetSaverWithCS)

//________________________________________________________________________
StHIJetSaverWithCS::StHIJetSaverWithCS(const char* name, StPicoDstMaker *picoMaker, const char* outName1 = "", const char* outName2 = "") : StJetFrameworkPicoBase(name) //StMaker(name),
{
  ...
  fDebugLevel = 0;
  fRunFlag = 0;       // see StHIJetSaverWithCS::fRunFlagEnum
  doppAnalysis = kFALSE;
...
//
//________________________________________________________________________
StHIJetSaverWithCS::~StHIJetSaverWithCS()
{ /*  */
...
//________________________________________________________________________
Int_t StHIJetSaverWithCS::Finish() { 
  cout << "StHIJetSaverWithCS::Finish()\n";
  ...
  cout<<"End of StHIJetSaverWithCS::Finish"<<endl;
  StMemStat::PrintMem("End of Finish...");

  return kStOK;
}

void StHIJetSaverWithCS::WriteTree(TTree *sometree) {
  sometree->Write();
}
//
// OLD user code says: //  Called every event after Make(). 
//_____________________________________________________________________________
void StHIJetSaverWithCS::Clear(Option_t *opt) {
  // fJets->Clear();
}
//
//  Function: This method is called every event.
//_____________________________________________________________________________
Int_t StHIJetSaverWithCS::Make() {
  ...
  mHIRecoJetsCS = static_cast<StHIRecoJets*>(GetMaker("HIRecoCS"));
  if(!mHIRecoJetsCS) {
    LOG_WARN << Form(" No %s! Skip! ", "HIReco") << endm;
    return kStWarn;
  }

  fJetsArrRecoCS = mHIRecoJetsCS->GetJets();
  rhovalueCS = mHIRecoJetsCS->GetArrayofRhoValueCS();
  sigvalueCS = mHIRecoJetsCS->GetArrayofSigmaValueCS();

  if (fPrintLevel) 
  cout << "Count again  = "  << numberofmcevents << "\t" << fJetsArrReco.size() << "\t" << fJetsArrRecoCS.size() << endl;
...
void StHIJetSaverWithCS::DeclareTree() {
...
void StHIJetSaverWithCS::DeclareHistograms(){
  hDiffJetPt = new TH1F("hDiffJetPt", "hDiffJetPt", 100, -50, 50);
}

void StHIJetSaverWithCS::BookTree()
{
  ...
  jettree->Branch("RecoJetEtaCS", &fRecoJetTreeCS.jeteta, "jeteta/F");
  jettree->Branch("RecoJetPhiCS", &fRecoJetTreeCS.jetphi, "jetphi/F");
  jettree->Branch("RecoJetAreaCS", &fRecoJetTreeCS.jetarea, "jetarea/F");
  jettree->Branch("RecoJetECS", &fRecoJetTreeCS.jetenergy, "jetenergy/F");
  jettree->Branch("RecoJetRhoValCS", &fRecoJetTreeCS.fRhoValforjet, "fRhoValforjet/F");
  jettree->Branch("RecoJetSigValCS", &fRecoJetTreeCS.fSigValforjet, "fRhoValforjet/F");
...
void StHIJetSaverWithCS::SaveJets(int iteration){
  ...
  recoJetsCS = fJetsArrRecoCS[iteration];
  fRhoValCS = rhovalueCS[iteration];
  fSigValCS = sigvalueCS[iteration];
  ...
  if (fPrintLevel) cout << "Jets = " << "\t" << iteration << "\t" << mcnjets << "\t" << reconjets << "\t" << reconjetsfrompythia << "\t" << reconjetsCS << endl;
  ...
  if (recojetidCS != -99){
    StJet *rjet = static_cast<StJet*>(recoJetsCS->At(recojetidCS));
    if(!rjet) return;

    fRecoJetTreeCS.jetpt = rjet->Pt();
    fRecoJetTreeCS.jeteta = rjet->Eta();
    fRecoJetTreeCS.jetphi = rjet->Phi();
    fRecoJetTreeCS.jetarea = rjet->Area();
    fRecoJetTreeCS.jetenergy = rjet->E();
    fRecoJetTreeCS.numberofconstituents = rjet->GetNumberOfTracks();
    fRecoJetTreeCS.fRhoValforjet = fRhoValCS;
    fRecoJetTreeCS.fSigValforjet = fSigValCS;
  }
...
Double_t StHIJetSaverWithCS::standardPhi(Double_t phi){

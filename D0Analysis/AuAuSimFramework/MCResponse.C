...
void MCResponse(const Char_t *inputFile="Run12.list", const Char_t *outputFile="MCResponse.root", Int_t nEv = 1, const Char_t *fOutJobappend="")
{
        ...
        jets.ReplaceAll(".root", "_jets.root");
...
//        Int_t nEvents = 1000000;
      if(nEv > 100) nEvents = 100000000;
        ...
        if((RunYear == mRun14) && doTEST) {
            // inputFile = "/star/u/droy1/Y2019/STAR/SampleFileList_D0NoDecay.list";
            // inputFile = "Run14MC_NewList.list";
            // inputFile = "./CORITestFiles.list";
            // inputFile = "/gpfs01/star/pwg/droy1/SimulationJetsFrameWork/Mar24FileList/TestFileList.list";
          // inputFile = "/gpfs01/star/pwg/droy1/STAR-Workspace/D0Analysis/AuAuSimFramework/TestFile.list";
          inputFile = "/gpfs01/star/pwg/droy1/STAR-Workspace/D0Analysis/AuAuSimFramework/Test3.list";
        }
	...
	//StPicoDstMaker *picoMaker = new StPicoDstMaker(0,inputFile,"picoDst");
        ...
        double smearfactor[3] = {5.9, 4.6, 2.3};
        TString binname[3] = {"Central", "MidCentral", "Peripheral"};

        StTagD0MCEvents       *D0MCTaggerSmear[3];
        StMCD0JetMaker        *jetTaskMCSmear[3];
        StSimD0EventsJetMaker *jetTaskRecoSmear[3];
        StSimJetSaver_Test         *SimJetSaverSmear[3];

        for (int centbin = 0; centbin < 3; centbin++){
          D0MCTaggerSmear[centbin] = new StTagD0MCEvents(Form("D0MCTaggerSmear_%s", binname[centbin].Data()) , picoMaker, outputFile);
          D0MCTaggerSmear[centbin]->SetUsePrimaryTracks(usePrimaryTracks);       // use primary tracks
          D0MCTaggerSmear[centbin]->SetTurnOnCentSelection(doCentSelection);    // run analysis for specific centrality: BOOLEAN
          D0MCTaggerSmear[centbin]->SetCentralityBinCut(CentralitySelection);   // specific centrality range to run: if above is FALSE, this doesn't matter
          D0MCTaggerSmear[centbin]->SetdoppAnalysis(dopp);
          D0MCTaggerSmear[centbin]->FixPoorlySmearedTracks();
          D0MCTaggerSmear[centbin]->SetAnalysisCentralityBin(centbin);                // 0 - 10 [centbin]-> 1,
          D0MCTaggerSmear[centbin]->SetDebugLevel(0);
          D0MCTaggerSmear[centbin]->SetPrintLevel(0);
          cout << D0MCTaggerSmear[centbin]->GetName() << endl;

          jetTaskMCSmear[centbin] = new StMCD0JetMaker(Form("JetMakerMCSmear_%s", binname[centbin].Data()), fJetConstituentCut, D0MCTaggerSmear[centbin]->GetName(), kTRUE, outputFile);
          jetTaskMCSmear[centbin]->SetJetType(fJetType);          // jetType
          jetTaskMCSmear[centbin]->SetJetAlgo(antikt_algorithm);  // jetAlgo
          jetTaskMCSmear[centbin]->SetRecombScheme(E_scheme); // recombination scheme
          jetTaskMCSmear[centbin]->SetRadius(fJetRadius);         // jet radius
          jetTaskMCSmear[centbin]->SetJetsName("Jets");
          jetTaskMCSmear[centbin]->SetMinJetPt(1.0);             // 15.0 signal jets
          jetTaskMCSmear[centbin]->SetMaxJetTrackPt(30.0);        // max track constituent
          jetTaskMCSmear[centbin]->SetMinJetTowerE(fJetConstituentCut);  // 2.0 correlations
          jetTaskMCSmear[centbin]->SetHadronicCorrFrac(1.0);      // fractional hadronic correction
          jetTaskMCSmear[centbin]->SetJetHadCorrType(StJetFrameworkPicoBase::kAllMatchedTracks); // set for default options:  kLastMatchedTrack, kHighestEMatchedTrack, kAllMatchedTracks
          jetTaskMCSmear[centbin]->SetGhostArea(0.005);           // ghost area
          jetTaskMCSmear[centbin]->SetMinJetArea(0.0);            // minimum jet area
          jetTaskMCSmear[centbin]->SetJetEtaRange(-0.6, 0.6); // fiducial eta acceptance
          jetTaskMCSmear[centbin]->SetJetPhiRange(0,2.0*pi);      // phi acceptance
          jetTaskMCSmear[centbin]->SetUsePrimaryTracks(usePrimaryTracks);
          jetTaskMCSmear[centbin]->SetRunFlag(RunFlag);           // run flag      
          jetTaskMCSmear[centbin]->SetdoppAnalysis(dopp);         // pp switch
          jetTaskMCSmear[centbin]->SetEventZVtxRange(ZVtxMin, ZVtxMax);     // can be tighter for Run16 (-20,20)
          jetTaskMCSmear[centbin]->SetTurnOnCentSelection(doCentSelection);
          jetTaskMCSmear[centbin]->SetdoConstituentSubtr(doConstituentSubtraction); 
          jetTaskMCSmear[centbin]->SetCentralityBinCut(CentralitySelection);
          jetTaskMCSmear[centbin]->SetEmcTriggerEventType(EmcTriggerEventType);  // kIsHT1 or kIsHT2 or kIsHT3
          jetTaskMCSmear[centbin]->SetTriggerToUse(TriggerToUse);
          jetTaskMCSmear[centbin]->SetRejectBadRuns(RejectBadRuns);    // switch to load and than omit bad runs
          jetTaskMCSmear[centbin]->SetD0Analysis(d0D0Analysis);
          jetTaskMCSmear[centbin]->SetPrintLevel(0);
          jetTaskMCSmear[centbin]->SetD0Kind(0);
          cout << jetTaskMCSmear[centbin]->GetName() << endl;

          jetTaskRecoSmear[centbin] = new StSimD0EventsJetMaker(Form("JetMakerRecoSmear_%s", binname[centbin].Data()), fJetConstituentCut, D0MCTaggerSmear[centbin]->GetName(), jetTaskMCSmear[centbin]->GetName(), kTRUE, outputFile);
          jetTaskRecoSmear[centbin]->SetJetType(fJetType);          // jetType
          jetTaskRecoSmear[centbin]->SetJetAlgo(antikt_algorithm);  // jetAlgo
          jetTaskRecoSmear[centbin]->SetRecombScheme(E_scheme); // recombination scheme
          jetTaskRecoSmear[centbin]->SetRadius(fJetRadius);         // jet radius
          jetTaskRecoSmear[centbin]->SetJetsName("Jets");
          jetTaskRecoSmear[centbin]->SetMinJetPt(0.0);             // 15.0 signal jets
          jetTaskRecoSmear[centbin]->SetMaxJetTrackPt(30.0);        // max track constituent
          jetTaskRecoSmear[centbin]->SetMinJetTowerE(fJetConstituentCut);  // 2.0 correlations
          jetTaskRecoSmear[centbin]->SetHadronicCorrFrac(1.0);      // fractional hadronic correction
          jetTaskRecoSmear[centbin]->SetJetHadCorrType(StJetFrameworkPicoBase::kAllMatchedTracks); // set for default options:  kLastMatchedTrack, kHighestEMatchedTrack, kAllMatchedTracks
          jetTaskRecoSmear[centbin]->SetGhostArea(0.005);           // ghost area
          jetTaskRecoSmear[centbin]->SetMinJetArea(0.0);            // minimum jet area
          jetTaskRecoSmear[centbin]->SetJetEtaRange(-1.0, 1.0); // fiducial eta acceptance
          jetTaskRecoSmear[centbin]->SetJetPhiRange(0,2.0*pi);      // phi acceptance
          jetTaskRecoSmear[centbin]->SetUsePrimaryTracks(usePrimaryTracks);
          jetTaskRecoSmear[centbin]->SetRunFlag(RunFlag);           // run flag      
          jetTaskRecoSmear[centbin]->SetdoppAnalysis(dopp);         // pp switch
          jetTaskRecoSmear[centbin]->SetEventZVtxRange(ZVtxMin, ZVtxMax);     // can be tighter for Run16 (-20,20)
          jetTaskRecoSmear[centbin]->SetTurnOnCentSelection(doCentSelection);
          jetTaskRecoSmear[centbin]->SetdoConstituentSubtr(doConstituentSubtraction); 
          jetTaskRecoSmear[centbin]->SetCentralityBinCut(CentralitySelection);
          jetTaskRecoSmear[centbin]->SetEmcTriggerEventType(EmcTriggerEventType);  // kIsHT1 or kIsHT2 or kIsHT3
          jetTaskRecoSmear[centbin]->SetTriggerToUse(TriggerToUse);
          jetTaskRecoSmear[centbin]->SetRejectBadRuns(RejectBadRuns);    // switch to load and than omit bad runs
          jetTaskRecoSmear[centbin]->SetD0Analysis(d0D0Analysis);
          jetTaskRecoSmear[centbin]->SetMCAnalysisWithoutCent();
          jetTaskRecoSmear[centbin]->SetDropTracksForEfficiency(kTRUE);
          jetTaskRecoSmear[centbin]->SetPrintLevel(0);
          jetTaskRecoSmear[centbin]->SetD0Kind(0);
          cout << jetTaskRecoSmear[centbin]->GetName() << endl;

          SimJetSaverSmear[centbin] = new StSimJetSaver_Test(Form("SimJetSaverSmear_%s", binname[centbin].Data()), picoMaker, jets.Data(), jetTaskMCSmear[centbin]->GetName(), jetTaskRecoSmear[centbin]->GetName(), D0MCTaggerSmear[centbin]->GetName(), "");
          SimJetSaverSmear[centbin]->SetSmearFactorForJetPt(smearfactor[centbin]);
          cout << SimJetSaverSmear[centbin]->GetName() << endl;
        }

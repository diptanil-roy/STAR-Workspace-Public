...
void MCResponseMaker(const Char_t *inputFile="Run12.list", const Char_t *outputFile="Test_NoHIOverlay.root", Int_t nEv = 1, const Char_t *fOutJobappend="")
{
//        Int_t nEvents = 10000;
        Int_t nEvents = 3000;
//        Int_t nEvents = 1000000;
      if(nEv > 100) nEvents = 100000000;
        ...
        if((RunYear == mRun14) && doTEST) {
           ...
           inputFile = "/gpfs01/star/pwg/droy1/SimulationJetsFrameWork/MissedFilesList_Mar20.list";
        }
        ...
        double fJetRadius = 0.2;  // 0.4, 0.3, 0.2
	...
	//StPicoDstMaker *picoMaker = new StPicoDstMaker(0,inputFile,"picoDst");
        ...
        for (int centbin = 0; centbin < 3; centbin++){
          ...
          jetTaskMCSmear[centbin]->SetRadius(radius[centbin]);         // jet radius
          jetTaskMCSmear[centbin]->SetJetsName("Jets");
          jetTaskMCSmear[centbin]->SetMinJetPt(0.0);             // 15.0 signal jets
          ...
          jetTaskMCSmear[centbin]->SetJetEtaRange(-1.0, 1.0); // fiducial eta acceptance
          ...
          jetTaskRecoSmear[centbin]->SetRadius(radius[centbin]);         // jet radius
          ...
          SimJetSaverSmear[centbin] = new StSimJetSaver(Form("SimJetSaverSmear_%s", binname[centbin].Data()), picoMaker, outputFile, jetTaskMCSmear[centbin]->GetName(), jetTaskRecoSmear[centbin]->GetName(), D0MCTaggerSmear[centbin]->GetName(), "");

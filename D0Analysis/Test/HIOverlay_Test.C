...
void HIOverlay_Test(const Char_t *inputFile="Run12.list", const Char_t *outputFile="ResponseMatrices", Int_t nEv = 1, const Char_t *fOutJobappend="")
{
//        Int_t nEvents = 10000;
        Int_t nEvents = 10;
//        Int_t nEvents = 1000000;
	...
	//StPicoDstMaker *picoMaker = new StPicoDstMaker(0,inputFile,"picoDst");
        ...
        const Char_t *mcfilename;
        // if (doTEST) mcfilename="/gpfs01/star/pwg/droy1/STAR-Workspace/D0Analysis/AuAuSimFramework/MCTestFileList.list";
        // else mcfilename = "/gpfs01/star/pwg/droy1/STAR-Workspace/D0Analysis/AuAuSimFramework/SimulationFileList_Mar24.list";

        // mcfilename = "/gpfs01/star/pwg/droy1/STAR-Workspace/D0Analysis/AuAuSimFramework/SimulationFileList_pthat_3_inf.list";
        mcfilename = "/gpfs01/star/pwg/droy1/STAR-Workspace/D0Analysis/AuAuSimFramework/SimulationFileList_pthat_0_3.list";

        StHIOverlay *HIOverlayMaker = new StHIOverlay("HIOverlay", picoMaker, out.Data(), mcfilename);
        HIOverlayMaker->SetNumberOfEventsToOverLay(10);
        cout << HIOverlayMaker->GetName() << endl;

        StHIMCJets *HIMCJetMaker = new StHIMCJets("HIMC", fJetConstituentCut, kFALSE, out.Data());
        HIMCJetMaker->SetJetType(fJetType);          // jetType
        HIMCJetMaker->SetJetAlgo(antikt_algorithm);  // jetAlgo
        HIMCJetMaker->SetRecombScheme(E_scheme); // recombination scheme
        HIMCJetMaker->SetRadius(fJetRadius);         // jet radius
        HIMCJetMaker->SetJetsName("Jets");
        HIMCJetMaker->SetMinJetPt(0.0);             // 15.0 signal jets
        HIMCJetMaker->SetMaxJetTrackPt(30.0);        // max track constituent
        HIMCJetMaker->SetMinJetTowerE(fJetConstituentCut);  // 2.0 correlations
        HIMCJetMaker->SetHadronicCorrFrac(1.0);      // fractional hadronic correction
        HIMCJetMaker->SetJetHadCorrType(StJetFrameworkPicoBase::kAllMatchedTracks); // set for default options:  kLastMatchedTrack, kHighestEMatchedTrack, kAllMatchedTracks
        HIMCJetMaker->SetGhostArea(0.005);           // ghost area
        HIMCJetMaker->SetMinJetArea(0.0);            // minimum jet area
        HIMCJetMaker->SetJetEtaRange(-1.0, 1.0); // fiducial eta acceptance
        HIMCJetMaker->SetJetPhiRange(0,2.0*pi);      // phi acceptance
        HIMCJetMaker->SetUsePrimaryTracks(usePrimaryTracks);
        HIMCJetMaker->SetRunFlag(RunFlag);           // run flag      
        HIMCJetMaker->SetdoppAnalysis(dopp);         // pp switch
        HIMCJetMaker->SetEventZVtxRange(ZVtxMin, ZVtxMax);     // can be tighter for Run16 (-20,20)
        HIMCJetMaker->SetTurnOnCentSelection(doCentSelection);
        HIMCJetMaker->SetdoConstituentSubtr(doConstituentSubtraction); 
        HIMCJetMaker->SetCentralityBinCut(CentralitySelection);
        HIMCJetMaker->SetEmcTriggerEventType(EmcTriggerEventType);  // kIsHT1 or kIsHT2 or kIsHT3
        HIMCJetMaker->SetTriggerToUse(TriggerToUse);
        HIMCJetMaker->SetRejectBadRuns(RejectBadRuns);    // switch to load and than omit bad runs
        cout << HIMCJetMaker->GetName() << endl;
        ...
        StHIJetSaver *HIJetSaverMaker = new StHIJetSaver("HIJetSaver", picoMaker, jets.Data());

...
const Int_t StMCD0EventsJetMaker::fgkConstIndexShift = 100000;

ClassImp(StMCD0EventsJetMaker)

//________________________________________________________________________
StMCD0EventsJetMaker::StMCD0EventsJetMaker() : 
  StMaker(),
  fD0Analysis(kFALSE),
...
//  mVertex(0x0),
  ...
  fjw("StMCD0EventsJetMaker", "StMCD0EventsJetMaker"),
  fRadius(0.4),
  fMinJetArea(0.001),
  fMinJetPt(1.0),
...
//________________________________________________________________________
StMCD0EventsJetMaker::StMCD0EventsJetMaker(const char *name, double mintrackPt = 0.20, bool doHistos = kFALSE, const char* outName = "") : 
  StMaker(name),
  fD0Analysis(kFALSE),
...
//
//________________________________________________________________________
Int_t StMCD0EventsJetMaker::Init() {
  DeclareHistograms();
...
Int_t StMCD0EventsJetMaker::Finish() {
/*
  //  Summarize the run.
...
//________________________________________________________________________
void StMCD0EventsJetMaker::DeclareHistograms() {
  // constants
...
// Write histograms
//________________________________________________________________________
void StMCD0EventsJetMaker::WriteHistograms() {
...
//________________________________________________________________________
void StMCD0EventsJetMaker::WriteHadCorrQAHistograms() {
...
void StMCD0EventsJetMaker::Clear(Option_t *opt) {
  fJets->Clear();
  fJetsBGsub->Clear();
}
//
...
int StMCD0EventsJetMaker::Make()
{
  ...
  if (fD0Analysis) {
    mD0MCTagger = static_cast<StTagD0MCEvents*>(GetMaker("D0MCTagger"));
    if(!mD0MCTagger) {
    ...
    d0TrackIndices = mD0MCTagger->GetD0MCIndices();
    ...
    for (id0candidate = 0; id0candidate < d0TrackIndices.size(); id0candidate++){
      ...
      cout << "Track Information : " << iTrack1 << "\t" << iTrack2 << endl;
...
//________________________________________________________________________
void StMCD0EventsJetMaker::FindJets(TObjArray *tracks, TObjArray *towers, Int_t algo, Double_t radius)
{
...
//________________________________________________________________________
void StMCD0EventsJetMaker::FindJets()
{
...
Bool_t StMCD0EventsJetMaker::AcceptJetMCTrack(StPicoMcTrack *trk){
...
void StMCD0EventsJetMaker::FindJetsWithD0s(Int_t track1, Int_t track2)
{
  // clear out existing wrapper object
  fjw.Clear();
  fFull_Event.clear();
  ...
  if((fJetType == kFullJet) || (fJetType == kChargedJet)) {
    ...
    cout << "Number of Input Tracks = " << numberofinputtracks << endl;
 ...
 */
void StMCD0EventsJetMaker::FillJetBranch()
{
  ...
  for(UInt_t ijet = 0, jetCount = 0; ijet < jets_incl.size(); ++ijet) {
    ...
    cout << "BEFORE ====== "<< Form("Added jet n. %d, pt = %f, area = %f, constituents = %d, tracks = %d, towers = %d", jetCount, jet->Pt(), jet->Area(), jet->GetNumberOfConstituents(), jet->GetNumberOfTracks(), jet->GetNumberOfTowers()) << endl;
    ...
    cout << Form("Added jet n. %d, pt = %f, area = %f, constituents = %d, tracks = %d, towers = %d", jetCount, jet->Pt(), jet->Area(), jet->GetNumberOfConstituents(), jet->GetNumberOfTracks(), jet->GetNumberOfTowers()) << endl;
...
void StMCD0EventsJetMaker::FillJetConstituents(StJet *jet, std::vector<fastjet::PseudoJet>& constituents,
    std::vector<fastjet::PseudoJet>& constituents_unsub, Int_t flag, TString particlesSubName)
{
  ...
  for(UInt_t ic = 0; ic < constituents.size(); ++ic) {
    ...
    if(int(uid) >= 0 && int(uid) < 99999) {
      cout << "UID = " << uid << "\t" << nt << endl;
      // cout << "Trouble Starts Here." << endl;
      jet->AddTrackAt(int(uid), int(nt));
      // cout << "Trouble Ends Here." << endl;
      ...
      // if(!AcceptJetMCTrack(trk, Bfield, mVertex)) { continue; }
 ...
 */
Bool_t StMCD0EventsJetMaker::GetSortedArray(Int_t indexes[], std::vector<fastjet::PseudoJet> array) const
{
 ...
 */
Bool_t StMCD0EventsJetMaker::IsLocked() const
{
  if (fLocked) {
...
//________________________________________________________________________
Bool_t StMCD0EventsJetMaker::AcceptTrack(StPicoTrack *trk, Float_t B, TVector3 Vert) {
...
//________________________________________________________________________
Bool_t StMCD0EventsJetMaker::AcceptJetTrack(StPicoTrack *trk, Float_t B, TVector3 Vert) {
  // constants: assume neutral pion mass
  ///double pi0mass = Pico::mMass[0]; // GeV
  double pi = 1.0*TMath::Pi();
...
// Tower Quality Cuts
//________________________________________________________________________
Bool_t StMCD0EventsJetMaker::AcceptJetTower(StPicoBTowHit *tower, Int_t towerID) {
  // constants:
  double pi = 1.0*TMath::Pi();
...
//________________________________________________________________________
Int_t StMCD0EventsJetMaker::GetCentBin(Int_t cent, Int_t nBin) const
{
  Int_t centbin = -1;
...
Bool_t StMCD0EventsJetMaker::GetMomentum(TVector3 &mom, const StPicoBTowHit* tower, Double_t mass, Int_t towerID, Double_t CorrectedEnergy) const {
...
void StMCD0EventsJetMaker::FillEmcTriggersArr() {
...
Double_t StMCD0EventsJetMaker::GetMaxTrackPt()
{
  // get # of tracks
...
//_________________________________________________________________________________________________
Double_t StMCD0EventsJetMaker::GetMaxTowerEt()
{
  // get # of towers
 ...
 */
void StMCD0EventsJetMaker::FillJetBGBranch()
{
   // E_scheme, pt_scheme, pt2_scheme, Et_scheme, Et2_scheme, BIpt_scheme, BIpt2_scheme, WTA_pt_scheme, WTA_modp_scheme
   fastjet::RecombinationScheme    recombScheme;
   if (fRecombScheme == 0)     recombScheme = fastjet::E_scheme;
...
// set sum weights
//________________________________________________________________________
void StMCD0EventsJetMaker::SetSumw2() {
...
void StMCD0EventsJetMaker::RunEventQA() {

...
ClassImp(StTagDMesonEvents)

//________________________________________________________________________
StTagDMesonEvents::StTagDMesonEvents(const char* name, StPicoDstMaker *picoMaker, const char* outName = "") : StJetFrameworkPicoBase(name) //StMaker(name),
{ 
  fRunNumber = 0;
  mPicoDstMaker = 0x0;
  mPicoDst = 0x0;
  ...
  fDebugLevel = 0;
  fRunFlag = 0;       // see StTagDMesonEvents::fRunFlagEnum
  doppAnalysis = kFALSE;
...
//
//________________________________________________________________________
StTagDMesonEvents::~StTagDMesonEvents()
{ 
  ...
  if (invmassdiffDStarvpt) delete invmassdiffDStarvpt;
  ...
  if (hNumberOfDStar) delete hNumberOfDStar;
  if (hNumberOfDStarBgLS) delete hNumberOfDStarBgLS;
...
//________________________________________________________________________
Int_t StTagDMesonEvents::Finish() { 

  cout << "StTagDMesonEvents::Finish()\n";
  ...
  cout<<"End of StTagDMesonEvents::Finish"<<endl;
...
//________________________________________________________________________
void StTagDMesonEvents::DeclareHistograms() {
  ...
  hNumberOfDStar = new TH1F("hNumberOfDStar", "hNumberOfDStar", 21, -0.5, 20.5);
  hNumberOfDStarBgLS = new TH1F("hNumberOfDStarBgLS", "hNumberOfDStarBgLS", 21, -0.5, 20.5);

}

void StTagDMesonEvents::DeclareD0Tree() {
...
void StTagDMesonEvents::DeclareDStarTree() {
...
void StTagDMesonEvents::BookD0Tree(){
  ...
  d0sigtree->Branch("VzMinusVzVpd", &fD0SigTree.vzmvpd, "vzmvpd/F");
  ...
  d0bgtree->Branch("VzMinusVzVpd", &fD0BgTree.vzmvpd, "vzmvpd/F");
...
void StTagDMesonEvents::BookDStarTree(){
  ...
  dstarsigtree->Branch("VzMinusVzVpd", &fDStarSigTree.vzmvpd, "vzmvpd/F");
  ...
  dstarbgtree->Branch("VzMinusVzVpd", &fDStarBgTree.vzmvpd, "vzmvpd/F");
...
void StTagDMesonEvents::WriteD0Tree(){
...
void StTagDMesonEvents::WriteDStarTree(){
  dstarsigtree->Write();
  dstarbgtree->Write();
}
...
void StTagDMesonEvents::WriteHistograms() {
  ...
  hNumberOfDStar->Write();
  hNumberOfDStarBgLS->Write();
}
//
// OLD user code says: //  Called every event after Make(). 
//_____________________________________________________________________________
void StTagDMesonEvents::Clear(Option_t *opt) {
  // fJets->Clear();
  fd0TrackIndices.clear();
  fd0BgUSTrackIndices.clear();
  fd0BgLSTrackIndices.clear();
...
Int_t StTagDMesonEvents::Make() {
  ...
  if (fTagDStarEvents) GetDStarTracks();
...
void StTagDMesonEvents::GetD0Tracks(){
  ...
  for(unsigned short itrk1 = 0; itrk1 < ntracks; itrk1++){
    ...
    for(unsigned short itrk2 = itrk1 + 1; itrk2 < ntracks; itrk2++){
      ...
      if (pid1*pid2==-2) { invmass->Fill(mass); invmassvpt->Fill(mass, mResMom.Perp()); }
      if (pid1*pid2==2)  { invmassbg->Fill(mass); invmassbgvpt->Fill(mass, mResMom.Perp());}

      if ((mass < fInvMassULBg1) || (mass > fInvMassULBg2)) continue; // Only tracks between 1.7, 2.02

      int piontrack = (abs(pid1) == 1) ? itrk1 : itrk2; 
      int kaontrack = (abs(pid1) == 2) ? itrk1 : itrk2;
      ...
      if (pid1*pid2==-2) {
        ...
        fD0SigTree.vzmvpd = mVertex.z() - mPicoEvent->vzVpd();
      ...
      if (pid1*pid2==2) {
        ...
        fD0BgTree.vzmvpd = mVertex.z() - mPicoEvent->vzVpd();
  ...
  // cout << fd0TrackIndices.size() << "\t" << fd0BgUSTrackIndices.size() << "\t" << fd0BgLSTrackIndices.size() << endl;
...
void StTagDMesonEvents::GetDStarTracks(){
  ...
  for (int d0 = 0; d0 < fd0TrackIndices.size(); d0++){
    int d0trk1 = fd0TrackIndices[d0][0];
    int d0trk2 = fd0TrackIndices[d0][1];

    double mass = fd0TrackIndices[d0][2];

    if (mass < 1.83 || mass > 1.9) continue;

    StPicoTrack *trk1 = static_cast<StPicoTrack*>(mPicoDst->track(d0trk1));
    if(!trk1){ continue; }

    StPicoTrack *trk2 = static_cast<StPicoTrack*>(mPicoDst->track(d0trk2));
    if(!trk2){ continue; }
    ...
    for(unsigned short itrk3 = 0; itrk3 < ntracks; itrk3++){
      if (itrk3 == d0trk1 || itrk3 == d0trk2) continue;
      ...
      short chargeD0Pi = trk1->charge();

      if (abs(chargeD0Pi*charge3) != 1) continue;
      ...
      if (chargeD0Pi*charge3 == 1){ // Choosing Pions with the same charge as the Pion From D0. This is our signal
        ...
        invmassDStarvpt->Fill(massDStar, mDStarMom.Perp());

        fdstarTrackIndices.push_back({d0trk1, d0trk2, itrk3, mass, massDstarminusmassD0});
        ...
        fDStarSigTree.vzmvpd = mVertex.z() - mPicoEvent->vzVpd();
        ...
        fDStarSigTree.pionpt = mTrk1Mom.Perp();
        fDStarSigTree.pioneta = mTrk1Mom.PseudoRapidity();
        fDStarSigTree.pionphi = standardPhi(mTrk1Mom.Phi());
        fDStarSigTree.pioncharge = trk1->charge();
        fDStarSigTree.piondca = trk1->gDCA(mVertex).Mag();
        fDStarSigTree.nsigmapion = trk1->nSigmaPion();

        fDStarSigTree.kaonpt = mTrk2Mom.Perp();
        fDStarSigTree.kaoneta = mTrk2Mom.PseudoRapidity();
        fDStarSigTree.kaonphi = standardPhi(mTrk2Mom.Phi());
        fDStarSigTree.kaoncharge = trk2->charge();
        fDStarSigTree.kaondca = trk2->gDCA(mVertex).Mag();
        fDStarSigTree.nsigmakaon = trk2->nSigmaKaon();
        ...
        fdstarLSTrackIndices.push_back({d0trk1, d0trk2, itrk3, mass, massDstarminusmassD0});
        ...
        fDStarBgTree.vzmvpd = mVertex.z() - mPicoEvent->vzVpd();
        ...
        fDStarBgTree.pionpt = mTrk1Mom.Perp();
        fDStarBgTree.pioneta = mTrk1Mom.PseudoRapidity();
        fDStarBgTree.pionphi = standardPhi(mTrk1Mom.Phi());
        fDStarBgTree.pioncharge = trk1->charge();
        fDStarBgTree.piondca = trk1->gDCA(mVertex).Mag();
        fDStarBgTree.nsigmapion = trk1->nSigmaPion();

        fDStarBgTree.kaonpt = mTrk2Mom.Perp();
        fDStarBgTree.kaoneta = mTrk2Mom.PseudoRapidity();
        fDStarBgTree.kaonphi = standardPhi(mTrk2Mom.Phi());
        fDStarBgTree.kaoncharge = trk2->charge();
        fDStarBgTree.kaondca = trk2->gDCA(mVertex).Mag();
        fDStarBgTree.nsigmakaon = trk2->nSigmaKaon();
  ...
  hNumberOfDStar->Fill(fdstarTrackIndices.size());
  hNumberOfDStarBgLS->Fill(fdstarLSTrackIndices.size());
}

Bool_t StTagDMesonEvents::IsAnAcceptableTrack(StPicoTrack *trk, bool dohistograms = kFALSE){
...
Int_t StTagDMesonEvents::TopologicalCuts(StPicoTrack *trk1, StPicoTrack *trk2, bool dohistograms = kFALSE){
...
void StTagDMesonEvents::FillPidHistograms(StPicoTrack *trk){
...
void StTagDMesonEvents::IsWhatParticle(StPicoTrack *trk, int &pid, double &m, double &e){ // NEW PID APPROACH
...
Int_t StTagDMesonEvents::IsWhatParticle(StPicoTrack *trk){ // Just to get the PID out
  int pid;
  double m; 
  double e;
...
void StTagDMesonEvents::InvariantMass(StPicoTrack *trk1, StPicoTrack *trk2, int &particle, double &invmass, TVector3 &momentum){
...
Double_t StTagDMesonEvents::InvariantMass(StPicoTrack *trk1, StPicoTrack *trk2){
  int particle; 
  double invmass; 
  TVector3 momentum;
...
bool StTagDMesonEvents::IsSoftPion(StPicoTrack *trk){
...
Int_t StTagDMesonEvents::GetFourCentBin(Double_t scaledCent) const{
  int centbin = -99;

...
Int_t StCentMaker::Init() {
  ...
  switch(fRunFlag) {
    ...
    case StJetFrameworkPicoBase::Run14_AuAu200_MB : // Run14: 200 GeV AuAu - HFT data set (MB trigger)
        switch(fCentralityDef) {
          case StJetFrameworkPicoBase::kgrefmult :
              grefmultCorr = CentralityMaker::instance()->getgRefMultCorr();
              break;
...
Int_t StCentMaker::Make() {
  ...
  double fBBCCoincidenceRate;
  double fZDCCoincidenceRate;

  if (fHIOverlay){
    fBBCCoincidenceRate = eventdatapicodst[0][1];
    fZDCCoincidenceRate = eventdatapicodst[0][2];
  }
  else{
    fBBCCoincidenceRate = mPicoEvent->BBCx();
    fZDCCoincidenceRate = mPicoEvent->ZDCx();
  }
  ...
  Int_t centbin;
  if (fHIOverlay){
    kgrefMult = eventdatapicodst[0][3];
    krefMult = eventdatapicodst[0][4];
  }
  else{
  ...
  if(!doppAnalysis) {
    ...
    if (fHIOverlay){
      grefmultCorr->init(eventdatapicodst[0][0]);
    }
    else grefmultCorr->init(fRunNumber);
...
//    if(grefmultCorr->isRefMultOk()) cout << "RefMult Ok" << endl;
    ...
    kWeight = grefmultCorr->getWeight();
  ...
  // cout << kgrefmult << "\t" << krefMult << "\t" <<  kcent16 << endl;
...
float StCentMaker::Weight(){
  return grefmultCorr->getWeight();
}

...
KStarFilter::KStarFilter( const char* name ) : StarFilterMaker(name)
{

}
//______________________________________________________________________________________________
void KStarFilter::AddTrigger( int id, double mnpt, double mxpt, double mneta, double mxeta, int idm )
{
  mTriggers.push_back( Trigger_t( id, mnpt, mxpt, mneta, mxeta, idm ) );
}
//______________________________________________________________________________________________
int KStarFilter::Init() {
...
//______________________________________________________________________________________________
int KStarFilter::Filter( StarGenEvent *_event ) 
{
  ...
  for ( int ipart=1 /*skip header*/; 
	...
	ipart++ )
    {
      ...
      if ( TMath::Abs(part->GetId()) != 313 ) continue;
      if ( pT < 1.00 || TMath::Abs(eta) > 1 ) continue;

      // if ( pT < 1.00 || TMath::Abs(Y) > 1 ) continue;

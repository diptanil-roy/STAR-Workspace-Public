...
JetFilter::JetFilter( const char* name, double zmin, double zmax ) : StarFilterMaker(name)
{
...
//______________________________________________________________________________________________
void JetFilter::AddTrigger( int id, double mnpt, double mxpt, double mneta, double mxeta, int idm )
{
...
//______________________________________________________________________________________________
int JetFilter::Filter( StarGenEvent *_event ) 
{
    ...
    ipart++ )
      {
        ...
        LOG_INFO << Form("Input (gid, id, pT, eta, phi) = %i \t %i \t %f \t %f \t %f", geantid, part->GetId(), pT, eta, phi) << endl;
                   
      }

      for (unsigned j = 0; j < sortedconstituents.size(); j++){
        LOG_INFO << Form("Constituent (id, pT, eta, phi) = %i \t %f \t %f \t %f", sortedconstituents[j].user_info<MyParticleId>().pid(), sortedconstituents[j].perp(), sortedconstituents[j].eta(), sortedconstituents[j].phi()) << endl;
      }

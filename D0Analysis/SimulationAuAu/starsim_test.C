...
void starsim_test( Int_t nevents=5000, Int_t runnumber=15117062, TString runfile_="15117062.dat", int sequence=0, int dummy = 0 )
{ 
...
//   }
  ...
  // pythia8->Set("PDF:pSet = 2");
  pythia8 -> Set(Form("PhaseSpace:ptHatMin=%f", ckin3 ));
  pythia8 -> Set(Form("PhaseSpace:ptHatMax=%f", ckin4 ));

  // pythia8->Set("BeamRemnants:primordialKThard = 1.0");
  // pythia8->readString("BeamRemnants:primordialKTsoft = 1.0");

  // pythia8->Set("SigmaProcess:renormScale2=3");
  // pythia8->Set("SigmaProcess:factorScale2=3");
  // pythia8->readString("SigmaProcess:renormMultFac = 1.0");
  // pythia8->readString("SigmaProcess:factorMultFac = 1.0");
  ...
  for (int count = 0; count < nevents; count++){
    ...
    command( Form( "rung %i %i", runnumber, count ) );

    trig( count, runnumber, 0.1 );

  }

  // cout << "Cross Section Information = " << pythia8->Stats()->luminosity() << "\t" << pythia8->Stats()->filterAcceptance() << endl;
  pythia8->Stats();
  // int count = 0;

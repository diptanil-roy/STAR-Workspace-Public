...
void StPidInfoTest::IsWhatParticleFlipped(StPicoTrack *trk, int &pidflipped, double &mflipped, double &eflipped){
  int pid;
  double m, e;
...
void StPidInfoTest::InvariantMassFlipped(StPicoTrack *trk1, StPicoTrack *trk2, int &particle, double &invmass, TVector3 &momentum){
...
void StPidInfoTest::InvariantMassWithCuts(StPicoTrack *trk1, StPicoTrack *trk2, StPicoTrackCovMatrix *cov1, StPicoTrackCovMatrix *cov2){
  ...
  //   if (pt1 + pt2 > ptcuts[0]){
  //     for (int i = 0; i < 3; i++){
  //       if (distanced0fromPV < Cuts[i]) invmasscuts[0 + i]->Fill(mass);
  //       if (dpik < Cuts[i]) invmasscuts[3 + i]->Fill(mass);
  //       if ((distanced0fromPV < Cuts[i]) && (dpik < Cuts[i])) invmasscuts[6 + i]->Fill(mass);

  //       if (distanced0fromPV < Cuts[i]) invmass_flipped_cuts[0 + i]->Fill(massflipped);
  //       if (dpik < Cuts[i]) invmass_flipped_cuts[3 + i]->Fill(massflipped);
  //       if ((distanced0fromPV < Cuts[i]) && (dpik < Cuts[i])) invmass_flipped_cuts[6 + i]->Fill(massflipped);
  //     }
  //   }

  //   if (pt1 + pt2 > ptcuts[1]){
  //     for (int i = 0; i < 3; i++){
  //       if (distanced0fromPV < Cuts[i]) invmasscuts[9 + i]->Fill(mass);
  //       if (dpik < Cuts[i]) invmasscuts[12 + i]->Fill(mass);
  //       if ((distanced0fromPV < Cuts[i]) && (dpik < Cuts[i])) invmasscuts[15 + i]->Fill(mass);

  //       if (distanced0fromPV < Cuts[i]) invmass_flipped_cuts[9 + i]->Fill(massflipped);
  //       if (dpik < Cuts[i]) invmass_flipped_cuts[12 + i]->Fill(massflipped);
  //       if ((distanced0fromPV < Cuts[i]) && (dpik < Cuts[i])) invmass_flipped_cuts[15 + i]->Fill(massflipped);
  //     }
  //   }

  //   if (pt1 + pt2 > ptcuts[2]){
  //     for (int i = 0; i < 3; i++){
  //       if (distanced0fromPV < Cuts[i]) invmasscuts[18 + i]->Fill(mass);
  //       if (dpik < Cuts[i]) invmasscuts[21 + i]->Fill(mass);
  //       if ((distanced0fromPV < Cuts[i]) && (dpik < Cuts[i])) invmasscuts[24 + i]->Fill(mass);

  //       if (distanced0fromPV < Cuts[i]) invmass_flipped_cuts[18 + i]->Fill(massflipped);
  //       if (dpik < Cuts[i]) invmass_flipped_cuts[21 + i]->Fill(massflipped);
  //       if ((distanced0fromPV < Cuts[i]) && (dpik < Cuts[i])) invmass_flipped_cuts[24 + i]->Fill(massflipped);
  //     }
  //   }
  // }
  ...
  //   if (pt1 + pt2 > ptcuts[0]){
  //     for (int i = 0; i < 3; i++){
  //       if (distanced0fromPV < Cuts[i]) invmassbgcuts[0 + i]->Fill(mass);
  //       if (dpik < Cuts[i]) invmassbgcuts[3 + i]->Fill(mass);
  //       if ((distanced0fromPV < Cuts[i]) && (dpik < Cuts[i])) invmassbgcuts[6 + i]->Fill(mass);
  //     }
  //   }

  //   if (pt1 + pt2 > ptcuts[1]){
  //     for (int i = 0; i < 3; i++){
  //       if (distanced0fromPV < Cuts[i]) invmassbgcuts[9 + i]->Fill(mass);
  //       if (dpik < Cuts[i]) invmassbgcuts[12 + i]->Fill(mass);
  //       if ((distanced0fromPV < Cuts[i]) && (dpik < Cuts[i])) invmassbgcuts[15 + i]->Fill(mass);
  //     }
  //   }

  //   if (pt1 + pt2 > ptcuts[2]){
  //     for (int i = 0; i < 3; i++){
  //       if (distanced0fromPV < Cuts[i]) invmassbgcuts[18 + i]->Fill(mass);
  //       if (dpik < Cuts[i]) invmassbgcuts[21 + i]->Fill(mass);
  //       if ((distanced0fromPV < Cuts[i]) && (dpik < Cuts[i])) invmassbgcuts[24 + i]->Fill(mass);
  //     }
  //   }
  // }

  // cout << "Event Vertex = " << mPicoEvent->primaryVertex().X() << "\t" << mPicoEvent->primaryVertex().Y() << "\t" << mPicoEvent->primaryVertex().Z() << endl;
  // cout << "Part 1 = " << trk1->origin().X() << "\t" << trk1->origin().Y() << "\t" << trk1->origin().Z() << endl;
  // cout << "Part 2 = " << trk2->origin().X() << "\t" << trk2->origin().Y() << "\t" << trk2->origin().Z() << endl;
  // cout << "Mom = " << mom_cord.X() << "\t" << mom_cord.Y() << "\t" << mom_cord.Z() << endl;
  // cout << "Distances = " << dpik << "\t" << distanced0fromPV << endl;
  ...
  // TVector3 mom_cord_dv(mother.GetX(), mother.GetY(), mother.GetZ());

  // mother.TransportToProductionVertex();

  // TVector3 mom_cord_pv(mother.GetX(), mother.GetY(), mother.GetZ());

  // cout << "Event Vertex = " << mPicoEvent->primaryVertex().X() << "\t" << mPicoEvent->primaryVertex().Y() << "\t" << mPicoEvent->primaryVertex().Z() << endl;
  // cout << "Original = " << mom_cord.X() << "\t" << mom_cord.Y() << "\t" << mom_cord.Z() << endl;
  // cout << "Decay = " << mom_cord_dv.X() << "\t" << mom_cord_dv.Y()<< "\t" << mom_cord_dv.Z() << endl;
  // cout << "Production = " << mom_cord_pv.X() << "\t" << mom_cord_pv.Y() << "\t" << mom_cord_pv.Z() << endl;
  ...
  // cout << "Coordinates = " << params1[0] << "\t" << params1[1] << "\t" << params1[2] << endl;

  // cout << "Part 2 = " << trk2->origin().X() << "\t" << trk2->origin().Y() << "\t" << trk2->origin().Z() << endl;

  // cout << params2[0] << "\t" << params2[1] << "\t" << params2[2] << endl;
  ...
  // cout << "Decay Length = " << decaylength << "\t" << decaylengthsigma << endl;
  ...
  // // D0 cuts only
  // for (int i = 0; i < 6; i++){
  //   if (distanced0fromPV < D0PVCut[i]) invmasscuts[0 + i]->Fill(mass);
  // }

  // //KPi cuts only
  // for (int i = 0; i < 4; i++){
  //   if (dpik < KPiCut[i]) invmasscuts[6 + i]->Fill(mass);
  // }

  // //Mixed cuts
  // for (int i = 0; i < 5; i++){
  //   if ((distanced0fromPV < DoubleCut[i]) && (dpik < DoubleCut[i])) invmasscuts[10 + i]->Fill(mass);
  // }

#ifndef STD0EVENTSJETMAKER_H
#define STD0EVENTSJETMAKER_H
...
class StD0EventsJetMaker : public StMaker {
 public:
  ...
  StD0EventsJetMaker();
  StD0EventsJetMaker(const char *name, double mintrackPt, bool dohistos, const char* outName);
  virtual ~StD0EventsJetMaker();
 ...
 private:
  ...
  // D0 tagger objects
  StPIDQA         *mD0Tagger;               // D0 Tagger object
  StGenerateARandomTrack *mGenerateRandomParticle; //Random Object


  std::vector<std::vector<int> >  d0TrackIndices; 
...
//  Bool_t                 mTowerStatusArr[4800];
  ...
  StD0EventsJetMaker(const StD0EventsJetMaker&);            // not implemented
  StD0EventsJetMaker &operator=(const StD0EventsJetMaker&); // not implemented

  ClassDef(StD0EventsJetMaker, 4) // Jet producing task
};
#endif

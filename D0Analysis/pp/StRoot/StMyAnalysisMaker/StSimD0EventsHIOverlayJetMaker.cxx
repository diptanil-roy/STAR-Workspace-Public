...
const Int_t StSimD0EventsHIOverlayJetMaker::fgkConstIndexShift = 100000;

ClassImp(StSimD0EventsHIOverlayJetMaker)

//________________________________________________________________________
StSimD0EventsHIOverlayJetMaker::StSimD0EventsHIOverlayJetMaker() : 
  StMaker(),
  fD0Analysis(kFALSE),
  fDrop(kFALSE),
...
//  mVertex(0x0),
  ...
  fjw("StSimD0EventsHIOverlayJetMaker", "StSimD0EventsHIOverlayJetMaker"),
  fRadius(0.4),
  fMinJetArea(0.001),
  fMinJetPt(1.0),
...
//________________________________________________________________________
StSimD0EventsHIOverlayJetMaker::StSimD0EventsHIOverlayJetMaker(const char *name, double mintrackPt = 0.20, const char *d0taggername = "", bool doHistos = kFALSE, const char* outName = "") : 
  StMaker(name),
  fD0Analysis(kFALSE),
  fDrop(kFALSE),
...
//
//________________________________________________________________________
Int_t StSimD0EventsHIOverlayJetMaker::Init() {
  DeclareHistograms();
...
Int_t StSimD0EventsHIOverlayJetMaker::Finish() {
/*
  //  Summarize the run.
...
//________________________________________________________________________
void StSimD0EventsHIOverlayJetMaker::DeclareHistograms() {
  // constants
...
// Write histograms
//________________________________________________________________________
void StSimD0EventsHIOverlayJetMaker::WriteHistograms() {
...
//________________________________________________________________________
void StSimD0EventsHIOverlayJetMaker::WriteHadCorrQAHistograms() {
...
void StSimD0EventsHIOverlayJetMaker::Clear(Option_t *opt) {
  fJets->Clear();
  fJetsBGsub->Clear();
}
//
...
//________________________________________________________________________________
int StSimD0EventsHIOverlayJetMaker::Make()
{
  ...
  towerdatapicodst = mReadATree->TowerTreeToVector();
...
void StSimD0EventsHIOverlayJetMaker::FindRecoJetsWithD0s(Int_t D0)
{

  // cout << "Called Jets With D0s" << endl;
  ...
  if((fJetType == kFullJet) || (fJetType == kChargedJet)) {
    ...
    for(unsigned short iTracks = 0; iTracks < ntracks; iTracks++){
      if (iTracks==d0RecoTrackIndices[id0candidate][0] || iTracks==d0RecoTrackIndices[id0candidate][1]) continue;
      if (fDrop){
        for (int i = 0; i < droppedTrackIndices[D0].size(); i++){
          if (iTracks == droppedTrackIndices[D0][i]) continue;
        }
      }
    ...
    for(unsigned short iTracks = 0; iTracks < ntracks; iTracks++){
      ...
      // Dropping Tracks based on Efficiency Correction
      if (fDrop){
        for (int i = 0; i < droppedTrackIndices[D0].size(); i++){
          if (iTracks == droppedTrackIndices[D0][i]) continue;
        }
    ...
    */
    ...
    /* For now, ignore any KPi reconstructed by the chain. Everything happens at FastSIM level. The first if statement doesn't trigger at all! */
    if (d0RecoTrackIndices[id0candidate][0] < -999 && d0RecoTrackIndices[id0candidate][1] < -999){
      ...
      if (trk1 && trk2){
        if (AcceptJetTrack(trk1, Bfield, mVertex) && AcceptJetTrack(trk2, Bfield, mVertex)){
          ...
          if((pt > fMinJetTrackPt && pt < fMaxJetTrackPt) && (eta > fJetTrackEtaMin) && (eta < fJetTrackEtaMax) && (phi > fJetTrackPhiMin) && (phi < fJetTrackPhiMax)){
            ...
            // add track input vector to FastJet
            if(doCorrectTracksforEffBeforeJetReco) {  // kTRUE - corrects tracks before FastJet reconstruction
              fjw.AddInputVector(pxCorr, pyCorr, pzCorr, energyCorr, 99999); // efficiency corrected components
            } else {
              // THIS IS DEFAULT !
              // send track info to FJ wrapper
            ...
            int trackIndex1 = d0RecoTrackIndices[id0candidate][0];
            int trackIndex2 = d0RecoTrackIndices[id0candidate][1];

            int matchedTowerIndex1 = trk1->bemcTowerIndex(); // towerIndex = towerID - 1
            int matchedTowerIndex2 = trk2->bemcTowerIndex(); // towerIndex = towerID - 1

            mTowerMatchTrkIndex[ matchedTowerIndex1 ][ mTowerStatusArr[ matchedTowerIndex1 ]] = trackIndex1;
            mTowerStatusArr[ matchedTowerIndex1 ] = mTowerStatusArr[ matchedTowerIndex1 ] + 1;  // 1+ means match, 0 for no match 

            mTowerMatchTrkIndex[ matchedTowerIndex2 ][ mTowerStatusArr[ matchedTowerIndex2 ]] = trackIndex2;
    ...
    else{ // This is triggered if one of the reco track is not matched properly, or doesn't pass our cut!
...
//=================================================================================================================
  ...
  if(fJetType == kFullJet){
    int HITracks = trackdatapicodst.size();

    cout << "Track Data Size = " << trackdatapicodst.size() << endl;

    for(unsigned short iTracks = 0; iTracks < HITracks; iTracks++){

      TVector3 mTrkMom;
      mTrkMom.SetXYZ(trackdatapicodst[iTracks][0], trackdatapicodst[iTracks][1], trackdatapicodst[iTracks][2]);
      ...
      double pz = mTrkMom.z();
      double p = mTrkMom.Mag();
      double energy = trackdatapicodst[iTracks][3];
      // short charge = trackssmearedwithfastsim[iTracks][8];
      // int matchedTowerIndex = trackssmearedwithfastsim[iTracks][9]; // towerIndex = towerID - 1
      ...
      fjw.AddInputVector(px, py, pz, energy, trackdatapicodst[iTracks][4]); // includes E
      // THIS IS DEFAULT !
      fastjet::PseudoJet particle_Track(px, py, pz, energy);
      particle_Track.set_user_index(trackdatapicodst[iTracks][4]);
      fFull_Event.push_back(particle_Track);
    }

    int HITowers = towerdatapicodst.size();

    cout << "Tower Data Size = " << towerdatapicodst.size() << endl;

    for(unsigned short iTowers = 0; iTowers < HITowers; iTowers++){

      TVector3 mTrkMom;
      mTrkMom.SetXYZ(towerdatapicodst[iTowers][0], towerdatapicodst[iTowers][1], towerdatapicodst[iTowers][2]);
      ...
      double pz = mTrkMom.z();
      double p = mTrkMom.Mag();
      double energy = towerdatapicodst[iTowers][3];
      // short charge = trackssmearedwithfastsim[iTracks][8];
      // int matchedTowerIndex = trackssmearedwithfastsim[iTracks][9]; // towerIndex = towerID - 1
      ...
      fjw.AddInputVector(px, py, pz, energy, towerdatapicodst[iTowers][4]); // includes E
      // THIS IS DEFAULT !
      fastjet::PseudoJet particle_Track(px, py, pz, energy);
      particle_Track.set_user_index(towerdatapicodst[iTowers][4]);
      fFull_Event.push_back(particle_Track);
    }
 ...
 */
void StSimD0EventsHIOverlayJetMaker::FillJetBranch()
{
 ...
 */
void StSimD0EventsHIOverlayJetMaker::FillJetConstituents(StJet *jet, std::vector<fastjet::PseudoJet>& constituents,
...
// ============================================================================================================================
      ...
      TVector3 mTrkMom;
      mTrkMom.SetXYZ(trackdatapicodst[uid-10000][0], trackdatapicodst[uid-10000][1], trackdatapicodst[uid-10000][2]);
      ...
      double pz = mTrkMom.z();
      double p = mTrkMom.Mag();
      double energy = trackdatapicodst[uid-10000][3];
      ...
      // fill QA histos for jet towers
      fHistJetNTowervsID->Fill(towIndex);
      fHistJetNTowervsADC->Fill(towerdatapicodst[towIndex][5]);
      fHistJetNTowervsE->Fill(towerdatapicodst[towIndex][7]);
      // fHistJetNTowervsEtCent[cbin - 1]->Fill(towEt);
      fHistJetNTowervsEt->Fill(towerdatapicodst[towIndex][6]);
      fHistJetNTowervsPhi->Fill(towerPhi);
      fHistJetNTowervsEta->Fill(towerEta);
      if (fHistJetNTowervsPhivsEta->Fill(towerPhi, towerEta) == -1) printf("%d \n", __LINE__);
      if (fHistQATowIDvsEta->Fill(towerID, towerEta) == -1) printf("%d \n", __LINE__);
...
//________________________________________________________________________
void StSimD0EventsHIOverlayJetMaker::FindJets(TObjArray *tracks, TObjArray *towers, Int_t algo, Double_t radius)
{
...
//________________________________________________________________________
void StSimD0EventsHIOverlayJetMaker::FindJets()
{
  // clear out existing wrapper object
  fjw.Clear();
  fFull_Event.clear();
 ...
 */
Bool_t StSimD0EventsHIOverlayJetMaker::GetSortedArray(Int_t indexes[], std::vector<fastjet::PseudoJet> array) const
{
 ...
 */
Bool_t StSimD0EventsHIOverlayJetMaker::IsLocked() const
{
  if (fLocked) {
...
//________________________________________________________________________
Bool_t StSimD0EventsHIOverlayJetMaker::AcceptTrack(StPicoTrack *trk, Float_t B, TVector3 Vert) {
...
//________________________________________________________________________
Bool_t StSimD0EventsHIOverlayJetMaker::AcceptJetTrack(StPicoTrack *trk, Float_t B, TVector3 Vert) {
  // constants: assume neutral pion mass
  ///double pi0mass = Pico::mMass[0]; // GeV
  double pi = 1.0*TMath::Pi();
...
// Tower Quality Cuts
//________________________________________________________________________
Bool_t StSimD0EventsHIOverlayJetMaker::AcceptJetTower(StPicoBTowHit *tower, Int_t towerID) {
  // constants:
  double pi = 1.0*TMath::Pi();
...
//________________________________________________________________________
Int_t StSimD0EventsHIOverlayJetMaker::GetCentBin(Int_t cent, Int_t nBin) const
{
  Int_t centbin = -1;
...
Bool_t StSimD0EventsHIOverlayJetMaker::GetMomentum(TVector3 &mom, const StPicoBTowHit* tower, Double_t mass, Int_t towerID, Double_t CorrectedEnergy) const {
...
void StSimD0EventsHIOverlayJetMaker::FillEmcTriggersArr() {
...
Double_t StSimD0EventsHIOverlayJetMaker::GetMaxTrackPt()
{
  // get # of tracks
...
//_________________________________________________________________________________________________
Double_t StSimD0EventsHIOverlayJetMaker::GetMaxTowerEt()
{
  // get # of towers
 ...
 */
void StSimD0EventsHIOverlayJetMaker::FillJetBGBranch()
{
   // E_scheme, pt_scheme, pt2_scheme, Et_scheme, Et2_scheme, BIpt_scheme, BIpt2_scheme, WTA_pt_scheme, WTA_modp_scheme
   fastjet::RecombinationScheme    recombScheme;
   if (fRecombScheme == 0)     recombScheme = fastjet::E_scheme;
...
// set sum weights
//________________________________________________________________________
void StSimD0EventsHIOverlayJetMaker::SetSumw2() {
...
void StSimD0EventsHIOverlayJetMaker::RunEventQA() {
...
// StPicoTrack * StSimD0EventsHIOverlayJetMaker::SmearMom(TVector3 p, int pid)
// {
//     float pt = p.Perp();
//     float pt1 = pt;

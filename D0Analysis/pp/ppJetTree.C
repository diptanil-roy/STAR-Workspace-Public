...
bool dopp = kTRUE;   // FIXME double check before submitting!
...
Double_t ZVtxMin = -100.0;
Double_t ZVtxMax = 100.0;
...
void ppJetTree(const Char_t *inputFile="Run14_P18ih_HPSS_15164046.list", const Char_t *outputFile="test.root", Int_t nEv = 10, const Char_t *fOutJobappend=""){
  ...
  TString jetfile = "JetTree_";
  jetfile += TString(outputFile);

  TFile *foutfortrees = new TFile(jetfile, "RECREATE");
  foutfortrees->Close(); 
  ...
  LoadLibs();
    ...
    if((RunYear == mRun14) && doMBset && doTEST) inputFile = "d0dummyfilelisttest.list";
    ...
    if(RunYear == mRun12) usePrimaryTracks = kFALSE;
  ...
  //StPicoDstMaker *picoMaker = new StPicoDstMaker(0,inputFile,"picoDst");
    ...
    StCentMaker *CentMaker = new StCentMaker("CentMaker", picoMaker, jetfile, doComments);
    ...
    //////////// Just checking PID Cuts for now /////////////////////
    StPIDQA *PidQA = new StPIDQA("PIDQA", picoMaker, jetfile);
    // if (doTEST) PidQA->SaveWideMassBin();
    // PidQA->SaveD0Trees();
    PidQA->ProcessHTEvents();
    cout << PidQA->GetName() << endl;


    StD0EventsJetMaker *jetTask = new StD0EventsJetMaker("JetMaker", fJetConstituentCut, kTRUE, outputFile);
    ...
    jetTask->SetD0Analysis(kTRUE);
    cout << jetTask->GetName() << endl;


    StJetTreeMaker *jettree = new StJetTreeMaker("JetTree", picoMaker, jetfile, "JetMaker", "");
    ...
    for (Int_t i = 0; i < nEvents; i++){
      ...
      else {if (i%50000 == 0) cout << "Working on eventNumber " << i << endl;}
  ...
  // close output file if open
    ...
    if (foutfortrees->IsOpen()) foutfortrees->Close();

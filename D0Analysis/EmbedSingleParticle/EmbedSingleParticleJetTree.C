...
void EmbedSingleParticleJetTree(const Char_t *inputFile="Run14_P18ih_HPSS_15164046.list", const Char_t *outputFile="test.root", Int_t nEv = 10, const Char_t *fOutJobappend=""){
  ...
  LoadLibs();
    ...
    if((RunYear == mRun14) && doMBset && doTEST) inputFile = "/star/u/droy1/Y2019/STAR/FileList_Feb17/TestFileList.list";
  ...
  //StPicoDstMaker *picoMaker = new StPicoDstMaker(0,inputFile,"picoDst");
    ...
    bool singleparticleembedding = kTRUE;

    StGenerateARandomTrack *RandomTrackClass = new StGenerateARandomTrack("RandomParticle", picoMaker, outputFile);
    RandomTrackClass->DoSingleParticleEmbedding();
    cout<<RandomTrackClass->GetName()<<endl;  // print name of class instance
    
    TString EmbedMakerAppend = "Embed";
    int d0kind = 1;

    if (singleparticleembedding) d0kind = -1;

    StD0EventsJetMaker *jetTask = new StD0EventsJetMaker(Form("JetMaker_%s", EmbedMakerAppend.Data()), fJetConstituentCut, kTRUE, outputFile);
    ...
    jetTask->SetD0Analysis(kFALSE);
    jetTask->GetEmbeddedParticle();
    cout << jetTask->GetName() << endl;

    if(doBackgroundJets) {
      StD0EventsJetMaker *jetTaskBG = new StD0EventsJetMaker(Form("JetMakerBG_%s", EmbedMakerAppend.Data()), fJetConstituentCut, kTRUE, outputFile);
      ...
      jetTaskBG->SetD0Analysis(kFALSE);
      jetTaskBG->GetEmbeddedParticle();
      cout << jetTaskBG->GetName() << endl;
    }

    bool dohisto = kFALSE;

    if(doBackgroundJets) { StD0Rho *rhoTask = new StD0Rho(Form("Rho_%s", EmbedMakerAppend.Data()), dohisto, outputFile, Form("JetMakerBG_%s", EmbedMakerAppend.Data()));
    } else { StD0Rho *rhoTask = new StD0Rho(Form("Rho_%s_%s", CutNames[cut].Data(), SignPair[bin].Data()), dohisto, outputFile, Form("JetMaker_%s", EmbedMakerAppend.Data())); }
    ...
    StJetTreeMaker *jettree = new StJetTreeMaker(Form("JetTree_%s", EmbedMakerAppend.Data()), picoMaker, JetTreeFileName.Data(), Form("JetMaker_%s", EmbedMakerAppend.Data()), Form("Rho_%s", EmbedMakerAppend.Data()));
    jettree->SetD0Kind(d0kind);
    jettree->GetEmbeddedParticle();

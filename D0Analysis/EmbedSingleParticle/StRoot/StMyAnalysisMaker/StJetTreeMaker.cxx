...
void StJetTreeMaker::RunJetsForEmbeddedParticle()
{
  ...
  for (int jetcollection = 0; jetcollection < fJetsArr.size(); jetcollection++){
    ...
    TLorentzVector fEmbeddedSingleParticle = fEmbeddedSingleParticleVector[jetcollection];

    // cout << "TREE EMBEDDED PARTICLE = " << fEmbeddedSingleParticle.Px() << "\t" << fEmbeddedSingleParticle.Py() << "\t" << fEmbeddedSingleParticle.Pz() << "\t" << fEmbeddedSingleParticle.E() << endl;
    ...
    for(int ijet = 0; ijet < njets; ijet++) {  // JET LOOP
      ...
      if (d0Jet){
        ...
        for(int itrk = 0; itrk < jet->GetNumberOfTracks(); itrk++) {
          ...
          if (trackid != 999999){
          ...
          else if (trackid == 999999){

            fJetTree.pionpt = fEmbeddedSingleParticle.Perp();
            fJetTree.pioneta = fEmbeddedSingleParticle.PseudoRapidity();
            fJetTree.pionphi = standardPhi(fEmbeddedSingleParticle.Phi());
            fJetTree.pioncharge = 1;

            fJetTree.kaonpt = fEmbeddedSingleParticle.Perp();
            fJetTree.kaoneta = fEmbeddedSingleParticle.PseudoRapidity();
            fJetTree.kaonphi = standardPhi(fEmbeddedSingleParticle.Phi());
            fJetTree.kaoncharge = -1;
            ...
            double px = fEmbeddedSingleParticle.Px();
            double py = fEmbeddedSingleParticle.Py();
            double pz = fEmbeddedSingleParticle.Pz();
            short charge = 0;

...
int StHIRecoJets::Make()
{
  ...
  for (int iter = 0; iter < numberofrecoevents; iter++){
    ...
    if (!fBackground && fPrintLevel) cout << "Event Number  = " << iter << endl;
...
//=================================================================================================================
  ...
  if (!fBackground){
    ...
    for(unsigned short iTracks = 0; iTracks < fRecoTracks[iteration].size(); iTracks++){
      ...
      if (int(v.M()) == 1 && !fBackground && fPrintLevel) cout << "Reco D0 In Jet Loop = " << v.Pt() << "\t" << v.PseudoRapidity() << endl;
...
void StHIRecoJets::FillJetBranch(int iteration)
{
  ...
  for(UInt_t ijet = 0, jetCount = 0; ijet < jets_incl.size(); ++ijet) {
    ...
    if (fPrintLevel) cout << "Jets = " << ijet << "\t" << jets_incl[ij].perp() << "\t" << jets_incl[ij].eta() << "\t" << jets_incl[ij].phi() << endl;
    ...
    for (int jtrk = 0; jtrk < fConstituents.size(); jtrk++){
      ...
      if (uid >=10000) {
        ...
        if (fPrintLevel) {if (uid >= 10000) cout << "Track ID = " << uid << "\t" << mass << endl;}

        if (int(mass)==1) { D0id = jtrk; D0Jet = kTRUE; break;}
      }
    }
    ...
    if (!fBackground){if (!D0Jet) continue;}

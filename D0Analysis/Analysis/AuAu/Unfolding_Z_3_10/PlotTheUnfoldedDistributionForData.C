...
void PlotTheUnfoldedDistributionForData(TString DirName = "tmp4", int SUPERITERATION = 20, int iteration = 4, TString NonClosureDir = ""){
	...
	for (int cent = 0; cent < 3; cent++){
		...
		for (int i = 0; i <= nSITER; i++){
			...
			UnfoldedPtRebinned[cent][i] = (TH1D *)UnfoldedPt[cent][i]->Rebin(njpt_gen_bins_var, Form("%s_Rebinned", UnfoldedPt[cent][i]->GetName()), jetpt_var_bin);
			...
			UnfoldedPtRebinned[cent][i]->GetYaxis()->SetRangeUser(pow(10,-13), pow(10, -3));
		}
	...
	for (int i = 0; i <= nSITER; i++){
		...
		RCPPt[0][i]->GetYaxis()->SetRangeUser(0, 5);
	}
	...
	for (int i = 0; i <= nSITER; i++){
		...
		RCPPt[1][i]->GetYaxis()->SetRangeUser(0, 5);
	}
	...
	for (int i = 0; i <= nSITER; i++){
		...
		RCPZ[0][i]->GetYaxis()->SetRangeUser(0, 5);
	}
	...
	for (int i = 0; i <= nSITER; i++){
		...
		RCPZ[1][i]->GetYaxis()->SetRangeUser(0, 5);
	}

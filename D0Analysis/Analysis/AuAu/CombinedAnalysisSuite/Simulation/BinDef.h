...
void DivideByBinWidth(TH1 *h){
  for(int i = 1;i<h->GetNbinsX()+1;i++){
    ...
    h->SetBinContent(i,val/width);
    h->SetBinError(i,er/width);
  }
}
...
TH1 *SetColor(TH1 *h, int color, int marker = 20, double markersize = 1.2, int fillcolor = 0){
  h->SetLineColor(color);
  if (fillcolor) h->SetFillColorAlpha(fillcolor, 0.6);
  // h->SetLineWidth(2);
...
TLine *GetLineAtOne(TH1D *h, double lowedge = NULL, double highedge = NULL){
  if (!lowedge) lowedge = h->GetBinLowEdge(1);
  if (!highedge) highedge = h->GetBinLowEdge(h->GetNbinsX()) + h->GetBinWidth(h->GetNbinsX());

  TLine *l = new TLine(lowedge, 1, highedge, 1);

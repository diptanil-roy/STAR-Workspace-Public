...
for (int i = lowlimit; i < highlimit; i++){
    ...
    */
    ...
    double lidoweight = LIDOPtCurveWeight[centhistogramtofill]->GetBinContent(LIDOPtCurveWeight[centhistogramtofill]->FindBin(MCJetPt));
    if (priormode == 0 || priormode == 1 || priormode == 4 || priormode == 6 || priormode == 7 || priormode == 8 || priormode == 9 || priormode == 10 || priormode == 11) priorweight = fonllweight;
    if (priormode == 2) priorweight = 1.0;
    if (priormode == 3) priorweight = fitweight;
    if (priormode == 5) priorweight = dataweighedweight;
    if (priormode == 12) priorweight = lidoweight;
    ...
    if (isOKWIDE) {
      ...
      if (MCJetPt > 5.) resp1D[centhistogramtofill]->Fill(RecoJetPtFromArea, MCJetPt, w);
      else resp1D[centhistogramtofill]->Fake(RecoJetPtFromArea, w);
...
switch (mode){
  ...
  for (int cent = 0;  cent < 3; cent++){
    ...
    gDirectory->WriteObject(respdR[cent], Form("RespWidedR_%i", cent));

    fMeasdRWide[cent]->Write();
    fFakedRWide[cent]->Write();
    fTruedR[cent]->Write();
    fMissdR[cent]->Write();

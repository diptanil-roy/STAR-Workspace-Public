...
void Method(){
	...
	for (int cent = 0; cent < 3; cent++){
		...
		for (int iter = NLOWITERATIONS; iter <= NITERATIONS; iter++){
			...
			// gPad->SetLogy();
			for (int siter = NSUPERITERATIONS; siter >= 1; siter--){
				// cout << iter << "\t" << siter << "\t" << UnfoldedJetZ[iter-3][siter][cent]->GetName() << endl;
				...
				UnfoldedJetZ[iter-3][siter][cent]->GetYaxis()->SetRangeUser(0.1,3.3);
	...
	for (int cent = 0; cent < 3; cent++){
		...
		for (int iter = NLOWITERATIONS; iter <= NITERATIONS; iter++){
			...
			for (int siter = NSUPERITERATIONS; siter >= 1; siter--){
				...
				// UnfoldedJetZ[iter-3][siter][cent]->GetYaxis()->SetRangeUser(7*pow(10, 2), 7*pow(10, 5));
				UnfoldedJetPt[iter-3][siter][cent]->GetYaxis()->SetRangeUser(0.1,3.3);

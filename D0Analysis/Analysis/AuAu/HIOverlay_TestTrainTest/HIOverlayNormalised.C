...
void Method(int pthatbin = 0, int SUPERITERATION = 0, int iteration = 3){
  ...
  for (int i = lowlimit; i < highlimit; i++){
    ...
    int xbin = (recoptbin - 1)*nBinsJetPtForZHist + recozbin-1;
    int ybin = (mcptbin - 1)*nBinsJetPtForMCZHist + mczbin-1;

    entries[centhistogramtofill][ybin]+=1;

  }
  for (int cent = 0; cent < 3; cent++){
    for (int i = 0; i < ynbins; i++){
      cout << "Bin i = " << i << "\t" << entries[cent][i] << endl; 
    }
  }
  ...
  for (int i = lowlimit; i < highlimit; i++){
    ...
    double w2 = (entries[centhistogramtofill][ybin-1] != 0) ? 1./double(entries[centhistogramtofill] [ybin-1]) : 0;
    ...
    if (isMCD0Pt && isRecoD0Pt && isMCJetPt && isRecoJetPt && isMCJetEta && isRecoJetEta) {
      resp2[centhistogramtofill]->Fill(RecoJetCorrPt, recoz, MCJetPt, mcz, w*w2);
...
void HIOverlayNormalised(int iteration = 3){

  for (int superiter = 0; superiter <= 0; superiter++){

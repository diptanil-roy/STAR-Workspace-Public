...
TString NAME = Form("Histograms3_D0%i_%iGeV_RecoJetPt_%i_%i_NewBinningOldCuts.root", (int)ptlow, (int)pthigh, -200, 1000);
...
double fillHistos(char file[100], TH1 *D0PtNoWeights, TH1 *D0Pt, TH1 *JetPt, TH1 *Z, TH1 *R, TH2 *ZPt, int cen_low, int cen_high, TH1 *hArea, TH1 *JetEta, TH1 *D0Eta, TH1 *JetPhi, TH1 *D0Phi, TH2 *D0JetEta, TH1 *Eta, TH2 *JetPt_R, TH2 *JetPt_Z)
{
    if (!SYS)
...
double fillHistos2(char file[100], TH1 *D0PtNoWeights, TH1 *D0Pt, TH1 *JetPt, TH1 *Z, TH1 *R, TH2 *ZPt, TH2 *D0JPt, TH2 *hard, int cen_low, int cen_high, TH1 *hArea, TH1 *JetEta, TH1 *D0Eta, TH1 *JetPhi, TH1 *D0Phi, TH2 *D0JetEta, TH1 *Eta, TH2 *JetPt_R)
{
...
double fillHistos1(char file[100], TH2 *PT2D, TH1 *D0PtNoWeights, TH1 *D0Pt, TH1 *JetPt, TH1 *Z, TH1 *R, TH2 *ZPt, int cen_low, int cen_high, TH1 *centrality, TH1 *hArea, TH1 *JetEta, TH1 *D0Eta, TH1 *JetPhi, TH1 *D0Phi, TH2 *D0JetEta, TH1 *Eta, TH2 *JetPt_R, TH2 *JetPt_Z)
{
    // if(!SYS)cout <<" >>> On fill with: " << file << endl;
    int sumw = 0;
    TFile *f_D = new TFile(file);
...
void applyWeightsFor1GeV(int save = SAVE)
{
    if (!SYS)
    ...
    { // Centrality Bins
        ...
        { // D0 pT Bins
            ...
            // JetPt[i][bin] = new TH1D(Form("JetPt_%i_%i", i, bin), Form("JetPt_%i_%i", i, bin), njpt_bins, jetpt_low ,jetpt_high);
            JetPt[i][bin] = new TH1D(Form("JetPt_%i_%i", i, bin), Form("JetPt_%i_%i", i, bin), njpt_bins, nbinsjetpt);
            // Z[i][bin] = new TH1D(Form("Z_%i_%i", i, bin), Form("Z_%i_%i", i, bin), nz_bins, z_low, z_high);
            ...
            // ZPt[i][bin] = new TH2D(Form("ZPt_%i_%i", i, bin), Form("ZPt_%i_%i", i, bin), njpt_bins ,jetpt_low ,jetpt_high, nz_bins, z_low, z_high);
            ZPt[i][bin] = new TH2D(Form("ZPt_%i_%i", i, bin), Form("ZPt_%i_%i", i, bin), njpt_bins, nbinsjetpt, nz_bins, nbinsz);
            // JetPt_R[i][bin] = new TH2D(Form("JetPt_R_%i_%i", i, bin), Form("JetPt_R_%i_%i", i, bin), njpt_bins, jetpt_low, jetpt_high, nBinsdR, dRBins);
    ...
    for (int bin = 0; bin < nd0ptbins; bin++)
    {
        // sumw += fillHistos1(Form("./sWeights/NewsWeight_PT_%i_%i_Cen_0_80.root", D0PtBin[bin], D0PtBin[bin + 1]), PT2D, D0PtUnweighted[0][bin], D0Pt[0][bin], JetPt[0][bin], Z[0][bin], R[0][bin], ZPt[0][bin], 0, 80, centrality, hArea, JetEta, D0Eta, JetPhi, D0Phi, D0JetEta, Eta, JetPt_R[0][bin], JetPt_Z[0]);
        sumw5 += fillHistos2(Form("./sWeights/NewsWeight_PT_%i_%i_Cen_40_80.root", D0PtBin[bin], D0PtBin[bin + 1]), D0PtUnweighted[5][bin], D0Pt[5][bin], JetPt[5][bin], Z[5][bin], R[5][bin], ZPt[5][bin], D0JPt_5, Hard2D2_5, 40, 80, hArea_5, JetEta_5, D0Eta_5, JetPhi_5, D0Phi_5, D0JetEta_5, Eta_5, JetPt_R[5][bin]);

        // The ones above this line are for QA only.
        sumw += fillHistos(Form("./sWeights/NewsWeight_PT_%i_%i_Cen_0_80.root", D0PtBin[bin], D0PtBin[bin + 1]), D0PtUnweighted[0][bin], D0Pt[0][bin], JetPt[0][bin], Z[0][bin], R[0][bin], ZPt[0][bin], 0, 80, hArea, JetEta, D0Eta, JetPhi, D0Phi, D0JetEta, Eta, JetPt_R[0][bin], JetPt_Z[0][bin]);
        sumw1 += fillHistos(Form("./sWeights/NewsWeight_PT_%i_%i_Cen_0_10.root", D0PtBin[bin], D0PtBin[bin + 1]), D0PtUnweighted[1][bin], D0Pt[1][bin], JetPt[1][bin], Z[1][bin], R[1][bin], ZPt[1][bin], 0, 10, hArea_1, JetEta_1, D0Eta_1, JetPhi_1, D0Phi_1, D0JetEta_1, Eta_1, JetPt_R[1][bin], JetPt_Z[1][bin]);
        sumw2 += fillHistos(Form("./sWeights/NewsWeight_PT_%i_%i_Cen_10_20.root", D0PtBin[bin], D0PtBin[bin + 1]), D0PtUnweighted[2][bin], D0Pt[2][bin], JetPt[2][bin], Z[2][bin], R[2][bin], ZPt[2][bin], 10, 20, hArea_2, JetEta_2, D0Eta_2, JetPhi_2, D0Phi_2, D0JetEta_2, Eta_2, JetPt_R[2][bin], JetPt_Z[2][bin]);
        sumw3 += fillHistos(Form("./sWeights/NewsWeight_PT_%i_%i_Cen_20_30.root", D0PtBin[bin], D0PtBin[bin + 1]), D0PtUnweighted[3][bin], D0Pt[3][bin], JetPt[3][bin], Z[3][bin], R[3][bin], ZPt[3][bin], 20, 30, hArea_3, JetEta_3, D0Eta_3, JetPhi_3, D0Phi_3, D0JetEta_3, Eta_3, JetPt_R[3][bin], JetPt_Z[3][bin]);
        sumw4 += fillHistos(Form("./sWeights/NewsWeight_PT_%i_%i_Cen_30_40.root", D0PtBin[bin], D0PtBin[bin + 1]), D0PtUnweighted[4][bin], D0Pt[4][bin], JetPt[4][bin], Z[4][bin], R[4][bin], ZPt[4][bin], 30, 40, hArea_4, JetEta_4, D0Eta_4, JetPhi_4, D0Phi_4, D0JetEta_4, Eta_4, JetPt_R[4][bin], JetPt_Z[4][bin]);
        sumw6 += fillHistos(Form("./sWeights/NewsWeight_PT_%i_%i_Cen_10_20.root", D0PtBin[bin], D0PtBin[bin + 1]), D0PtUnweighted[6][bin], D0Pt[6][bin], JetPt[6][bin], Z[6][bin], R[6][bin], ZPt[6][bin], 10, 40, hArea_6, JetEta_6, D0Eta_6, JetPhi_6, D0Phi_6, D0JetEta_6, Eta_6, JetPt_R[6][bin], JetPt_Z[6][bin]);
        sumw6 += fillHistos(Form("./sWeights/NewsWeight_PT_%i_%i_Cen_20_30.root", D0PtBin[bin], D0PtBin[bin + 1]), D0PtUnweighted[6][bin], D0Pt[6][bin], JetPt[6][bin], Z[6][bin], R[6][bin], ZPt[6][bin], 10, 40, hArea_6, JetEta_6, D0Eta_6, JetPhi_6, D0Phi_6, D0JetEta_6, Eta_6, JetPt_R[6][bin], JetPt_Z[6][bin]);
        sumw6 += fillHistos(Form("./sWeights/NewsWeight_PT_%i_%i_Cen_30_40.root", D0PtBin[bin], D0PtBin[bin + 1]), D0PtUnweighted[6][bin], D0Pt[6][bin], JetPt[6][bin], Z[6][bin], R[6][bin], ZPt[6][bin], 10, 40, hArea_6, JetEta_6, D0Eta_6, JetPhi_6, D0Phi_6, D0JetEta_6, Eta_6, JetPt_R[6][bin], JetPt_Z[6][bin]);
        sumw7 += fillHistos(Form("./sWeights/NewsWeight_PT_%i_%i_Cen_40_80.root", D0PtBin[bin], D0PtBin[bin + 1]), D0PtUnweighted[7][bin], D0Pt[7][bin], JetPt[7][bin], Z[7][bin], R[7][bin], ZPt[7][bin], 40, 80, hArea_7, JetEta_7, D0Eta_7, JetPhi_7, D0Phi_7, D0JetEta_7, Eta_7, JetPt_R[7][bin], JetPt_Z[7][bin]);
    }

...
void Method(int SUPERITERATION = 0, int iteration = 3, TString DirName = "MCMCUnf"){
	...
	TString FoldedFileName = Form("%s/FinalResponse_Step_%i_IterParam_%i_njptbin_%i_nzbin_%i.root", DirName.Data(), 0, iteration, njpt_gen_bins, nz_gen_bins); // Step 0 is always where I store the folded distro to be unfolded
	...
	for (int i = 0; i < 3; i++){
		...
		c[i]->cd(1);
		MeasuredPt[i]->GetYaxis()->SetRangeUser(pow(10, 2), pow(10, 7));
		...
		c[i]->cd(2);
		MeasuredZ[i]->GetYaxis()->SetRangeUser(pow(10, 2), pow(10, 7));
		MeasuredZ[i]->Draw("EP");
	...
	FinalOutName.ReplaceAll("FinalResponse_", "Output_");
...
void UnfoldTheMCFoldedDistribution(int SUPERITERATION = 1, int iteration = 3, TString DirName = "MCMCUnf"){

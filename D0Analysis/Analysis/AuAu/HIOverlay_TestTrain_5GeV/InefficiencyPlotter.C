...
void Method(int pthatbin = 0, int SUPERITERATION = 0, int iteration = 3){
  ...
  TH1D *hRecoPt[3];
  TH1D *hRecoPtAccepted[3];

  TH1D *Ratio[3];
  TH1D *Fakes[3];

  for (int i = 0; i < 3; i++){
    hMCPt[i] = new TH1D(Form("hMCPt_%i", i), Form("hMCPt_%i", i), nBinsJetPtForMCZHist, JetPtBinsForMCZHist);
    hMCPtAccepted[i] = new TH1D(Form("hMCPtAccepted_%i", i), Form("hMCPtAccepted_%i", i), nBinsJetPtForMCZHist, JetPtBinsForMCZHist);

    hRecoPt[i] = new TH1D(Form("hRecoPt_%i", i), Form("hRecoPt_%i", i), nBinsJetPtForZHist, JetPtBinsForZHist);
    hRecoPtAccepted[i] = new TH1D(Form("hRecoPtAccepted_%i", i), Form("hRecoPtAccepted_%i", i), nBinsJetPtForZHist, JetPtBinsForZHist);
  }
  ...
  for (int i = lowlimit; i < highlimit; i++){
    ...
    bool isMCD0Pt = MCD0Pt > 5 && MCD0Pt < 10;
    ...
    if ((isMCD0Pt && isMCJetPt && isMCJetEta)) hMCPt[centhistogramtofill]->Fill(MCJetPt);

    if (isRecoJetPt && isRecoD0Pt && isRecoJetEta) {hMCPtAccepted[centhistogramtofill]->Fill(MCJetPt); hRecoPt[centhistogramtofill]->Fill(RecoJetCorrPt);}

    if (isRecoJetPt && isRecoD0Pt && isRecoJetEta && !(isMCD0Pt && isMCJetPt && isMCJetEta)) {hRecoPtAccepted[centhistogramtofill]->Fill(RecoJetCorrPt);}
  ...
  for (int i = 0; i < 3; i++){
    ...
    Fakes[i] = (TH1D *)hRecoPtAccepted[i]->Clone();
    Fakes[i]->Divide(hRecoPt[i]);
  }
  ...
  TCanvas *d = new TCanvas("d", "d", 2000, 800);
  d->Divide(3);

  for (int i = 0; i < 3; i++){
    d->cd(i+1);
    Fakes[i]->Draw();
    Fakes[i]->SetNameTitle(Form("Fakes_%i", i), Form("Fakes_%i", i));
    // Ratio[i]->GetXaxis()->SetRangeUser(5, 20);
    // Ratio[i]->GetYaxis()->SetRangeUser(0, 1);
  }

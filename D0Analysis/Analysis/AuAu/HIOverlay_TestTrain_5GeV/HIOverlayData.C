...
void Method(int pthatbin = 0, int SUPERITERATION = 0, int iteration = 3){
  ...
  for (int i = 0; i < 3; i++){
    // resp2[i]->Scale(1./NEvents->GetBinContent(i+1));
    cout << "Centrality = " << i << "\t" << resp2[i]->Hresponse()->Integral() << "\t" << resp2[i]->Hfakes()->Integral() << "\t" << resp2[i]->Hmeasured()->Integral() << endl;

    cout << "Centrality = " << i << "\t" << miss << "\t" << fakes << "\t" << measured << endl;
    hVzMC[i]->Write();
...
void DataUnfoldMaker(int SUPERITERATION = 0, int iteration = 3){
  ...
  TFile *OriginalResponseFile = new TFile("../SPlotFrameWork/ApplyWeights/Histograms3_D05_10GeV_JetPt0_20_With2DJetPtZ.root");
  OriginalResponseFile->cd();
  ...
  RCPPt[0] = (TH1D *)ProcessSpectraHistogram((TH1D *)UnfoldedPt[0]->Clone("RCPPt_0"));
  RCPPt[1] = (TH1D *)ProcessSpectraHistogram((TH1D *)UnfoldedPt[1]->Clone("RCPPt_1"));
  RCPPt[2] = (TH1D *)ProcessSpectraHistogram((TH1D *)UnfoldedPt[0]->Clone("RCPPt_2"));
...
void HIOverlayData(int iteration = 3){

  for (int superiter = 0; superiter <= 30; superiter++){

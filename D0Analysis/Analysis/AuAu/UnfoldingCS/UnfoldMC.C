...
void Method(TString DirName = "MCMCUnf", int SUPERITERATION = 1, int iteration = 4, bool mimicDataUncertainty = kFALSE){
	...
	TString FoldedFileName = Form("%s/Response_Step_%i_IterParam_%i_njptbin_%i_nzbin_%i_Split_%i.root", DirName.Data(), 0, iteration, njpt_gen_bins_var, nz_gen_bins, 2); // Step 0 is always where I store the folded distro to be unfolded
	...
	if (mimicDataUncertainty){
		TFile *DataFile = new TFile(Form("../SPlot_Mar2_2023/Histograms3_D0%i_10GeV_RecoJetPt_%i_%i_NewBinningOldCuts.root", 1, (int)nbinsjetpt[0], (int)nbinsjetpt[njpt_bins]), "READ");
		DataFile->cd();

...
void Method(){
	...
	for (int iter = NLOWITERATIONS; iter <= NITERATIONS; iter++){
		...
		for (int siter = 0; siter <= NSUPERITERATIONS; siter++){
			...
			for (int cent = 0; cent < 3; cent++){
				if (siter == 0) {
					MCJetPt[cent] = (TH1D *)f->Get(Form("PriorMCPt_%i", cent));
					MCJetZ[cent] = (TH1D *)f->Get(Form("PriorMCZ_%i", cent));
			...
			RatioJetPt[iter-3][siter][2]->Divide(UnfoldedJetPt[iter-3][siter][1]);
	...
	// for (int cent = 0; cent < 3; cent++){
	// 	for (int iter = 3; iter < 3 + 1; iter++){
	// 		cout << "====================================== ITER = " << iter << endl;
	// 		for (int siter = NSUPERITERATIONS; siter < NSUPERITERATIONS+1; siter++){
	// 				cout << iter << "\t" << siter << "\t" << cent << "\t" << UnfoldedJetZ[iter-3][siter][cent]->GetName() << endl;
	// 		}
	// 	}
	// }
	...
	for (int cent = 0; cent < 3; cent++){
		...
		for (int iter = NLOWITERATIONS; iter <= NITERATIONS; iter++){
			...
			for (int siter = NSUPERITERATIONS; siter >= 0; siter--){
				...
				UnfoldedJetPt[iter-3][siter][cent]->GetYaxis()->SetRangeUser(1.0*pow(10, -14), pow(10, -4));

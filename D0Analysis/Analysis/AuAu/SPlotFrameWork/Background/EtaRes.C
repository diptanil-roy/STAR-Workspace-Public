void EtaRes(){
    ...
    for(int i = 1;i<eta_sigma_pt_0_10->GetNbinsX()+1;i++){
	...
	TH1F* h1 = (TH1F*)temp_1->ProjectionX("h1",i,i);//bin1,bin2);
	eta_sigma_pt_0_10->SetBinContent(i,h1->GetRMS());
	cout << eta_sigma_pt_0_10->GetBinLowEdge(i) << " " << bin1 << " " << bin2 << " " << h1->GetRMS() << " " << temp_1->GetYaxis()->GetBinLowEdge(bin1) << " " << temp_1->GetYaxis()->GetBinLowEdge(i)+ temp_1->GetYaxis()->GetBinWidth(i) <<endl;

	TH1F* h1 = (TH1F*)temp_3->ProjectionX("h1",i,i);//bin1,bin2);
	eta_sigma_pt_10_40->SetBinContent(i,h1->GetRMS());
	
	TH1F* h1 = (TH1F*)temp_6->ProjectionX("h1",i,i);//bin1,bin2);

...
vector<double> invmassplotter(const int centbin = 0, const int d0bin = 0){
    ...
    int allowedD0PtBinsForRooFit[] = {0, 3, 4, 5, 6, 7, 8, 9, 10, 11}; // Refer to the D0 pT definitions above

    RooRealVar invmassSig("invmassSig", "invmassSig", 1.7, 2.1);
    RooRealVar invmassSigWeight("invmassSigWeight", "invmassSigWeight", 0, 10);

    RooDataSet* data1 = RooDataSet::read(Form("./TxtFiles/Sigbg_CentBin_%i_D0PtBin_%i.txt", centbin, allowedD0PtBinsForRooFit[d0bin]), RooArgList(invmassSig, invmassSigWeight), "Q") ;
    ...
    RooDataSet* data2 = RooDataSet::read(Form("./TxtFiles/LSbg_CentBin_%i_D0PtBin_%i.txt", centbin, allowedD0PtBinsForRooFit[d0bin]), RooArgList(invmassBg, invmassBgWeight), "Q") ;
    ...
    double n1startvalue, n2startvalue, n3startvalue;

    if (bgfit->status() > 0) {n1startvalue = 2; n2startvalue = 1; n3startvalue = 1;}
    else{n1startvalue = p1.getVal(); n2startvalue = p2.getVal(); n3startvalue = p3.getVal();}

    RooRealVar n1("n1","n1", n1startvalue, -10, 10);
    RooRealVar n2("n2","n2", n2startvalue, -10, 10);
    RooRealVar n3("n3","n3", n3startvalue, -10, 10);
    ...
    Plot->GetYaxis()->SetTitle("Counts (/50 MeV/#it{c}^{2})");
    Plot->GetXaxis()->SetTitle("m_{K#pi} [GeV/#it{c}^{2}]");
    if (d0bin == 0) Plot->GetYaxis()->SetRangeUser(1, 1200);
    ...
    TString d0ptbins_name[10] = {"1.0 - 10.0", "1.0 - 1.5", "1.5 - 2.0", "2.0 - 2.5", "2.5 - 3.0", "3.0 - 4.0", "4.0 - 5.0", "5.0 - 6.0", "6.0 - 8.0", "8.0 - 10.0"};

    TString c0_filename = "./Plots/";
    ...
    TCanvas *c0 = new TCanvas(c0_filename.Data(), c0_filename.Data(), 5,5,900   ,600);
    ...
    auto legend = new TLegend(0.90,0.65,0.65,0.88);
    ...
    data->AddText(Form("0-80 %%"));
    data->AddText(Form("#it{p}_{T,K#pi} [GeV/#it{c}] #in %s ", "(5, 10)"));
    data->Draw("SAME");
...
void TemporaryUnbinnedYieldMaker(){
	const int nBinsCent = 4;
  	const int nBinsD0Pt = 10;
	...
	for (int centbin = 0; centbin < 1; centbin++){
        ...
        YieldFiles[centbin].open(Form("./RooYield/%s.txt", centbinsname[centbin].Data()));

        for (int d0bin = 0; d0bin < 1; d0bin++){

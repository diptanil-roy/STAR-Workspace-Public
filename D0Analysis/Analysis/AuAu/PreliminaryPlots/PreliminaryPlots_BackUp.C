...
void RCP(){
	...
	TH1D *tmpHist = new TH1D("tmpHist", "tmpHist", 20, 5, 200);
	...
	TH1D *tmp1 = (TH1D *)Central_Sys->Clone("tmp1");
	tmp1->SetNdivisions(510,"y");

	tmpHist->GetYaxis()->SetRangeUser(0, 1.8);
	tmpHist->Draw();
	
	Central_Sys->Draw("E2 SAME");
	Central_Sys->SetFillColorAlpha(kGreen-2, 0.35);
	Central_Sys->SetFillStyle(3356);
	Central_Sys->SetTitleOffset(1.1, "Y");
	Central_Sys->SetMarkerStyle(20);
    ...
    gPad->SaveAs("RCP_LogX_D0Pt5GeV_AuAu200GeV_Run14_STAR.pdf");
}

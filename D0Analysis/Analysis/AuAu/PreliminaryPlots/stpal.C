// Macro: stpal.C
// Author: G. Van Buren (BNL), 2021
// Purpose: Replicates TColor palettes available in ROOT6 for ROOT5
//   more information at
//   https://root.cern.ch/doc/master/classTColor.html
// Usage:
//   To get the default "Bird" palette of ROOT6:
//     .x stpal.C(kBird)
//   To get an inverted palette, just use a negative sign:
//     .x stpal.C(-kCherry)
//

void stpal(int ncolors=57);
...
bool invertIt = kFALSE;
Int_t TColor_CreateGradientColorTable(UInt_t Number, Double_t* Stops,
                                Double_t* Red, Double_t* Green,
                                Double_t* Blue, UInt_t NColors, Float_t alpha) {
  if (invertIt) {
    Double_t invRed[256];
    Double_t invGreen[256];
    Double_t invBlue[256];
    for (int i=0;i<Number;i++) {
      invRed[i] = Red[Number-i-1];
      invGreen[i] = Green[Number-i-1];
      invBlue[i] = Blue[Number-i-1];
    }
    return TColor::CreateGradientColorTable(Number,Stops,invRed,invGreen,invBlue,NColors,alpha);
  }
  return TColor::CreateGradientColorTable(Number,Stops,Red,Green,Blue,NColors,alpha);
}

void stpal(int ncolors) {
...
case 51:
   {
      ...
      Idx = TColor_CreateGradientColorTable(9, stops, red, green, blue, 255, alpha);
   }
   break;
...
case 199:
   {
      Double_t blue[9]   = { 242./255., 234./255., 237./255., 230./255., 212./255., 156./255., 99./255., 45./255., 0./255.};
      Double_t green[9] = { 243./255., 238./255., 238./255., 168./255., 101./255.,  45./255.,  0./255.,  0./255., 0./255.};
      Double_t red[9]  = { 230./255.,  95./255.,  11./255.,   8./255.,   9./255.,   3./255.,  1./255.,  1./255., 0./255.};
      Idx = TColor_CreateGradientColorTable(9, stops, red, green, blue, 255, alpha);
   }
   break;

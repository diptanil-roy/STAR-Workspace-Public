...
int GetD0PtBin(Double_t pt){
  ...
  if (pt >= 5.0 && pt < 6.0) bin = 9;
  if (pt >= 6.0 && pt < 8.0) bin = 10;
  if (pt >= 8.0 && pt <= 10.0) bin = 11;
...
void JetAnalysis(TString cutlevel = "Standard", TString Sign = "UnlikeSign", TFile *outfile = 0x0, double fiducialptcut = 2.0){
  ...
  for (int event = 0; event < nentries;  event++){
    ...
    TString filename;
    if (Sign.CompareTo("UnlikeSign") == 0) filename = "TxtFiles/Sigbg";
    else if (Sign.CompareTo("LikeSign") == 0) filename = "TxtFiles/LSbg";
...
void Method(double fiducialptcut = 2.0){
  ...
  const int nBinsD0PtForRooFit = 10;
  ...
  for (int i = 0; i < 4; i++){
    for (int j = 0; j < nBinsD0PtForRooFit; j++){
      ...
      // cout << i << "\t" << j << endl;
      sigbg[i][j].open(Form("TxtFiles/Sigbg_CentBin_%i_D0PtBin_%i.txt", i, allowedD0PtBinsForRooFit[j]), ios::out | ios::binary);
      sigbg[i][j].close();
      lsbg[i][j].open(Form("TxtFiles/LSbg_CentBin_%i_D0PtBin_%i.txt", i, allowedD0PtBinsForRooFit[j]), ios::out | ios::binary);
      lsbg[i][j].close();
    }
  }
  ...
  outfile = "./D0JetsDistribution.root";
...
void TemporaryUnbinnedFit(double fiducialptcut = 1.0){

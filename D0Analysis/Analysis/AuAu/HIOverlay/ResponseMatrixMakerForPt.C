...
TH1D *Method(double method){
    ...
    else g = new TFile(Form("RecoZReweight/ResponsePlots_Weighted_pow_%.1f.root", method));
	g->cd();
	for (int i = 0; i < 3; i++){
    ...
    // Central
    Response[0]  = (TH2D *)ResponseFunc(ZResampled[0]);
    MCJetPt[0]   = (TH1D *)MCFunc(ZResampled[0]);
    RecoJetPt[0] = (TH1D *)RecoFunc(ZResampled[0]);

    // MidCentral
    // ZResampled[1]->GetAxis(4)->SetRange(2,14);
    Response[1]  = (TH2D *)ResponseFunc(ZResampled[1]);
    MCJetPt[1]   = (TH1D *)MCFunc(ZResampled[1]);
    RecoJetPt[1] = (TH1D *)RecoFunc(ZResampled[1]);

    // Peripheral
    // ZResampled[2]->GetAxis(4)->SetRange(3,14);
    Response[2]  = (TH2D *)ResponseFunc(ZResampled[2]);
    MCJetPt[2]   = (TH1D *)MCFunc(ZResampled[2]);
    RecoJetPt[2] = (TH1D *)RecoFunc(ZResampled[2]);

    RooUnfoldResponse *response[3];

    for (int i = 0; i < 3; i++){
		response[i] = new RooUnfoldResponse(RecoJetPt[i],MCJetPt[i],Response[i],Form("Response_%i", i),Form("Response_%i", i));
    }
    ...
    RooUnfoldBayes unfold0 (response[0], PreProcessFoldedHistogram(JetPt[0], EfficiencyRatioForReco[0]), 4);
    RooUnfoldBayes unfold1 (response[1], PreProcessFoldedHistogram(JetPt[1], EfficiencyRatioForReco[1]), 4);
    RooUnfoldBayes unfold2 (response[2], PreProcessFoldedHistogram(JetPt[2], EfficiencyRatioForReco[2]), 4);

    UnfoldedJetPt[0] = (TH1D *)ProcessSpectraHistogram((TH1D*) unfold0.Hreco(), EfficiencyRatioForGen[0]);
    UnfoldedJetPt[1] = (TH1D *)ProcessSpectraHistogram((TH1D*) unfold1.Hreco(), EfficiencyRatioForGen[1]);
    UnfoldedJetPt[2] = (TH1D *)ProcessSpectraHistogram((TH1D*) unfold2.Hreco(), EfficiencyRatioForGen[2]);
    ...
    b->SaveAs(Form("RecoZReweight/UnfoldedSpectra_%.1f.pdf", method));
    ...
    c->SaveAs(Form("RecoZReweight/RCP_%.1f.pdf", method));
...
void ResponseMatrixMakerForPt(){
	...
	int col[13] = {1, 51, 84};
	double methodname[3] = {100.0, 300.0};
    ...
    c->SaveAs(Form("RecoZReweight/RatioofRCPs.pdf"));
}

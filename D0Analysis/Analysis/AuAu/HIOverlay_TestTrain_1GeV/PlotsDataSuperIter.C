...
void Method(){
   ...
   double taa[3] = {941.23714, 391.35550, 56.62475};
	...
	const int NSUPERITERATIONS = 10;

	TH1D *NonClosure[NITERATIONS][NSUPERITERATIONS + 2][3];
	TFile *FiveGeVNonClosure = new TFile("FiveGeVNonClosure.root");
   	FiveGeVNonClosure->cd();
   	for (int cent = 0; cent < 3; cent++){
	   for (int siter = NSUPERITERATIONS; siter >= 0; siter--){
	   		NonClosure[0][siter][cent] = (TH1D *)FiveGeVNonClosure->Get(Form("NonClosure_%i_%i_%i", 0, siter, cent));
	   		NonClosure[0][siter][cent]->SetDirectory(0);
	   	}
	}
	...
	TH1D *RebinnedUnfoldedJetPt[NITERATIONS][NSUPERITERATIONS + 2][3];
	TH1D *RebinnedUnfoldedJetZ[NITERATIONS][NSUPERITERATIONS + 2][3];

	for (int iter = NLOWITERATIONS; iter <= NITERATIONS; iter++){
		// cout << "====================================== ITER = " << iter << endl;
		for (int siter = 0; siter <= NSUPERITERATIONS; siter++){
			TFile *f = new TFile(Form("Data/Response_%i_%i.root", siter, iter));
			for (int cent = 0; cent < 3; cent++){
				...
				if (siter == 0) {
					...
					cout << "Int" << VzMC[iter-3][cent]->Integral() << endl;

					MCJetPt[cent]->Scale(1./MCJetPt[cent]->Integral());
					MCJetZ[cent]->Scale(1./MCJetZ[cent]->Integral());
				}
				...
				// }
				UnfoldedJetPt[iter-3][siter][cent]->Divide(NonClosure[iter-3][siter][cent]);
				...
				// UnfoldedJetPt[iter-3][siter][cent]->Rebin(nBinsJetPtForPlotting, Form("RebinUnfoldedPt_%i_%i_%i", iter, siter, cent), JetPtBinsForPlot);

				// RebinnedUnfoldedJetPt[iter-3][siter][cent] = (TH1D *)Rebin(UnfoldedJetPt[iter-3][siter][cent], Form("RebinUnfoldedPt_%i_%i_%i", iter, siter, cent), nBinsJetPtForPlotting, JetPtBinsForPlot);

				RebinnedUnfoldedJetPt[iter-3][siter][cent] = (TH1D *)UnfoldedJetPt[iter-3][siter][cent]->Rebin(nBinsJetPtForPlotting, Form("RebinUnfoldedPt_%i_%i_%i", iter, siter, cent), JetPtBinsForPlot);
				cout << iter << "\t" << siter << "\t" << RebinnedUnfoldedJetPt[iter-3][siter][cent]->GetName() << "\t" << RebinnedUnfoldedJetPt[iter-3][siter][cent]->Integral() << endl;


				RebinnedUnfoldedJetPt[iter-3][siter][cent]->SetDirectory(0);
	...
	for (int iter = NLOWITERATIONS; iter <= NITERATIONS; iter++){
		for (int siter = 0; siter <= NSUPERITERATIONS; siter++){
			...
			RatioJetPt[iter-3][siter][0] = (TH1D *)RebinnedUnfoldedJetPt[iter-3][siter][0]->Clone();
			RatioJetPt[iter-3][siter][1] = (TH1D *)RebinnedUnfoldedJetPt[iter-3][siter][1]->Clone();
			RatioJetPt[iter-3][siter][2] = (TH1D *)RebinnedUnfoldedJetPt[iter-3][siter][0]->Clone();
			RatioJetPt[iter-3][siter][0]->Divide(RebinnedUnfoldedJetPt[iter-3][siter][2]);
			RatioJetPt[iter-3][siter][1]->Divide(RebinnedUnfoldedJetPt[iter-3][siter][2]);
	...
	TFile *g = new TFile("../JetRCP_PreliminaryQM.root");
	TH1D *JetRCP0010 = (TH1D *)g->Get("JetRCP_0_10");
	TH1D *JetRCP0010Sys = (TH1D *)g->Get("JetRCP_0_10_Sys");
	...
	for (int cent = 0; cent < 3; cent++){
		...
		for (int iter = NLOWITERATIONS; iter <= NITERATIONS; iter++){
			...
			for (int siter = NSUPERITERATIONS; siter >= 0; siter--){
            	...
            	// if (siter == NSUPERITERATIONS)UnfoldedJetZ[iter-3][siter][cent]->SetTitle(Form("Resolution Parameter #%i", iter - 3 + 1));
   ...
   TCanvas *d[3];
	for (int cent = 0; cent < 3; cent++){
		...
		for (int iter = NLOWITERATIONS; iter <= NITERATIONS; iter++){
			...
			gPad->SetLogy();
			for (int siter = NSUPERITERATIONS; siter >= 0; siter--){
				UnfoldedJetPt[iter-3][siter][cent]->GetXaxis()->SetRangeUser(5, 30);
				...
				UnfoldedJetPt[iter-3][siter][cent]->GetYaxis()->SetRangeUser(1.0*pow(10, -9), pow(10, 0));
 ...
 //    }
   ...
   TCanvas *d3;
	...
	for (int iter = NLOWITERATIONS; iter <= NITERATIONS; iter++){
		...
		for (int siter = NSUPERITERATIONS; siter >= 0; siter--){
			...
			RatioJetPt[iter-3][siter][0]->GetYaxis()->SetRangeUser(-0.5, 2);
        ...
        SetColor(RatioJetPt[iter-3][0][0], kBlack);
        SetColor(JetRCP0010, kRed);
        JetRCP0010->Draw("SAME");
        JetRCP0010Sys->Draw("E2 SAME");
        // gPad->SetTitle(Form("Resolution Parameter #%i", iter - 3 + 1));
        gPad->BuildLegend();
   }
   ...
   TCanvas *d4;
	d4 = new TCanvas (Form("RCP_%i", 1), Form("RCP_%i", 1), 2000, 1000);
	// d4->Divide(5);
	for (int iter = NLOWITERATIONS; iter <= NITERATIONS; iter++){
		// d3->cd();
		d4->cd(iter - 3 + 1);
		// gPad->SetLogy();
		for (int siter = NSUPERITERATIONS; siter >= 0; siter--){
			RatioJetPt[iter-3][siter][1]->GetXaxis()->SetRangeUser(5, 20);
			RatioJetPt[iter-3][siter][1]->GetYaxis()->SetRangeUser(-0.5,2);
			RatioJetPt[iter-3][siter][1]->SetLineColorAlpha(colors[siter*3],1);
        	RatioJetPt[iter-3][siter][1]->Draw("L SAME");
        	cout << "Integral = " << RatioJetPt[iter-3][siter][1]->Integral() << endl;
        	if (siter == NSUPERITERATIONS)RatioJetPt[iter-3][siter][1]->SetTitle(Form("Resolution Parameter #%i", iter - 3 + 1));
        }
        SetColor(RatioJetPt[iter-3][0][1], kBlack);
        // gPad->SetTitle(Form("Resolution Parameter #%i", iter - 3 + 1));
        gPad->BuildLegend();
   }

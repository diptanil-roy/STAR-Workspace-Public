...
void Method(int cent = 0, int iteration = 3, TString DirName = "MCMCUnf", int mode = 0){
  ...
  TFile *f = new TFile("MCResponse_Jan26.root");
  ...
  TH1F *dPt[40]; // This is the dPt vs Pt generated from Embed_April2.root
  TH1F *Eta_Mean;
  TH1F *Eta_Sigma;
  TH1F *Phi_Mean;
  TH1F *Phi_Sigma;
  ...
  if (cent == 0) {
    for (int i = 0; i < 40; i++){
      dPt[i] = (TH1F *)pTdepdpTSmearing->Get(Form("dPt_Cent_%i_%i_Pt_%i_%i", 0, 10, i, i+1));
    }
    Eta_Mean = (TH1F *)EtaResFile->Get(Form("eta_mean_pt_%i_%i", 0, 10));
    Eta_Sigma = (TH1F *)EtaResFile->Get(Form("eta_sigma_pt_%i_%i", 0, 10));

    Phi_Mean = (TH1F *)PhiResFile->Get(Form("phi_mean_pt_%i_%i", 0, 10));
    Phi_Sigma = (TH1F *)PhiResFile->Get(Form("phi_sigma_pt_%i_%i", 0, 10));
  }
  else if (cent == 1){
    for (int i = 0; i < 40; i++){
      dPt[i] = (TH1F *)pTdepdpTSmearing->Get(Form("dPt_Cent_%i_%i_Pt_%i_%i", 10, 40, i, i+1));
    }
    Eta_Mean = (TH1F *)EtaResFile->Get(Form("eta_mean_pt_%i_%i", 10, 40));
    Eta_Sigma = (TH1F *)EtaResFile->Get(Form("eta_sigma_pt_%i_%i", 10, 40));

    Phi_Mean = (TH1F *)PhiResFile->Get(Form("phi_mean_pt_%i_%i", 10, 40));
    Phi_Sigma = (TH1F *)PhiResFile->Get(Form("phi_sigma_pt_%i_%i", 10, 40));
  }
  else if (cent == 2){
    for (int i = 0; i < 40; i++){
      dPt[i] = (TH1F *)pTdepdpTSmearing->Get(Form("dPt_Cent_%i_%i_Pt_%i_%i", 40, 80, i, i+1));
    }

    Eta_Mean = (TH1F *)EtaResFile->Get(Form("eta_mean_pt_%i_%i", 40, 80));
    Eta_Sigma = (TH1F *)EtaResFile->Get(Form("eta_sigma_pt_%i_%i", 40, 80));

    Phi_Mean = (TH1F *)PhiResFile->Get(Form("phi_mean_pt_%i_%i", 40, 80));
    Phi_Sigma = (TH1F *)PhiResFile->Get(Form("phi_sigma_pt_%i_%i", 40, 80));
  }
  ...
  for (int i = lowlimit; i < highlimit; i++){
    ...
    RecoJetCorrPt = RecoJetPt + dPt[ptbin-1]->GetRandom();
    ...
    double eta_mean_val = Eta_Mean->GetBinContent(ptbin);
    double eta_sigma_val = Eta_Sigma->GetBinContent(ptbin);

    double phi_mean_val = Phi_Mean->GetBinContent(ptbin);
    double phi_sigma_val = Phi_Sigma->GetBinContent(ptbin);
  ...
  cout << "Entries = " << fMeas[cent]->Integral() << "\t" << fTrue[cent]->Integral() << "\t" << fMeas1D[cent]->Integral() << "\t" << fTrue1D[cent]->Integral() << endl;
...
void PrepareResponseMatrixUsingMCOnly(int SUPERITERATION = 0, int iteration = 3, TString DirName = "MCMCUnf", int mode = 0){

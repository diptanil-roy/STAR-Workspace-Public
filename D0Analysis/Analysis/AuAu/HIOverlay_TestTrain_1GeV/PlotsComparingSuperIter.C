...
void Method(){
	...
	TH1D *MCPt[NITERATIONS][NSUPERITERATIONS + 2][3];
	TH1D *MCZ[NITERATIONS][NSUPERITERATIONS + 2][3];
	...
	for (int iter = NLOWITERATIONS; iter <= NITERATIONS; iter++){
		...
		for (int siter = 0; siter <= NSUPERITERATIONS; siter++){
			TFile *f = new TFile(Form("TestTrain_JetPt0GeV/Response_%i_%i.root", siter, iter));
			for (int cent = 0; cent < 3; cent++){
				...
				if (siter == 0) {
					...
					// MCJetPt[cent]->Scale(1./MCJetPt[cent]->Integral());
					// MCJetZ[cent]->Scale(1./MCJetPt[cent]->Integral());
				}
				...
				MCPt[iter-3][siter][cent] = (TH1D *)f->Get(Form("MCPt_%i", cent));
				MCPt[iter-3][siter][cent]->SetNameTitle(Form("MCPt_%i_%i_%i", iter, siter, cent), Form("MCPt_%i_%i_%i", iter, siter, cent));

				MCZ[iter-3][siter][cent] = (TH1D *)f->Get(Form("MCZ_%i", cent));
				MCZ[iter-3][siter][cent]->SetNameTitle(Form("MCZ_%i_%i_%i", iter, siter, cent), Form("MCZ_%i_%i_%i", iter, siter, cent));
				...
				MCPt[iter-3][siter][cent]->SetDirectory(0);
				MCZ[iter-3][siter][cent]->SetDirectory(0);
	...
	for (int cent = 0; cent < 3; cent++){
		...
		for (int iter = NLOWITERATIONS; iter <= NITERATIONS; iter++){
			...
			// for (int siter = NSUPERITERATIONS; siter >= 0; siter--){
				...
				UnfoldedJetZ[iter-3][siter][cent]->Divide(MCZ[iter-3][siter][cent]);
				// UnfoldedJetZ[iter-3][siter][cent]->GetYaxis()->SetRangeUser(7*pow(10, 2), 7*pow(10, 5));
				UnfoldedJetZ[iter-3][siter][cent]->GetYaxis()->SetRangeUser(0.1,13);
				...
				UnfoldedJetZ[iter-3][siter][cent]->GetYaxis()->SetTitle("#frac{Unfolded}{True}");
				UnfoldedJetZ[iter-3][siter][cent]->GetYaxis()->SetTitleOffset(0.9);
				if (siter%2 == 0) UnfoldedJetZ[iter-3][siter][cent]->SetLineColorAlpha(colors[siter/2],1);
            else UnfoldedJetZ[iter-3][siter][cent]->SetLineColorAlpha(colors[(siter+1)/2],0.4);
            	...
            	SetName(UnfoldedJetZ[iter-3][siter][cent], Form("Superiteration %i", siter));
            }
            SetName(UnfoldedJetZ[iter-3][0][cent], "Unweighted");
            SetColor(UnfoldedJetZ[iter-3][0][cent], kBlack);


           	SetName(MCZ[iter-3][0][cent], "True Z");
	         SetColor(MCZ[iter-3][0][cent], kRed);
	         MCZ[iter-3][0][cent]->SetLineWidth(3);
	         MCZ[iter-3][0][cent]->Divide(MCZ[iter-3][0][cent]);
	         MCZ[iter-3][0][cent]->GetXaxis()->SetRangeUser(0,1);
	         // MCZ[iter-3][0][cent]->GetYaxis()->SetRangeUser(0.1,3.3);
	         MCZ[iter-3][0][cent]->GetXaxis()->SetNameTitle(Form("z"), Form("z"));
				MCZ[iter-3][0][cent]->GetYaxis()->SetTitle("#frac{Unfolded}{True}");
				MCZ[iter-3][0][cent]->GetYaxis()->SetTitleOffset(0.9);
	         MCZ[iter-3][0][cent]->Draw("SAME");
			// gPad->SetTitle(Form("Resolution Parameter #%i", iter - 3 + 1));
            // MCJetZ[cent]->GetXaxis()->SetRangeUser(0,1);
            // MCJetZ[cent]->SetLineColor(kBlack);
            // MCJetZ[cent]->SetMarkerStyle(20);
            // MCJetZ[cent]->SetMarkerColor(kBlack);
 ...
 //    }
	...
	for (int cent = 0; cent < 3; cent++){
		...
		for (int iter = NLOWITERATIONS; iter <= NITERATIONS; iter++){
			...
			// for (int siter = NSUPERITERATIONS; siter >= 0; siter--){
			for (int siter = 0; siter <= NSUPERITERATIONS; siter++){
				UnfoldedJetPt[iter-3][siter][cent]->GetXaxis()->SetRangeUser(5,20);
				UnfoldedJetPt[iter-3][siter][cent]->Divide(MCPt[iter-3][siter][cent]);
				// UnfoldedJetZ[iter-3][siter][cent]->GetYaxis()->SetRangeUser(7*pow(10, 2), 7*pow(10, 5));
				UnfoldedJetPt[iter-3][siter][cent]->GetYaxis()->SetRangeUser(0.1,13);
				// UnfoldedJetPt[iter-3][siter][cent]->GetYaxis()->SetNameTitle(Form("#frac{1}{N} #frac{dN}{dp_{T,jet}}"), Form("#frac{1}{N} #frac{dN}{dp_{T,jet}}"));
				UnfoldedJetPt[iter-3][siter][cent]->GetXaxis()->SetNameTitle(Form("p_{T,Jet} [GeV/#it{c}]"), Form("p_{T,Jet} [GeV/#it{c}]"));
				UnfoldedJetPt[iter-3][siter][cent]->GetYaxis()->SetTitle("#frac{Unfolded}{True}");
				UnfoldedJetPt[iter-3][siter][cent]->GetYaxis()->SetTitleOffset(0.9);
				// UnfoldedJetPt[iter-3][siter][cent]->GetYaxis()->SetRangeUser(1.0*pow(10, -8), pow(10, 0));
				if (siter%2 == 0) UnfoldedJetPt[iter-3][siter][cent]->SetLineColorAlpha(colors[siter/2],1);
            else UnfoldedJetPt[iter-3][siter][cent]->SetLineColorAlpha(colors[(siter+1)/2],0.4);
            ...
            SetName(UnfoldedJetPt[iter-3][siter][cent], Form("Superiteration %i", siter));
         }
         SetName(UnfoldedJetPt[iter-3][0][cent], "Unweighted");
         SetColor(UnfoldedJetPt[iter-3][0][cent], kBlack);

         SetName(MCPt[iter-3][0][cent], "True Pt");
        	SetColor(MCPt[iter-3][0][cent], kRed);
         MCPt[iter-3][0][cent]->SetLineWidth(3);
         MCPt[iter-3][0][cent]->Divide(MCPt[iter-3][0][cent]);
         MCPt[iter-3][0][cent]->Draw("SAME");
         MCPt[iter-3][0][cent]->GetXaxis()->SetRangeUser(5,20);
         // MCPt[iter-3][0][cent]->GetYaxis()->SetRangeUser(0.1,3.3);
         MCPt[iter-3][0][cent]->GetYaxis()->SetTitleOffset(0.9);
         MCPt[iter-3][0][cent]->GetXaxis()->SetNameTitle(Form("p_{T,Jet} [GeV/#it{c}]"), Form("p_{T,Jet} [GeV/#it{c}]"));
			MCPt[iter-3][0][cent]->GetYaxis()->SetTitle("#frac{Unfolded}{True}");
		// gPad->SetTitle(Form("Resolution Parameter #%i", iter - 3 + 1));
         // MCJetPt[cent]->GetXaxis()->SetRangeUser(1,30);
         // MCJetPt[cent]->SetLineColor(kBlack);
         // MCJetPt[cent]->SetMarkerStyle(20);
         // MCJetPt[cent]->SetMarkerColor(kBlack);
         // MCJetPt[cent]->Draw("HIST SAME");
         gPad->BuildLegend();
        }
    }

   TFile *FiveGeVNonClosure = new TFile("FiveGeVNonClosure_JetPt3.root", "RECREATE");
   FiveGeVNonClosure->cd();
   for (int cent = 0; cent < 3; cent++){
	   for (int siter = NSUPERITERATIONS; siter >= 0; siter--){
			TH1D *h = (TH1D *)UnfoldedJetPt[0][siter][cent]->Clone();
			SetName(h, Form("NonClosure_%i_%i_%i", 0, siter, cent));
			h->Write();
		}
	}
	FiveGeVNonClosure->Close();
 ...
 //    }
    ...
    for (int cent = 0; cent < 3; cent++){
    	for (int iter = NLOWITERATIONS; iter <= NITERATIONS; iter++){
        	...
        	Chi2ForJetPt[iter-3][cent] = new TH1D(Form("Chi2ForJetPt_%i_%i", iter, cent), Form("Chi2ForJetPt_%i_%i", iter, cent), 201, low, high);
        	Chi2ForJetZ[iter-3][cent] = new TH1D(Form("Chi2ForJetZ_%i_%i", iter, cent), Form("Chi2ForJetZ_%i_%i", iter, cent), 201, low, high);
        	...
        	for (int siter = 0; siter <= NSUPERITERATIONS; siter++){
        		...
        		double chi2ptvalue_mine = Chi2Func(UnfoldedJetPt[iter-3][siter][cent], MCPt[iter-3][siter][cent]);
        		...
        		double chi2zvalue_mine = Chi2Func(UnfoldedJetZ[iter-3][siter][cent], MCZ[iter-3][siter][cent]);
   ...
   TCanvas *d3;
	...
	for (int iter = NLOWITERATIONS; iter <= NITERATIONS; iter++){
		...
		for (int siter = NSUPERITERATIONS; siter >= 0; siter--){
			...
			RatioJetPt[iter-3][siter][0]->SetLineColorAlpha(colors[siter*2],1);
   ...
   TCanvas *d4;
	...
	for (int iter = NLOWITERATIONS; iter <= NITERATIONS; iter++){
		...
		for (int siter = NSUPERITERATIONS; siter >= 0; siter--){
			...
			RatioJetPt[iter-3][siter][2]->SetLineColorAlpha(colors[siter*2],1);
	...
	for (int cent = 0; cent < 3; cent++){
		...
		for (int iter = NLOWITERATIONS; iter <= NITERATIONS; iter++){
			...
			for (int siter = NSUPERITERATIONS; siter >= 0; siter--){
				...
				Weight[iter-3][siter][cent]->SetLineColorAlpha(colors[siter*2],1);
            	// else UnfoldedJetZ[iter-3][siter][cent]->SetLineColorAlpha(colors[siter/2],0.5);

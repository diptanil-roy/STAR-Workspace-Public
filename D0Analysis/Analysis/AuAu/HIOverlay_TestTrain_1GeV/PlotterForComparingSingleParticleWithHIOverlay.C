...
void PlotterForComparingSingleParticleWithHIOverlay(){
	...
	RFile[1] = "Mar2_FONLL_HI4/OldResponse_Step_1_IterParam_4_njptbin_15_nzbin_10.root";
	...
	// RFile[6] = "Feb24_FONLL_HI_Final_2D4/OldResponse_Step_1_IterParam_4_njptbin_15_nzbin_10.root";
	...
	for (int fi = 1; fi < filenum; fi++){
		...
		for (int i = 0; i < 3; i++){
			...
			cout << UnfoldedPt2D[fi][i]->Integral() << endl;
			cout << UnfoldedZ2D[fi][i]->Integral() << endl;
	...
	cout << "Imported all the histograms." << endl;
	...
	for (int i = 0; i < 3; i++){
		...
		hData_Old_Rebinned[i]->GetXaxis()->SetRangeUser(5, 15);
		hData_Old_Rebinned[i]->GetYaxis()->SetRangeUser(5*pow(10, -12), 2*pow(10, 1));
		...
		for (int fi = 1; fi < filenum; fi++){
			...
			UnfoldedPt1D[fi][i]->GetYaxis()->SetRangeUser(5*pow(10, -12), 2*pow(10, 1));
			SetColor(UnfoldedPt1D[fi][i], color[fi]);
			l3[i]->AddEntry(UnfoldedPt1D[fi][i],Form("%s", LegAppend[fi].Data()),"p");


			UnfoldedPt1D[fi][i]->GetXaxis()->SetNdivisions(505);
			UnfoldedPt1D[fi][i]->GetYaxis()->SetNdivisions(504);
			UnfoldedPt1D[fi][i]->GetXaxis()->SetTitle("p_{T,Jet}^{D^{0}} [GeV/#it{c}]");
			UnfoldedPt1D[fi][i]->GetYaxis()->SetTitle("#frac{1}{2#pi#it{N}_{evt}} #frac{d^{2}#it{N}_{Jet}}{#it{p}_{T,jet} d#it{p}_{T,jet} d#it{#eta}} [GeV/#it{c}]^{-2}");
			UnfoldedPt1D[fi][i]->GetYaxis()->SetTitleSize(0.055);
			UnfoldedPt1D[fi][i]->GetXaxis()->SetTitleSize(0.055);
			UnfoldedPt1D[fi][i]->GetYaxis()->SetLabelSize(0.06);
			UnfoldedPt1D[fi][i]->GetXaxis()->SetLabelSize(0.06);
			UnfoldedPt1D[fi][i]->GetYaxis()->SetTitleOffset(2.0);
			UnfoldedPt1D[fi][i]->GetXaxis()->SetTitleOffset(1.1);
			UnfoldedPt1D[fi][i]->GetXaxis()->SetRangeUser(5, 15);
		}
		...
		for (int gi = 1; gi < filenum; gi++){
			...
			UnfoldedPt2D[gi][i]->GetYaxis()->SetRangeUser(5*pow(10, -12), 2*pow(10, 1));

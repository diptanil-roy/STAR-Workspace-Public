...
void plot_D0CrossS2_forRcp() {
    ...
    for(int icent=0; icent<nCent_Draw; icent++) {
        ...
        for(int ipt=0; ipt<npt; ipt++) {
          ...
          TmpCom[ipt] =  sqrt(pow(fD0Tpc[icent]->GetBinContent(fD0Tpc[icent]->GetXaxis()->FindBin(pt_mean[ipt])),2));
    ...
    ofstream outData;
    outData.open(Form("D0_Xsection2_forRcp.txt"));

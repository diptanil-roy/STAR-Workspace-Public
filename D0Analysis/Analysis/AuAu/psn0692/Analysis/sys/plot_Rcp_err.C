...
void plot_Rcp_err()
{
   ...
   //
   const int nerr = 8;
   const char nameErr[nerr][250] = {"TPC track", "PID", "raw yield extract", "single track p_{T}", "50%/150% topo. eff.", "double count", "vertex correction", "general sys."};
   ...
   for (int icent = 0; icent < ncent; icent++)
   {
      ...
      for(int ipt=0; ipt<npt; ipt++) {
          if(in.eof()) break;
          in >> vtxStat[icent][ipt];
          vtxSys[icent][ipt] = sqrt(pow(vtxSys[icent][ipt],2)+pow(vtxStat[icent][ipt],2)- 0.05*0.05);//5% general sys was subtracted here
      }
      in.close();
   ...
   for (int icent = 0; icent < ncent; icent++)
   {
      ...
      // double TmpCom =  sqrt(pow(fpiTpcRcp1[icent]->Eval(1), 2) ) + sqrt(pow(fkTpcRcp1[icent]->Eval(1), 2) );//seperate PID
      // cout << "TmpCom = " << TmpCom << endl;
        double TmpCom[npt];
        for(int ipt=0; ipt<npt; ipt++) {
          TmpCom[ipt] =  fD0TpcRcp1[icent]->GetBinContent(fD0TpcRcp1[icent]->GetXaxis()->FindBin(ptmean[ipt]));
      ...
      for (int ipt = 0; ipt < npt; ipt++)
      {
         yRcp1sys[icent][ierr][ipt] = TmpCom[ipt];
      }
      ierr++;

      //PID
      double TmpCom2 =  sqrt(pow(1. - fpipid->Eval(1), 2)) + sqrt(pow(1. - fkpid->Eval(1), 2));//seperate PID
      //sys 1.2 -- PID
      for (int ipt = 0; ipt < npt; ipt++)
      {
         // yRcp1sys[icent][ierr][ipt] = TmpCom2;
         yRcp1sys[icent][ierr][ipt] = 1.e-6;// PID do not contribute to the Rcp
      }
      ierr++;
      ...
      for (int ipt = 0; ipt < npt; ipt++)
      {
         ...
         yRcp1sys[icent][ierr][ipt] = 1.e-6;// double counting do not contribute to the Rcp
      }
      ierr++;
      ...
      //sys 8 second track
      for(int ipt=0; ipt<npt; ipt++) {
          yRcp1sys[icent][ierr][ipt] = sqrt(pow(0.05,2)+pow(0.05,2));
      }
      ierr++;
   ...
   for (int icent = 0; icent < ncent; icent++)
   {
    ...
    for(int icent=0; icent<ncent; icent++) {
      if(!(icent == 0 || icent==4) ) continue; // print 0-10%
        ...
        for(int ierr=0; ierr<nerr; ierr++) {
          for(int ipt=0; ipt<npt; ipt++) {
            cout << nameCent[icent] << " " << nameErr[ierr] << " "<<  yRcp1sys[icent][ierr][ipt] << endl;
          }
        }

          for(int ipt=0; ipt<npt; ipt++) {
            cout << " combine HFT " << nameCent[icent] << " "<<  sqrt(pow(yRcp1sys[icent][3][ipt],2)+pow(yRcp1sys[icent][4][ipt],2)) << endl;
          }
   ...
   TCanvas* c1 = new TCanvas("c1", "A Canvas",10,10,600,500);
   setPad(c1);
   gPad->SetLogy();
   for(int icent=0; icent<ncent; icent++) {
       float ymin = 1.01e-3;
       ...
       h0->SetMinimum(ymin),
       h0->SetMaximum(ymax);
       setHisto(h0,"","p_{T} (GeV/c)", "relative Sys. error");
   ...
   for (int icent = 0; icent < ncent; icent++)
   {
      ...
      // double TmpCom =  sqrt(pow(fpiTpcRcp2[icent]->Eval(1), 2) ) + sqrt(pow(fkTpcRcp2[icent]->Eval(1), 2) );//seperate PID
      // cout << "TmpCom = " << TmpCom << endl;
        double TmpCom[npt];
        for(int ipt=0; ipt<npt; ipt++) {
          TmpCom[ipt] =  fD0TpcRcp2[icent]->GetBinContent(fD0TpcRcp2[icent]->GetXaxis()->FindBin(ptmean[ipt]));
          cout<< "TmpCom[ipt] = " << TmpCom[ipt] << endl;
        }
      ...
      for (int ipt = 0; ipt < npt; ipt++)
      {
         yRcp2sys[icent][ierr][ipt] = TmpCom[ipt];
      }
      ierr++;
      ...
      for (int ipt = 0; ipt < npt; ipt++)
      {
         // yRcp2sys[icent][ierr][ipt] = TmpCom2;
         yRcp1sys[icent][ierr][ipt] = 1.e-6;// PID do not contribute to the Rcp
      }
      ierr++;
      ...
      for (int ipt = 0; ipt < npt; ipt++)
      {
         yRcp2sys[icent][ierr][ipt] = sqrt(pow(y0sys[icent][ipt], 2) + pow(y0sys[3][ipt], 2));
      }
      ierr++;
      ...
      for (int ipt = 0; ipt < npt; ipt++)
      {
         // yRcp2sys[icent][ierr][ipt] = sqrt(pow(y1sys[icent][ipt], 2) + pow(y1sys[3][ipt], 2));
         yRcp2sys[icent][ierr][ipt] = 1.e-6;// double counting do not contribute to the Rcp
      }
      ierr++;
      ...
      for (int ipt = 0; ipt < npt; ipt++)
      {
         yRcp2sys[icent][ierr][ipt] = sqrt(pow(vtxSys[icent][ipt], 2)+pow(vtxSys[3][ipt], 2));
      ...
      //sys 8 second track
      for(int ipt=0; ipt<npt; ipt++) {
          yRcp2sys[icent][ierr][ipt] = sqrt(pow(0.05,2)+pow(0.05,2));
      }
      ierr++;

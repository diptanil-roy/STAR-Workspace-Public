...
void write_Rcp_err() {
    ...
    for(int icent=0; icent<ncent; icent++) {
        ...
        for(int ipt=0; ipt<npt; ipt++) {
            ...
            vtxSys[icent][ipt] = sqrt(pow(vtxSys[icent][ipt],2)+pow(vtxStat[icent][ipt],2));
        }
        in.close();
    }
    ...
    for(int icent=0; icent<ncent; icent++) {
        ...
        double TmpCom[npt];
        for(int ipt=0; ipt<npt; ipt++) {
          // TmpCom[ipt] =  sqrt(pow(fD0TpcRcp1[icent]->GetBinContent(fD0TpcRcp1[icent]->GetXaxis()->FindBin(ptmean[ipt])),2) + pow(1.-fpipid->Eval(1),2) + pow(1.-fkpid->Eval(1),2));
          TmpCom[ipt] =  sqrt(pow(fD0TpcRcp1[icent]->GetBinContent(fD0TpcRcp1[icent]->GetXaxis()->FindBin(ptmean[ipt])),2));
          cout<< "TmpCom[ipt] = " << TmpCom[ipt] << endl;
        }

        //sys 1 -- tpc track
        for(int ipt=0; ipt<npt; ipt++) {
            yRcp1sys[icent][ipt] = sqrt(pow(TmpCom[ipt],2)+pow(yRcp1sys[icent][ipt],2)+pow(0.014,2));
        }
        ...
        //sys 5 -- double count
        // this part will not contribute to the Rcp
        for(int ipt=0; ipt<npt; ipt++) {
    ...
    for(int icent=0; icent<ncent; icent++) {
        ...
        double TmpCom[npt];
        for(int ipt=0; ipt<npt; ipt++) {
          // TmpCom[ipt] =  sqrt(pow(fD0TpcRcp2[icent]->GetBinContent(fD0TpcRcp2[icent]->GetXaxis()->FindBin(ptmean[ipt])),2) + pow(1.-fpipid->Eval(1),2) + pow(1.-fkpid->Eval(1),2));
          TmpCom[ipt] =  sqrt(pow(fD0TpcRcp2[icent]->GetBinContent(fD0TpcRcp2[icent]->GetXaxis()->FindBin(ptmean[ipt])),2));
          cout<< "TmpCom[ipt] = " << TmpCom[ipt] << endl;
        }

        //sys 1 -- tpc track
        for(int ipt=0; ipt<npt; ipt++) {
            yRcp2sys[icent][ipt] = sqrt(pow(TmpCom[ipt],2)+pow(yRcp2sys[icent][ipt],2)+pow(0.014,2));
            // cout <<  "\t" << yRcp2sys[icent][ipt] << endl;
        }

        //sys 2 -- yield extaction 
        for(int ipt=0; ipt<npt; ipt++) {
            yRcp2sys[icent][ipt] = sqrt(pow(y0sys[icent][ipt],2) + pow(y0sys[3][ipt],2) +pow(yRcp2sys[icent][ipt],2));
        }
        ...
        // this part will not contribute to the Rcp
        for(int ipt=0; ipt<npt; ipt++) {
            // yRcp2sys[icent][ipt] = sqrt(pow(y1sys[icent][ipt],2) + pow(y1sys[3][ipt],2) +pow(yRcp2sys[icent][ipt],2));
        }

        // do the vertex reso. correction
        // vtx sys. error
        for(int ipt=0; ipt<npt; ipt++) {
            // yRcp2sys[icent][ipt] = sqrt(pow(vtxSys[icent][ipt],2)+pow(vtxSys[3][ipt],2)+pow(yRcp2sys[icent][ipt],2));
        ...
        TGraphErrors* gD0Rcpsys_vtx = new TGraphErrors(npt, ptmean, unit, 0, vtxSys[3]);
        gD0Rcpsys_vtx->SetMarkerStyle(MARKERSTYLE[0]);
        gD0Rcpsys_vtx->SetMarkerSize(2.);
        gD0Rcpsys_vtx->SetMarkerColor(COLOR[3]);
        gD0Rcpsys_vtx->SetTitle(nameCent[3]);
        gD0Rcpsys_vtx->Write(Form("gD0Rcp2_sys_vtx_%s",nameCent1[3]));

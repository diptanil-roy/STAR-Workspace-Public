...
// if(icent != 0) continue;
        ...
        for(int ipt=0; ipt<npt - 1 ; ipt++) {
            ...
            // hInvMass[ipt]->Fit(fD0,"INOR","",fitRange_lw, fitRange_up);
            hInvMass[ipt]->Fit(fD0,"R","",fitRange_lw, fitRange_up);
            hInvMass[ipt]->Fit(fD0,"R","",fitRange_lw, fitRange_up);
            //hInvMass[ipt]->Fit(fD0,"INOR","",fitRange_lw, fitRange_up);
            ...
            drawLatex(0.55,0.9,Form("RawYield = %2.0f #pm %2.0f",yield, yielderr),22,0.05,4);
            drawLatex(0.55,0.8,Form("sig. = %2.1f",yield*1.0/yielderr),22,0.05,4);
        ...
        for(int ipt=npt -1; ipt<npt; ipt++) {// use US to calculate the raw signal
            pad1->cd();
            pad1->SetLogy(0);
            hInvMassUL[ipt]->Draw("E");
            setHisto(hInvMassUL[ipt],"","M_{k#pi} (GeV/c^{2})", "Counts (per 10 MeV/c^{2})");
            ...
            hInvMass[ipt]->SetMarkerStyle(20);
            hInvMass[ipt]->SetMarkerSize(1.2);
            hInvMass[ipt]->SetMarkerColor(4);
            
            // Fit residual bg and signal (gauss + pol1)
            float N = hInvMassUL[ipt]->GetBinContent(hInvMassUL[ipt]->FindBin(1.86));
            resifunFit->SetParameters(0.5*N, -0.5*N);
            hInvMassUL[ipt]->Fit(resifunFit,"INOR","",fitRange_lw, fitRange_up);
            hInvMassUL[ipt]->Fit(resifunFit,"INOR","",fitRange_lw, fitRange_up);
            // hInvMassUL[ipt]->Fit(resifunFit,"INOR","",fitRange_lw, fitRange_up);
            //cout << resifunFit->GetParameter(0) << "\t" << resifunFit->GetParameter(1) << endl;
            // fD0->SetParameters(N, 1.866, 0.014, hInvMassUL[ipt]->GetBinWidth(4),resifunFit->GetParameter(0),resifunFit->GetParameter(1));
            fD0->SetParameters(N, 1.866, 0.014, hInvMassUL[ipt]->GetBinWidth(4),1,1);
            //fD0->SetParLimits(0,0,1.e9);
            // fD0->SetParLimits(1,1.85,1.88);
            // fD0->SetParLimits(2,0.01,0.03);
            fD0->SetParameter(2,sigmamean[npt-2]);
            // fD0->SetParLimits(2,sigmamean[npt-2]*0.4, sigmamean[npt-2]*1.5);
            fD0->FixParameter(3,hInvMassUL[ipt]->GetBinWidth(4));
            //fD0->FixParameter(4,resifunFit->GetParameter(0));
            //fD0->FixParameter(5,resifunFit->GetParameter(1));
            fD0->SetParError(4,resifunFit->GetParError(0));
            fD0->SetParError(5,resifunFit->GetParError(1));
            hInvMassUL[ipt]->Fit(fD0,"INOR","",fitRange_lw, fitRange_up);
            hInvMassUL[ipt]->Fit(fD0,"INOR","",fitRange_lw, fitRange_up);
            // hInvMassUL[ipt]->Fit(fD0,"R","",fitRange_lw, fitRange_up);
            // hInvMassUL[ipt]->Fit(fD0,"R","",fitRange_lw, fitRange_up);
            //hInvMassUL[ipt]->Fit(fD0,"INOR","",fitRange_lw, fitRange_up);
            ...
            float higx2 = higx + 2*nsigma + shift;
            float signalAndBg, signalAndBg_err, sideBand1, sideBand1_err, sideBand2, sideBand2_err;
            hIntegralAndError(hInvMassUL[ipt],lowx,higx,signalAndBg,signalAndBg_err);
            hIntegralAndError(hInvMassUL[ipt],lowx1,higx1,sideBand1,sideBand1_err);
            hIntegralAndError(hInvMassUL[ipt],lowx2,higx2,sideBand2,sideBand2_err);
            cout << hInvMassUL[ipt]->FindBin(lowx) << "\t" << hInvMassUL[ipt]->FindBin(higx) << endl;
            float bg = 0.5*(sideBand1+sideBand2);
            float bg_err = 0.5*sqrt(pow(sideBand1_err,2)+pow(sideBand2_err,2));
            //float yield = signalAndBg - bg;
            //float yielderr = sqrt(pow(signalAndBg_err,2)+pow(bg_err,2));
            ...
            hIntegralAndError(hInvMassMixUL[ipt],lowx,higx,mixbg,mixbg_err);
            B = bg + mixbg;
            cout << "bin width: " << hInvMassUL[ipt]->GetBinWidth(2) << endl;
            cout << "S = " << S << endl;
            cout << "B = " << B << endl;
            int N_y = round(yield);
            int N_error = round(yielderr);
            ...
            legend->SetBorderSize(0);
            legend->SetFillStyle(0);
            legend->AddEntry(hInvMassUL[ipt]," US (SE)","lep");
            legend->AddEntry(hInvMass[ipt]," US (SE) - US (ME)","lep");
            legend->AddEntry(fD0," Fit","l");
            //legend2->AddEntry(hInvMass[ipt]," Unlike - Like","lep");
            legend->Draw();

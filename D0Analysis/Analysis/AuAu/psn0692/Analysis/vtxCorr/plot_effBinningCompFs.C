...
void plot_effBinningCompFs()
{
    ...
    TH2F* h2Base_hj;
    TH2F* h2Eff_hj[neff];
    TFile* fin1 = new TFile("./out/eff.FastSimu.Hijing.root");
    ...
    TH2F* h2Base_fs;
    TH2F* h2Eff_fs[neff];
    TFile* fin2 = new TFile("./out_old/eff.FastSimu.Hijing.root");
    ...
    for(int icent=0; icent<ncent_vtx; icent++) {
        for(int ieff=0; ieff<neff; ieff++) {
            ...
            const float ymax = 5*heffFs[icent][ieff]->GetMaximum();
            ...
            lg1->AddEntry(heffHj[icent][ieff],"New FastSim","p");
            lg1->AddEntry(heffFs[icent][ieff],"Old FastSim","p");
            lg1->Draw();

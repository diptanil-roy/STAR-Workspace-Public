...
#include "./0319/anaCuts.h"
void plot_effBinning_22()
{
    globalSetting();
    char dir[250];
    char name[250];
    ...
    // modified
    const int ncent_vtx = 8;
    const char nameCent_vtx[ncent_vtx][100] = {"70-80%","60-70%","50-60%","40-50%","30-40%","20-30%","10-20%","0-10%"};
    const char nameCent_vtx1[ncent_vtx][100] = {"70_80","60_70","50_60","40_50","30_40","20_30","10_20","0_10"};
    
    //read eff from pure hijing
    TH2F* h2Base_hj;
    TH2F* h2Eff_hj[neff];
    // TFile* fin1 = new TFile("../out/effPureHijing.root");
    TFile* fin1 = new TFile("./out/effPureHijing_sys.root");
    ...
    TFile* fin2 = new TFile("./out/eff.FastSimu.Hijing_sys.root");
    ...
    for(int icent=0; icent<ncent_vtx; icent++) {
        ...
        for(int ieff=0; ieff<neff; ieff++) {
            ...
            heffHj[icent][ieff] = histDivide(heffHj[icent][ieff],hbase_hj,true);
            ...
            heffFs[icent][ieff] = histDivide(heffFs[icent][ieff],hbase_fs,true);
        }
    }
    ...
    for(int icent=0; icent<ncent_vtx; icent++) {
        for(int ieff=0; ieff<neff; ieff++) {
            float ymin = 0.8*heffHj[icent][ieff]->GetMinimum();
            float ymax = 5*heffFs[icent][ieff]->GetMaximum();
            if(ymin<1e-6) ymin = 5e-4;
            pad1->cd();
            pad1->SetLogy();
    ...
    // TFile* fOut = new TFile("Mc_d0Eff_peripher_newCuts_22ALL.root", "RECREATE");
    TFile* fOut = new TFile("Mc_d0Eff_peripher_newCuts_0401.root", "RECREATE");
    for(int icent=0; icent<ncent_vtx; icent++) {
        for(int ieff=0; ieff<neff; ieff++) {

...
void plot_effBinning()
{
    ...
    for(int icent=0; icent<ncent_vtx; icent++) {
        for(int ieff=0; ieff<neff; ieff++) {
            ...
            c1->SaveAs(name);
            sprintf(name,"%s/%s_%s.eps",dir,effName1[ieff],nameCent_vtx1[icent]);
            c1->SaveAs(name);
            sprintf(name,"%s/%s_%s.pdf",dir,effName1[ieff],nameCent_vtx1[icent]);
            c1->SaveAs(name);
            sprintf(name,"%s/%s_%s.png",dir,effName1[ieff],nameCent_vtx1[icent]);
            c1->SaveAs(name);
        }
    }

...
void TPDFManager::draw(std::vector<TGraphAsymmErrors*>& asgraphs,Option_t* drawOpt,bool legend,bool logx,bool logy,bool logz)
{
  ...
  asgraphs.at(0)->Draw(drawOpt);
  if(legend) mLegend->AddEntry(asgraphs.at(0),asgraphs.at(0)->GetTitle(),"LP");

  for(size_t iHist=1;iHist<asgraphs.size();++iHist)
  {
    asgraphs.at(iHist)->Draw(Form("sames%s",drawOpt));
    if(legend) mLegend->AddEntry(asgraphs.at(iHist),asgraphs.at(iHist)->GetTitle(),"LP");
  }
...
void TPDFManager::draw(TGraphAsymmErrors* asgraphs,Option_t* drawOpt,bool legend,bool logx,bool logy,bool logz)
{
    std::vector<TGraphAsymmErrors*> temp;
    temp.push_back(asgraphs);

    draw(temp,drawOpt,legend,logx,logy,logz);
}

void TPDFManager::draw(TGraphAsymmErrors* asgraphs0,TGraphAsymmErrors* asgraphs1,Option_t* drawOpt,bool legend,bool logx,bool logy,bool logz)
{
    std::vector<TGraphAsymmErrors*> temp;
    temp.push_back(asgraphs0);
    temp.push_back(asgraphs1);

...
StEventPlane::StEventPlane(const char* name, StPicoDstMaker *picoMaker, StRefMultCorr* grefmultCorrUtil, int harmonic)
  ...
  mDeltaPsi(0),mDeltaPsiEtaPlus(0),mDeltaPsiEtaMinus(0),
...
{
  for(int ii= 0 ; ii< maxShiftHarmonics; ++ii){
    prfShiftQxCent[ii] = NULL;
    prfShiftQyCent[ii] = NULL;
    //
    prfShiftQxCentEtaPlus[ii] = NULL;
    prfShiftQyCentEtaPlus[ii] = NULL;
    prfShiftQxCentEtaMinus[ii] = NULL;
    prfShiftQyCentEtaMinus[ii] = NULL;
  }
...
Int_t StEventPlane::Init()
{
   if (mFileOut)
   {
      ...
      // ---- - - - ----
      hEventPlaneShiftCent = new TH2F("hEventPlaneShiftCent", "hEventPlaneShiftCent", 9, 0, 9, 60, 0, (2.0/mHarmonic)*PI);
      hEventPlaneShiftEtaPlusCent = new TH2F("hEventPlaneShiftEtaPlusCent", "hEventPlaneShiftEtaPlusCent", 9, 0, 9, 60, 0, (2.0/mHarmonic)*PI);
      hEventPlaneShiftEtaMinusCent = new TH2F("hEventPlaneShiftEtaMinusCent", "hEventPlaneShiftEtaMinusCent", 9, 0, 9, 60, 0, (2.0/mHarmonic)*PI);
      //
...
Int_t StEventPlane::Finish()
{
  ...
  // ---- - - - ----
  hEventPlaneShiftCent->Write();
  hEventPlaneShiftEtaPlusCent->Write();
  hEventPlaneShiftEtaMinusCent->Write();
  // ---- - - - ----
  hQyQxCent->Write();
  hQyQx1Cent->Write();
  hQyQx2Cent->Write();
...
void StEventPlane::getRunInfo(int const runNumber)
{
   ...
   sprintf(fileName, "%s/%i.qVector_shift_corrected.root", EventPlaneConstants::qVectorRunDir.Data(), mRunNumber);
   TFile* fQVector = new TFile(fileName);
   if(fQVector->IsZombie())
     {
       int dayNumber = mRunNumber%1000000/1000;
       sprintf(fileName, "%s/%03d.qVector_shift_corrected.root", EventPlaneConstants::qVectorDayDir.Data(), dayNumber);
       delete fQVector;
   ...
   cout<<"Done lomnitz loading recenter profiles"<<endl;
   for( int iHarmonic = 0 ; iHarmonic < maxShiftHarmonics ; ++iHarmonic){
     cout<<"Lomnitz loading "<<mHarmonic*(iHarmonic+1)<<" profileS"<<endl;
     prfShiftQxCent[iHarmonic] = (TProfile*)fQVector->Get(Form("prfShiftQxCent_harmonic_%i",mHarmonic*(iHarmonic+1)))->Clone(Form("prfShiftQxCent_harmonic_%i",mHarmonic*(iHarmonic+1)));
     prfShiftQyCent[iHarmonic] = (TProfile*)fQVector->Get(Form("prfShiftQyCent_harmonic_%i",mHarmonic*(iHarmonic+1)))->Clone(Form("prfShiftQyCent_harmonic_%i",mHarmonic*(iHarmonic+1)));
     //
     prfShiftQxCentEtaPlus[iHarmonic] = (TProfile*)fQVector->Get(Form("prfShiftQxCentEtaPlus_harmonic_%i",mHarmonic*(iHarmonic+1)))->Clone(Form("prfShiftQxCentEtaPlus_harmonic_%i",mHarmonic*(iHarmonic+1)));
     prfShiftQyCentEtaPlus[iHarmonic] = (TProfile*)fQVector->Get(Form("prfShiftQyCentEtaPlus_harmonic_%i",mHarmonic*(iHarmonic+1)))->Clone(Form("prfShiftQyCentEtaPlus_harmonic_%i",mHarmonic*(iHarmonic+1)));
     //
     prfShiftQxCentEtaMinus[iHarmonic] = (TProfile*)fQVector->Get(Form("prfShiftQxCentEtaMinus_harmonic_%i",mHarmonic*(iHarmonic+1)))->Clone(Form("prfShiftQxCentEtaMinus_harmonic_%i",mHarmonic*(iHarmonic+1)));
     prfShiftQyCentEtaMinus[iHarmonic] = (TProfile*)fQVector->Get(Form("prfShiftQyCentEtaMinus_harmonic_%i",mHarmonic*(iHarmonic+1)))->Clone(Form("prfShiftQyCentEtaMinus_harmonic_%i",mHarmonic*(iHarmonic+1)));
     //
     prfShiftQxCent[iHarmonic]->SetDirectory(0);
     prfShiftQyCent[iHarmonic]->SetDirectory(0);
     prfShiftQxCentEtaPlus[iHarmonic]->SetDirectory(0);
     prfShiftQyCentEtaPlus[iHarmonic]->SetDirectory(0);
     prfShiftQxCentEtaMinus[iHarmonic]->SetDirectory(0);
     prfShiftQyCentEtaMinus[iHarmonic]->SetDirectory(0);
   }
...
int StEventPlane::calculateEventPlane()
{
   ...
   if (mAcceptEvent)
   {
      ...
      hEventPlaneShiftCent->Fill(mCent, mEventPlane + mDeltaPsi );
      ...
      hEventPlaneShiftEtaPlusCent->Fill(mCent, mEventPlaneEtaPlus + mDeltaPsiEtaPlus);

      hQyQxEtaMinusCent->Fill(mCent, QxEtaMinus, QyEtaMinus);
      hEventPlaneEtaMinusCent->Fill(mCent, mEventPlaneEtaMinus);
      hEventPlaneShiftEtaMinusCent->Fill(mCent, mEventPlaneEtaMinus + mDeltaPsiEtaMinus);
...
void StEventPlane::calculateEventPlaneShift()
{
  for(int ii = 0 ; ii < maxShiftHarmonics; ++ii){
    float arg = mHarmonic*(ii+1)*mEventPlane;
    float argPlus = mHarmonic*(ii+1)*mEventPlaneEtaPlus;
    float argMinus = mHarmonic*(ii+1)*mEventPlaneEtaMinus;
    //
    mDeltaPsi+= 2.0/(ii+1)*(prfShiftQxCent[ii]->GetBinContent(mCent+1)*std::sin(arg)-prfShiftQyCent[ii]->GetBinContent(mCent+1)*std::cos(arg));
    mDeltaPsiEtaPlus+= 2.0/(ii+1)*(prfShiftQxCentEtaPlus[ii]->GetBinContent(mCent+1)*std::sin(argPlus)-prfShiftQyCentEtaPlus[ii]->GetBinContent(mCent+1)*std::cos(argPlus));
    mDeltaPsiEtaMinus+= 2.0/(ii+1)*(prfShiftQxCentEtaMinus[ii]->GetBinContent(mCent+1)*std::sin(argMinus)-prfShiftQyCentEtaMinus[ii]->GetBinContent(mCent+1)*std::cos(argMinus));
  }
  mDeltaPsi=mDeltaPsi/mHarmonic;
  mDeltaPsiEtaPlus=mDeltaPsiEtaPlus/mHarmonic;
  mDeltaPsiEtaMinus=mDeltaPsiEtaMinus/mHarmonic;
}

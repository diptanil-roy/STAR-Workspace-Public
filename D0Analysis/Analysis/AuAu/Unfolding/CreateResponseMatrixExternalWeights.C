...
void Method(TString DirName = "MCMCUnf", int SUPERITERATION = 1, int iteration = 4, int mode = 0, TString TargetDistributionFileName = ""){

	cout << "File Reader Method" << endl;

    if (TargetDistributionFileName.Data() == "") {cout << "No Weight File Name Given" << endl; return;}

    TFile *TargetDistributionFile = new TFile(TargetDistributionFileName.Data(), "READ");
    TH2D *TargetDistribution = (TH2D *)TargetDistributionFile->Get("ExternalTruthHistogram");

    cout << "Target Distribution File Read" << endl;
    cout << "Target Distribution Integral = " << TargetDistribution->Integral() << endl;
	...
	if (SUPERITERATION == 0 || SUPERITERATION == 1){
		...
		for (int i = 0; i < 3; i++){
			ZUnf[i] = (TH1D *)TargetDistribution->ProjectionY(Form("ZUnf_%i", i));
			for (int binx = 1; binx <= Weight[i]->GetNbinsX(); binx++ ){
                ZUnf_JetPt[i][binx-1] = (TH1D *)TargetDistribution->ProjectionY(Form("ZUnf_%i_%i", i, binx));
                ZUnf_JetPt[i][binx-1]->Scale(1./ZUnf_JetPt[i][binx-1]->Integral());
				for (int biny = 1; biny <= Weight[i]->GetNbinsY(); biny++ ){
					...
					if (mode == 0 || mode == 2) w = ZUnf_JetPt[i][binx-1]->GetBinContent(biny);
	...
	for (int cent = 0; cent < 3; cent++){
		...
		if (cent == 0) cout << "Integral of Truth Pt = " << TruthPt[cent]->Integral() << endl;
	}
	...
	for (int cent = 0; cent < 3; cent++){
		...
		if (cent == 0) cout << "Integral of Tmp Val = " << tmpvalintegral << endl;
		if (cent == 0) cout << "Integral of Tmp Val Step 2 = " << tmpvalintegralstep << endl;
	}
...
void CreateResponseMatrixExternalWeights(TString DirName = "MCMCUnf", int SUPERITERATION = 1, int iteration = 4, int mode = 0, TString TargetDistributionFileName = ""){ // Mode 0 = Full MC; Mode 1 = Split 1; Mode 2 = Split 2
	Method(DirName, SUPERITERATION, iteration, mode, TargetDistributionFileName);
}
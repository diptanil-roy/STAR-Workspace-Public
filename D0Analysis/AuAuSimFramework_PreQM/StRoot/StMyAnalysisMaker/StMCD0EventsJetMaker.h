#ifndef STMCD0EVENTSJETMAKER_H
#define STMCD0EVENTSJETMAKER_H
...
class StMCD0EventsJetMaker : public StMaker {
 public:
  ...
  StMCD0EventsJetMaker();
  StMCD0EventsJetMaker(const char *name, double mintrackPt, bool dohistos, const char* outName);
  virtual ~StMCD0EventsJetMaker();
 ...
 private:
  ...
  // D0 tagger objects
  StTagD0MCEvents         *mD0MCTagger;               // D0 Tagger object
...
//  Bool_t                 mTowerStatusArr[4800];
  ...
  StMCD0EventsJetMaker(const StMCD0EventsJetMaker&);            // not implemented
  StMCD0EventsJetMaker &operator=(const StMCD0EventsJetMaker&); // not implemented

  ClassDef(StMCD0EventsJetMaker, 4) // Jet producing task
};
#endif

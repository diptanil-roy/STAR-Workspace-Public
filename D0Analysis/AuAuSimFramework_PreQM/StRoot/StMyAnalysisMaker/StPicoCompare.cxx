...
ClassImp(StPicoCompare)

//________________________________________________________________________
StPicoCompare::StPicoCompare(const char* name, StPicoDstMaker *picoMaker, const char* outName = "", const char* jetMakerName = "", const char* rhoMakerName = "") : StJetFrameworkPicoBase(name) //StMaker(name),
{
  ...
  fDebugLevel = 0;
  fRunFlag = 0;       // see StPicoCompare::fRunFlagEnum
  doppAnalysis = kFALSE;
...
//________________________________________________________________________
Int_t StPicoCompare::Finish() { 

  cout << "StPicoCompare::Finish()\n";
  ...
  cout<<"End of StPicoCompare::Finish"<<endl;
...
//________________________________________________________________________
void StPicoCompare::DeclareHistograms() {
  ...
  invmass_flipped = new TH1F("invmass_flipped", "invmass_flipped", 1000, 0.5, 2.5); 

  kpi_ul_pt = new TH2F("kpi_ul_pt", "kpi_ul_pt", 1000, 0, 50, 1000, 0, 50);

  kpi_ls_pt = new TH2F("kpi_ls_pt", "kpi_ls_pt", 1000, 0, 50, 1000, 0, 50);

  hD0pt = new TH1F("hD0pt", "hD0pt", 1000, 0, 50);

  trackchi2byndf = new TH1F("trackchi2byndf", "trackchi2byndf", 100, 0, 10);
  ...
  for (int i = 0; i < 6; i++){
    nskavnspi[i] = new TH2F(Form("nskavnspi_pt%i", i),Form("nskavnspi_pt%i", i), 100, -0.5, 3.5, 100, -0.5, 3.5);
    dedxkavdedxpi[i] = new TH2F(Form("dedxkavdedxpi_pt%i", i),Form("dedxkavdedxpi_pt%i", i), 100, -0.5, 3.5, 100, -0.5, 3.5);
    invbetakavinvbetapi[i] = new TH2F(Form("invbetakavinvbetapi_pt%i", i),Form("invbetakavinvbetapi_pt%i", i), 100, -0.5, 3.5, 100, -0.5, 3.5);
  }

  for (int i = 0; i < 4; i++){
    for (int j = 0; j < 4; j++){
      for (int k = 0; k < 4; k++){
        invmasscuts[i][j][k] = new TH1F(Form("invmasscuts_%i_%i_%i", i, j, k),Form("invmasscuts_%i_%i_%i", i, j, k), 1000, 0.5, 2.5); // Histogram for Delta Pi
        invmass_flipped_cuts[i][j][k] = new TH1F(Form("invmass_flipped_cuts%i_%i_%i", i, j, k),Form("invmass_flipped_cuts%i_%i_%i", i, j, k), 1000, 0.5, 2.5); // Histogram for Delta Pi
        invmassbgcuts[i][j][k] = new TH1F(Form("invmassbgcuts%i_%i_%i", i, j, k),Form("invmassbgcuts%i_%i_%i", i, j, k), 1000, 0.5, 2.5); // Histogram for Delta Pi
      }
    }
  }
  // for (int cutnum = 0; cutnum < 27; cutnum++){

  //   invmasscuts[cutnum] = new TH1F(Form("invmasscuts %i", cutnum),Form("invmasscuts %i", cutnum), 1000, 0.5, 2.5); // Histogram for Delta Pi
  //   invmass_flipped_cuts[cutnum] = new TH1F(Form("invmass_flipped_cuts %i", cutnum),Form("invmass_flipped_cuts %i", cutnum), 1000, 0.5, 2.5); // Histogram for Delta Pi
  //   invmassbgcuts[cutnum] = new TH1F(Form("invmassbgcuts %i", cutnum),Form("invmassbgcuts %i", cutnum), 1000, 0.5, 2.5); // Histogram for Delta Pi
  ...
  for (int ptbin = 0; ptbin < numberofptbins; ptbin++){
    invmass_ptbin[ptbin] = new TH1F(Form("invmass_ptbin%i", ptbin), Form("invmass_ptbin%i", ptbin), 1000, 0.5, 2.5);
    invmassbg_ptbin[ptbin] = new TH1F(Form("invmassbg_ptbin%i", ptbin), Form("invmassbg_ptbin%i", ptbin), 1000, 0.5, 2.5);
  }
  ...
  decaylengthd0 = new TH1F("decaylengthd0", "decaylengthd0", 10000, 0, 1);

  distancepik = new TH1F("distancepik", "distancepik", 10000, 0, 1);

  distanced0PV = new TH1F("distanced0PV", "distanced0PV", 10000, 0, 1);

  dcakPV = new TH1F("dcakPV", "dcakPV", 10000, 0, 1);

  dcapiPV = new TH1F("dcapiPV", "dcapiPV", 10000, 0, 1);
  ...
  // distanced0PV = new TH1F("distanced0PV", "distanced0PV", 10000, -0.05, 0.5);
...
void StPicoCompare::WriteHistograms() {

  hCentrality->Write();
  hMultiplicity->Write();
  hptdistro->Write();
  hetadistro->Write();
  const char *event_cuts[10] = {"Total Events", "|V_{z}| < 6 cm", "|V_{z} - V_{z(VPD)}| < 3 cm", "nBEMCMatch > 0", "nBTOFMatch > 0", "", "", "", "", ""};
  ...
  // trackchi2byndf->Write();
  // for (int i = 0; i < 6; i++){
  //   nskavnspi[i]->Write();
  //   dedxkavdedxpi[i]->Write();
  //   invbetakavinvbetapi[i]->Write();
  // }
  // //gPtvpPt->Write();
  // dEdXvpT->Write();
  // dEdXvp->Write();
  ...
  for (int ptbin = 0; ptbin < numberofptbins; ptbin++){
    invmass_ptbin[ptbin]->Write();
    invmassbg_ptbin[ptbin]->Write();
  }
  ...
  //   for (int j = 0; j < 3; j++){
  //     for (int k = 0; k < 4; k++){
  //       invmasscuts[i][j][k]->Write();
  //       invmass_flipped_cuts[i][j][k]->Write();
  //       invmassbgcuts[i][j][k]->Write();
  //     }
  //   }
  // } 
...
//_____________________________________________________________________________
void StPicoCompare::Clear(Option_t *opt) {
  fJets->Clear();
}
//
//  Function: This method is called every event.
//_____________________________________________________________________________
Int_t StPicoCompare::Make() {
...
void StPicoCompare::TestTracks(){
  ...
  for(unsigned short itrk = 0; itrk < ntracks; itrk++){
    ...
    if(doUsePrimTracks) {
      ...
      mTrk1Mom = trk->gMom(mVertex, Bfield);
    }
  ...
  for(unsigned short itrk1 = 0; itrk1 < ntracks; itrk1++){
    ...
    dcakPV->Fill(trk1->gDCA(mPicoEvent->primaryVertex()).Mag());
    ...
    for(unsigned short itrk2 = itrk1 + 1; itrk2 < ntracks; itrk2++){
      ...
      dcapiPV->Fill(trk2->gDCA(mPicoEvent->primaryVertex()).Mag());
      ...
      if (particle==-2) invmass->Fill(mass);
      if (particle==2) invmassbg->Fill(mass);
    }
  }
}
...
void StPicoCompare::RunJets()
{

  double bfield = mPicoEvent->bField();

  Double_t d0ptcuts[9] = {0.0, 0.5, 2.0, 5.0, 10.0, 15.0, 20.0, 30.0, 1000.0};
  ...
  for (int ijet = 0; ijet < njets; ijet++){
    ...
    cout << "Jet #" << ijet << "\t" << "Jet Pt = " << jeteta << "\t" << jetphi << endl; 
    if (!testrun)
    {
      if (corrjetpt < 15.) continue;
      if (jet->GetNumberOfTracks() == 1) continue;
    }

    // loop over ALL jet constituents (which might be less than 2 GeV/c but are inside the cone)
    // for(unsigned short itrk1 = 0; itrk1 < ntracks; itrk1++){
    for(unsigned short itrk1 = 0; itrk1 < ntracks; itrk1++){
      // get track pointer
      // int trackid1 = jet->TrackAt(itrk1);
      ...
      // if(pid1 != 0) cout << "PID 1 = " << pid1 << endl;

      if (abs(pid1)!=1) continue;

      cout << "PID 1 = " << pid1 << endl;

      dcapiPV->Fill(trk1->gDCA(mPicoEvent->primaryVertex()).Mag());
      ...
      for(unsigned short itrk2 = 0; itrk2 < ntracks; itrk2++){
        ...
        // if (pid2 != 0) cout << "PID 2 = " << pid2 << endl;

        if (abs(pid2)!=2) continue;

        cout << "PID 2 = " << pid2 << endl;

        if(abs(pid1*pid2)==2) cout << "PID 1 * PID 2 = " << pid1*pid2 << endl;

        dcakPV->Fill(trk2->gDCA(mPicoEvent->primaryVertex()).Mag());
        ...
        Double_t pt = mResMom.Perp();

        // Ensuring that the candidate is within the jet cone

        cout << "Candidate phis = " << phi << "\t" << eta << endl; 

        double deltaetad = dEta(jeteta, eta);
        double deltaphid = dPhi(jetphi, phi);
        double deltard = dR(deltaetad, deltaphid);

        cout << "Candidate deltas = " << deltaetad << "\t" << deltaphid << "\t" << deltard << endl;

        if (deltard > fJetRad) continue;

        // Filling out the information about D0 candidate pts only when they are inside a jet && satisfy topological cuts
        ...
        if(pid1*pid2 < 0) hD0US_pT->Fill(pt);
        if(pid1*pid2 > 0) hD0LS_pT->Fill(pt);

        for(int ptbin = 0; ptbin < numberofptbins - 1; ptbin++){
          if ((pt >= d0ptcuts[ptbin]) && (pt < d0ptcuts[ptbin+1])){
            if(pid1*pid2 < 0) invmass_ptbin[ptbin]->Fill(mass);
            if(pid1*pid2 > 0) invmassbg_ptbin[ptbin]->Fill(mass);
          }
        }

        if(pid1*pid2 < 0) invmass->Fill(mass);
        if(pid1*pid2 > 0) invmassbg->Fill(mass);
...
Bool_t StPicoCompare::IsAnAcceptableTrack(StPicoTrack *trk){
...
Bool_t StPicoCompare::IsTrackWithinJet(StJet *jet, StPicoTrack *trk){
...
void StPicoCompare::FillPidHistograms(StPicoTrack *trk){
...
Bool_t StPicoCompare::TopologicalCuts(StPicoTrack *trk1, StPicoTrack *trk2){
  ...
  if (abs(pid1*pid2) != 2) return kFALSE;
  ...
  decaylengthd0->Fill(mDecayLength);
  distancepik->Fill(mDcaDaughters);
  distanced0PV->Fill(perpDcaToVtx);
...
void StPicoCompare::IsWhatParticle(StPicoTrack *trk, int &pid, double &m, double &e){ // NEW PID APPROACH
...
Int_t StPicoCompare::IsWhatParticle(StPicoTrack *trk){ // Just to get the PID out
  int pid;
  double m; 
  double e;
...
// void StPicoCompare::IsWhatParticle(StPicoTrack *trk, int &pid, double &m, double &e){
//   if(!IsAnAcceptableTrack(trk)){pid = 0; m = 0.; e = 0.; return;}
...
void StPicoCompare::IsWhatParticleFlipped(StPicoTrack *trk, int &pidflipped, double &mflipped, double &eflipped){
  int pid;
  double m, e;
...
void StPicoCompare::InvariantMass(StPicoTrack *trk1, StPicoTrack *trk2, int &particle, double &invmass, TVector3 &momentum){
...
Double_t StPicoCompare::InvariantMass(StPicoTrack *trk1, StPicoTrack *trk2){
  int particle; 
  double invmass; 
  TVector3 momentum;
...
void StPicoCompare::InvariantMassFlipped(StPicoTrack *trk1, StPicoTrack *trk2, int &particle, double &invmass, TVector3 &momentum){
...
// void StPicoCompare::IsD0(StPicoTrack *trk1, StPicoTrack *trk2, bool &D0, bool &D0BgUnlike, bool &D0BgLike, double &mass, TVector3 &mom3){
...
// void StPicoCompare::ProcessTrackForKF(StPicoTrack *trk, StPicoTrackCovMatrix *picocov, Double_t params[6], Double_t cov[21]){
...
// Bool_t StPicoCompare::IsD0KFParticle(StPicoTrack *trk1, StPicoTrack *trk2, StPicoTrackCovMatrix *cov1, StPicoTrackCovMatrix *cov2){
...
void StPicoCompare::InvariantMassWithCuts(StPicoTrack *trk1, StPicoTrack *trk2, StPicoTrackCovMatrix *cov1, StPicoTrackCovMatrix *cov2){
...
// void StPicoCompare::ProcessJetForJetShape(){
...
Int_t StPicoCompare::GetFourCentBin(Double_t scaledCent) const{
  int centbin = -99;

...
const Int_t StMCD0JetMaker::fgkConstIndexShift = 100000;

ClassImp(StMCD0JetMaker)

//________________________________________________________________________
StMCD0JetMaker::StMCD0JetMaker() : 
  StMaker(),
  fD0Analysis(kFALSE),
...
//  mVertex(0x0),
  ...
  fjw("StMCD0JetMaker", "StMCD0JetMaker"),
  fRadius(0.4),
  fMinJetArea(0.001),
  fMinJetPt(1.0),
...
//________________________________________________________________________
StMCD0JetMaker::StMCD0JetMaker(const char *name, double mintrackPt = 0.20, const char *d0taggername = "", bool doHistos = kFALSE, const char* outName = "") : 
  StMaker(name),
  fD0Analysis(kFALSE),
...
//________________________________________________________________________
void StMCD0JetMaker::DeclareHistograms() {
  // constants
...
// Write histograms
//________________________________________________________________________
void StMCD0JetMaker::WriteHistograms() {
...
//________________________________________________________________________
void StMCD0JetMaker::WriteHadCorrQAHistograms() {
...
void StMCD0JetMaker::Clear(Option_t *opt) {
  fJets->Clear();
  fJetsBGsub->Clear();
}
//
...
//________________________________________________________________________________
int StMCD0JetMaker::Make()
{
  //cout << "MC Jet Maker" << endl;
  //cout << fMinJetTrackPt << "\t" << fMaxJetTrackPt << "\t" << fJetTrackEtaMin << "\t" << fJetTrackEtaMax << endl;
  ...
  if (fD0Analysis) {
    ...
    // cout << "D0 Tagger Name = " << fD0TaggerName << endl;
    ...
    d0TrackIndices = mD0Tagger->GetD0MCIndices();
    DroppedMCTracks = mD0Tagger->GetMCTracksToDrop();
    ...
    for (id0candidate = 0; id0candidate < d0TrackIndices.size(); id0candidate++){
      ...
      iTrack1 = d0TrackIndices[id0candidate][0];
      iTrack2 = d0TrackIndices[id0candidate][1];
      iD0 = id0candidate;
      idvx = d0TrackIndices[id0candidate][2];
      
/*
      cout << "******" << endl;
      cout << "MC Jet Loop D0 # " << iD0 << endl;
      cout << "******" << endl;
*/
      FindMCJetsWithD0s(iD0);
      FillJetBranch();
    }
  }
...
Bool_t StMCD0JetMaker::AcceptJetMCTrack(StPicoMcTrack *trk){
  ...
  if ((pt < fMinJetTrackPt) || (pt > fMaxJetTrackPt) || (eta < fJetTrackEtaMin) || (eta > fJetTrackEtaMax) || (phi < fJetTrackPhiMin) || (phi > fJetTrackPhiMax)) return kFALSE;

  return kTRUE;

}

void StMCD0JetMaker::FindMCInclusiveJets()
{
  ...
  // cout << "Inside MC Jet D0 Loop # " << D0 << endl; 
  ...
  double pi = 1.0*TMath::Pi();
  double pi0mass = Pico::mMass[0]; // GeV
  unsigned int ntracks = mPicoDst->numberOfMcTracks();
  ...
  if((fJetType == kFullJet) || (fJetType == kChargedJet)) {
    ...
    for(unsigned short iTracks = 0; iTracks < ntracks; iTracks++){
      ...
      StPicoMcTrack *trk = static_cast<StPicoMcTrack*>(mPicoDst->mcTrack(iTracks));
      if(!trk){ continue; }

      int idvxstop = trk->idVtxStop();
      int idvxstart = trk->idVtxStart();

      // if (idvxstop != 0) continue;
      if (idvxstart != 1) continue;
      ...
      // if (trk->geantId() == 37 || trk->geantId() == 38) cout << "D0 Pt Eta Phi " << pt << "\t" << eta << "\t" << phi << endl; 

      if (!AcceptJetMCTrack(trk)) continue;
...
void StMCD0JetMaker::FindMCJetsWithD0s(Int_t D0)
{
  // if (fPrintLevel) cout << "This is D0 # " << D0 << "***********************" << endl;
  // clear out existing wrapper object
  fjw.Clear();
  fFull_Event.clear();
  ...
  // if (fPrintLevel){
  //   for (int i = 0; i < DroppedMCTracks[D0].size(); i++){
  //     cout << "Dropped Track At Jet Maker = " << DroppedMCTracks[D0][i] << endl;
  //   }
  // }
  ...
  if((fJetType == kFullJet) || (fJetType == kChargedJet)) {
    ...
    for(unsigned short iTracks = 0; iTracks < ntracks; iTracks++){
      ...
      bool tracktodrop = std::find(DroppedMCTracks[D0].begin(), DroppedMCTracks[D0].end(), iTracks) != DroppedMCTracks[D0].end();
      // bool tracktodrop = kFALSE;
      if (tracktodrop) {

        // if (fPrintLevel) cout << "Dropped Track = " << trk->geantId() << "\t" << pt << "\t" << eta << "\t" << phi << endl;
        continue; // This should exclusively be tracks that would have made it to the detector from the decay of kaons and pions that came from D0
      }

      int idvxstop = trk->idVtxStop();
      int idvxstart = trk->idVtxStart();

      if (idvxstop != 0 && (trk->geantId() != 37 && trk->geantId() != 38)) continue; // Only Final State Particles are included in JetMaker. D0s are the only exception
      if (idvxstart == d0TrackIndices[D0][2]) {
        // if (fPrintLevel) cout << "This is the Kaon Pion From D0 = " << trk->geantId() << "\t" << trk->p().Perp() << "\t" << trk->p().PseudoRapidity() << endl;
        continue; //Get rid of the kaon pion which comes from the relevant D0
      }
      ...
      /* One D0 per jet! The other D0s are replaced by the corresponding KPi tracks*/

      // if (trk->geantId() == 37 || trk->geantId() == 38) cout << "Idvx before continue : " << idvxstop << "\t" << d0TrackIndices[D0][2] << endl; 
      if ((trk->geantId() == 37 || trk->geantId() == 38) && (idvxstop != d0TrackIndices[D0][2])) continue;
      // if (trk->geantId() == 37 || trk->geantId() == 38) cout << "Idvx after continue : " << idvxstop << "\t" << d0TrackIndices[D0][2] << endl; 

      if (trk->geantId() == 4 || trk->geantId() == 5 || trk->geantId() == 6 || trk->geantId() == 10 || trk->geantId() == 16 || trk->geantId() == 17 || trk->geantId() == 18 || trk->geantId() == 20 || trk->geantId() == 22 ) continue;

      // if (((trk->geantId() == 37 || trk->geantId() == 38)) && (pt < 1.0 || pt > 10.0)) continue; // This check has been rendered redundant with changes to the IsD0MCTrack function in StTagD0MCEvents.
      ...
      if (trk->geantId() != 37 && trk->geantId() != 38){
        fjw.AddInputVector(px, py, pz, energy, iTracks); // includes E
        fastjet::PseudoJet particle_Track(px, py, pz, energy);
        particle_Track.set_user_index(iTracks);
        fFull_Event.push_back(particle_Track);
      ...
      // if (fPrintLevel > 1){
      //   if (charge != 0 || (trk->geantId() == 37 || trk->geantId() == 38))cout << "Constituents = " << trk->geantId() << "\t" << pt << "\t" << eta << "\t" << phi << "\t" << trk->idVtxStop() << endl;
      //   else cout << "Towers = " << trk->geantId() << "\t" << pt << "\t" << eta << "\t" << phi << trk->idVtxStop() << endl;
      // }      
...
//     /* Instead of the other D0s, I input the corresponding KPi tracks */ 
//     for (int D0num = 0; D0num < d0TrackIndices.size(); D0num++){
//       if (D0num == D0) continue;
//       StPicoMcTrack *trk1 = static_cast<StPicoMcTrack*>(mPicoDst->mcTrack(d0TrackIndices[D0num][0]));
//       StPicoMcTrack *trk2 = static_cast<StPicoMcTrack*>(mPicoDst->mcTrack(d0TrackIndices[D0num][1]));
...
//       double phi1 = mTrk1Mom.Phi();
//       if(phi1 < 0.0)    phi1 += 2.0*pi;  // force from 0-2pi
//       if(phi1 > 2.0*pi) phi1 -= 2.0*pi;  // force from 0-2pi
//       double eta1 = mTrk1Mom.PseudoRapidity();
//       double pt1 = mTrk1Mom.Perp();
//       double px1 = mTrk1Mom.x();
//       double py1 = mTrk1Mom.y();
//       double pz1 = mTrk1Mom.z();
//       double p1 = mTrk1Mom.Mag();
//       double energy1 = 1.0*TMath::Sqrt(p1*p1 + pi0mass*pi0mass);

//       double phi2 = mTrk2Mom.Phi();
//       if(phi2 < 0.0)    phi2 += 2.0*pi;  // force from 0-2pi
//       if(phi2 > 2.0*pi) phi2 -= 2.0*pi;  // force from 0-2pi
//       double eta2 = mTrk2Mom.PseudoRapidity();
//       double pt2 = mTrk2Mom.Perp();
//       double px2 = mTrk2Mom.x();
//       double py2 = mTrk2Mom.y();
//       double pz2 = mTrk2Mom.z();
//       double p2 = mTrk2Mom.Mag();
//       double energy2 = 1.0*TMath::Sqrt(p2*p2 + pi0mass*pi0mass);

//       if(pt1 < fMinJetTrackPt) continue;
//       if(pt1 > fMaxJetTrackPt) continue;
//       if((eta1 < fJetTrackEtaMin) || (eta1 > fJetTrackEtaMax)) continue;

//       if(pt2 < fMinJetTrackPt) continue;
//       if(pt2 > fMaxJetTrackPt) continue;
//       if((eta2 < fJetTrackEtaMin) || (eta2 > fJetTrackEtaMax)) continue;

//       fHistNTrackvsPt->Fill(pt1);
//       fHistNTrackvsPhi->Fill(phi1);
//       fHistNTrackvsEta->Fill(eta1);
//       fHistNTrackvsPhivsEta->Fill(phi1, eta1);

//       fHistNTrackvsPt->Fill(pt2);
//       fHistNTrackvsPhi->Fill(phi2);
//       fHistNTrackvsEta->Fill(eta2);
//       fHistNTrackvsPhivsEta->Fill(phi2, eta2);

//       fjw.AddInputVector(px1, py1, pz1, energy1, d0TrackIndices[D0num][0]); // includes E
//       fastjet::PseudoJet particle1_Track(px1, py1, pz1, energy1);
//       particle1_Track.set_user_index(d0TrackIndices[D0num][0]);
//       fFull_Event.push_back(particle1_Track);

//       fjw.AddInputVector(px2, py2, pz2, energy2, d0TrackIndices[D0num][1]); // includes E
//       fastjet::PseudoJet particle2_Track(px2, py2, pz2, energy2);
//       particle2_Track.set_user_index(d0TrackIndices[D0num][1]);
//       fFull_Event.push_back(particle2_Track);
...
//     StPicoMcTrack *trk1 = static_cast<StPicoMcTrack*>(mPicoDst->mcTrack(track1));
//     StPicoMcTrack *trk2 = static_cast<StPicoMcTrack*>(mPicoDst->mcTrack(track2));

//     if (trk1 && trk2){
//        if (AcceptJetMCTrack(trk1) && AcceptJetMCTrack(trk2)){
//         TVector3 mTrkD0Mom;

//         mTrkD0Mom = trk1->p() + trk2->p();
 ...
 */
void StMCD0JetMaker::FillJetBranch()
{
  ...
  for(UInt_t ijet = 0, jetCount = 0; ijet < jets_incl.size(); ++ijet) {
    ...
    // cout << "MC Jet # : " << ijet << "\t" << jet->Pt() << "\t" <<  jet->Area() << "\t" << jet->GetNumberOfConstituents() << endl;
 ...
 */
void StMCD0JetMaker::FillJetConstituents(StJet *jet, std::vector<fastjet::PseudoJet>& constituents,
    std::vector<fastjet::PseudoJet>& constituents_unsub, Int_t flag, TString particlesSubName)
{
  ...
  for(UInt_t ic = 0; ic < constituents.size(); ++ic) {
    ...
    if(uid >= 0 && uid < 99999) {
      ...
      StPicoMcTrack *trk = static_cast<StPicoMcTrack*>(mPicoDst->mcTrack(uid));
      if(!trk) continue;
      ...
      // cout << "MC Jet Tracks : " << uid << "\t" << pt << "\t" << trk->geantId() << endl;
      ...
      if (charge == 0) neutralE+=trk->energy();
      ...
      jet->AddTrackAt(uid, nt);
      StPicoMcTrack *trk1 = static_cast<StPicoMcTrack*>(mPicoDst->mcTrack(iTrack1));
      StPicoMcTrack *trk2 = static_cast<StPicoMcTrack*>(mPicoDst->mcTrack(iTrack2));
      ...
      if (trk1 && trk2){
        //if (AcceptJetMCTrack(trk1) && AcceptJetMCTrack(trk2)){
          ...
          // cout << "MC Jet Tracks : " << uid << "\t" << pt << endl;
...
//________________________________________________________________________
void StMCD0JetMaker::FindJets(TObjArray *tracks, TObjArray *towers, Int_t algo, Double_t radius)
{
 ...
 */
Bool_t StMCD0JetMaker::GetSortedArray(Int_t indexes[], std::vector<fastjet::PseudoJet> array) const
{
 ...
 */
Bool_t StMCD0JetMaker::IsLocked() const
{
  if (fLocked) {
...
//________________________________________________________________________
Bool_t StMCD0JetMaker::AcceptTrack(StPicoTrack *trk, Float_t B, TVector3 Vert) {
...
//________________________________________________________________________
Bool_t StMCD0JetMaker::AcceptJetTrack(StPicoTrack *trk, Float_t B, TVector3 Vert) {
  // constants: assume neutral pion mass
  ///double pi0mass = Pico::mMass[0]; // GeV
  double pi = 1.0*TMath::Pi();
...
// Tower Quality Cuts
//________________________________________________________________________
Bool_t StMCD0JetMaker::AcceptJetTower(StPicoBTowHit *tower, Int_t towerID) {
  // constants:
  double pi = 1.0*TMath::Pi();
...
//________________________________________________________________________
Int_t StMCD0JetMaker::GetCentBin(Int_t cent, Int_t nBin) const
{
  Int_t centbin = -1;
...
Bool_t StMCD0JetMaker::GetMomentum(TVector3 &mom, const StPicoBTowHit* tower, Double_t mass, Int_t towerID, Double_t CorrectedEnergy) const {
...
void StMCD0JetMaker::FillEmcTriggersArr() {
...
Double_t StMCD0JetMaker::GetMaxTrackPt()
{
  // get # of tracks
...
//_________________________________________________________________________________________________
Double_t StMCD0JetMaker::GetMaxTowerEt()
{
  // get # of towers
 ...
 */
void StMCD0JetMaker::FillJetBGBranch()
{
   // E_scheme, pt_scheme, pt2_scheme, Et_scheme, Et2_scheme, BIpt_scheme, BIpt2_scheme, WTA_pt_scheme, WTA_modp_scheme
   fastjet::RecombinationScheme    recombScheme;
   if (fRecombScheme == 0)     recombScheme = fastjet::E_scheme;
...
void StMCD0JetMaker::RunEventQA() {

...
void EmbeddedParticleJetTreeMaker(const Char_t *inputFile="Run14_P18ih_HPSS_15164046.list", const Char_t *outputFile="test.root", Int_t nEv = 10, const Char_t *fOutJobappend=""){
  ...
  //StPicoDstMaker *picoMaker = new StPicoDstMaker(0,inputFile,"picoDst");
    ...
    if (singleparticleembedding) D0Tagger->DoSingleParticleEmbedding();
    ...
    jetTask->SetD0Kind(d0kind);
    if (singleparticleembedding) jetTask->GetEmbeddedParticle();
    ...
    if(doBackgroundJets) {
      ...
      jetTaskBG->SetD0Kind(d0kind);
      if (singleparticleembedding) jetTaskBG->GetEmbeddedParticle();
    ...
    if (singleparticleembedding) jettree->GetEmbeddedParticle();
    ...
    for (Int_t i = 0; i < nEvents; i++){
      if (doTEST) {if (i%10 == 0) cout << "Working on eventNumber " << i << endl;}

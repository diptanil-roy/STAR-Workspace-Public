...
void D0DstMaker_AuAu(const Char_t *inputFile="Run14_P18ih_HPSS_15164046.list", const Char_t *outputFile="test.root", Int_t nEv = 10, const Char_t *fOutJobappend=""){
  ...
  // create the analysis maker!
    ...
    StTagD0Events *D0Tagger_Tight = new StTagD0Events("D0Tagger", picoMaker, outputFile);
    D0Tagger_Tight->SetUsePrimaryTracks(usePrimaryTracks);       // use primary tracks
    D0Tagger_Tight->SetTurnOnCentSelection(doCentSelection);    // run analysis for specific centrality: BOOLEAN
    D0Tagger_Tight->SetCentralityBinCut(CentralitySelection);   // specific centrality range to run: if above is FALSE, this doesn't matter
    D0Tagger_Tight->SetdoppAnalysis(dopp);
    D0Tagger_Tight->SetSignalRange(0.75, 1.1);
    D0Tagger_Tight->SetBackgroundRangeLS(0.75, 1.1);
    D0Tagger_Tight->SetDebugLevel(0);
    D0Tagger_Tight->SetTopoCutsLevel(1);
    cout<<D0Tagger_Tight->GetName()<<endl;  // print name of class instance

...
class StTagD0Events;
class StTagD0TPCEvents;
class StTagKStarEvents;
...
void KStarDstMaker_AuAu(const Char_t *inputFile="Run14_P18ih_HPSS_15164046.list", const Char_t *outputFile="test.root", Int_t nEv = 10, const Char_t *fOutJobappend=""){
  ...
  LoadLibs();
    ...
    // if((RunYear == mRun14) && doMBset && doTEST) inputFile = "1.list";
    // if((RunYear == mRun14) && doMBset && doTEST) inputFile = "test_Run14_MB.list";
    // if((RunYear == mRun14) && doMBset && doTEST) inputFile = "Run14MB_SmallTest.list";
    if((RunYear == mRun14) && doMBset && doTEST) inputFile = "/star/u/droy1/Y2019/STAR/Test.list";
    // if((RunYear == mRun14) && doMBset && doTEST) inputFile = "D0FilesTestList.list";
    ...
    // Int_t fBadRunListVers = StJetFrameworkPicoBase::fBadRuns_w_missing_HT; // fBadRuns, fBadRuns_w_missing_HT, fBadRuns_wo_missing_HT,
    Int_t fBadRunListVers = StJetFrameworkPicoBase::fBadRuns; // fBadRuns (only for 2014), fBadRuns_w_missing_HT, fBadRuns_wo_missing_HT,
    ...
    if(CentralityDefinition == StJetFrameworkPicoBase::kgrefmult_P16id) { ZVtxMin = -6.0; ZVtxMax = 6.0; }
  ...
  // create the analysis maker!
    ...
    picoMaker->setVtxMode((int)(StPicoDstMaker::PicoVtxMode::VpdOrDefault)); //Changed to VPD or Default because HF groups have historically used this! Good opportunity to see why!
    ...
    StTagKStarEvents *KStarTagger_Tight = new StTagKStarEvents("KStarTagger", picoMaker, outputFile);
    KStarTagger_Tight->SetUsePrimaryTracks(usePrimaryTracks);       // use primary tracks
    KStarTagger_Tight->SetTurnOnCentSelection(doCentSelection);    // run analysis for specific centrality: BOOLEAN
    KStarTagger_Tight->SetCentralityBinCut(CentralitySelection);   // specific centrality range to run: if above is FALSE, this doesn't matter
    KStarTagger_Tight->SetdoppAnalysis(dopp);
    KStarTagger_Tight->SetSignalRange(0, 2.5);
    KStarTagger_Tight->SetBackgroundRangeLS(0, 2.5);
    KStarTagger_Tight->SetDebugLevel(0);
    KStarTagger_Tight->SetTopoCutsLevel(0);
    cout<<KStarTagger_Tight->GetName()<<endl;  // print name of class instance

    // // Making the PicoDst

    // TString D0DstFile_Cuts = "D0Dst_Cuts_";
    // D0DstFile_Cuts += outputFile;
    // D0DstFile_Cuts.ReplaceAll(".root", "");
    // D0DstFile_Cuts += ".picoDst.root";

    // TFile *D0out_Cuts = new TFile(D0DstFile_Cuts.Data(), "RECREATE");
    // D0out_Cuts->Close();

    // StTaggedEventFileMaker *FileMaker_Tight = new StTaggedEventFileMaker("FileMaker_Tight", picoMaker, "D0Tagger", D0DstFile_Cuts);

    // cout << FileMaker_Tight->GetName() << endl;
    ...
    if(D0out_Cuts->IsOpen()) D0out_Cuts->Close();

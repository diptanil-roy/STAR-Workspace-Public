#ifndef StJetTreeUpdatedMaker_h
#define StJetTreeUpdatedMaker_h
...
class StJetTreeUpdatedMaker : public StJetFrameworkPicoBase {
  public:

    StJetTreeUpdatedMaker(const char *name, StPicoDstMaker *picoMaker, const char *outName);
    virtual ~StJetTreeUpdatedMaker();
    ...
    void    BookTree();
    void    WriteTree();
    void    RunJetsForEmbeddedParticle();
  ...
  private:
    ...
    StGenerateARandomTrack *mGenerateRandomParticle;

    // base class pointer object
    StJetFrameworkPicoBase *mBaseMaker;
    StD0EventsJetMaker *mD0JetMaker;//!
    StD0Rho *D0RhoMaker;

    StCSJetMaker *mCSJetMaker1;//!
    StCSJetMaker *mCSJetMaker2;//!
    ...
    TClonesArray     *fJetsCS1;//!jet collection
    TClonesArray     *fJetsCS2;//!jet collection
    std::vector<TClonesArray *> fJetsArr;
    std::vector<TClonesArray *> fJetsArrCS1;
    std::vector<TClonesArray *> fJetsArrCS2;

    std::vector<double> rhovalue;
    std::vector<double> rhovalueCS1;
    std::vector<double> rhovalueCS2;
    std::vector<double> sigmavalueCS1;
    std::vector<double> sigmavalueCS2;

    StJetTreeStruct fJetTree;
    StJetTreeStruct fJetTreeCS1;
    StJetTreeStruct fJetTreeCS2;
    ...
    ClassDef(StJetTreeUpdatedMaker, 1)
};
#endif

...
const Int_t StD0EventsJetMaker::fgkConstIndexShift = 100000;

ClassImp(StD0EventsJetMaker)

//________________________________________________________________________
StD0EventsJetMaker::StD0EventsJetMaker() : 
  StMaker(),
  fD0Analysis(kFALSE),
...
//  mVertex(0x0),
  ...
  fjw("StD0EventsJetMaker", "StD0EventsJetMaker"),
  fRadius(0.4),
  fMinJetArea(0.001),
  fMinJetPt(1.0),
...
//________________________________________________________________________
StD0EventsJetMaker::StD0EventsJetMaker(const char *name, double mintrackPt = 0.20, bool doHistos = kFALSE, const char* outName = "") : 
  StMaker(name),
  fD0Analysis(kFALSE),
...
Int_t StD0EventsJetMaker::Finish() {
/*
  //  Summarize the run.
...
//________________________________________________________________________
void StD0EventsJetMaker::DeclareHistograms() {
  // constants
...
// Write histograms
//________________________________________________________________________
void StD0EventsJetMaker::WriteHistograms() {
...
//________________________________________________________________________
void StD0EventsJetMaker::WriteHadCorrQAHistograms() {
...
void StD0EventsJetMaker::Clear(Option_t *opt) {
  fJets->Clear();
  fJetsBGsub->Clear();
}
//
...
//________________________________________________________________________________
int StD0EventsJetMaker::Make()
{

  // cout << "D0 Event Maker" << endl;
  ...
  if (fD0Analysis) {
    ...
    // if ( fEmbedSingleParticle ) fEmbeddedSingleParticleVector = mD0Tagger->GetEmbeddedParticle();

    for (id0candidate = 0; id0candidate < d0TrackIndices.size(); id0candidate++){

      // cout << "Candidate # " << id0candidate << endl;

      // cout << Form("%s:: Candidate # = %i", GetName(), id0candidate) << endl;
      ...
      // cout << iTrack1 << "\t" << iTrack2 << endl;

      // fEmbeddedSingleParticle = fEmbeddedSingleParticleVector[id0candidate];
  ...
  if (fEmbedSingleParticle){
    ...
    fEmbeddedSingleParticle = fEmbeddedSingleParticleVector[0];

    // cout << fEmbeddedSingleParticle.Perp() << "\t" <<fEmbeddedSingleParticle.Eta() << "\t" << fEmbeddedSingleParticle.Phi() << "\t" << fEmbeddedSingleParticle.E() << endl;

    FindJetsWithEmbeddedTracks();
    // Fill jet branch
    if(!doConstituentSubtr) FillJetBranch();
    if( doConstituentSubtr) FillJetBGBranch();
  }
...
//________________________________________________________________________
void StD0EventsJetMaker::FindJets(TObjArray *tracks, TObjArray *towers, Int_t algo, Double_t radius)
{
...
//________________________________________________________________________
void StD0EventsJetMaker::FindJets()
{
  ...
  if((fJetType == kFullJet) || (fJetType == kChargedJet)) {
    ...
    if (fEmbedSingleParticle){
      ...
      // cout << "JET EMBEDDED PARTICLE = " << fEmbeddedSingleParticle.Px() << "\t" << fEmbeddedSingleParticle.Py() << "\t" << fEmbeddedSingleParticle.Pz() << "\t" << fEmbeddedSingleParticle.E() << endl;

      fjw.AddInputVector(fEmbeddedSingleParticle.Px(), fEmbeddedSingleParticle.Py(), fEmbeddedSingleParticle.Pz(), fEmbeddedSingleParticle.E(), 999999); // includes E
    }
...
void StD0EventsJetMaker::FindJetsWithD0s(Int_t track1, Int_t track2)
{
  // clear out existing wrapper object
  fjw.Clear();
  fFull_Event.clear();
  ...
  if((fJetType == kFullJet) || (fJetType == kChargedJet)) {
    ...
    ///////////////////////////////////////// Embedded Particle Bit (if Required) /////////////////////////////////////////
...
//=================================================================================================================
  ...
  // run jet finder
  // cout << "StD0EventsJetMaker:: " << GetName() << "\t" << "Events size = " << fFull_Event.size() << endl;
  fjw.Run();
}

void StD0EventsJetMaker::FindJetsWithEmbeddedTracks()
{
  // clear out existing wrapper object
  fjw.Clear();
  fFull_Event.clear();
  ...
  if((fJetType == kFullJet) || (fJetType == kChargedJet)) {
    ...
    if (fEmbedSingleParticle){
      ...
      fHistNTrackvsPt->Fill(fEmbeddedSingleParticle.Perp());
      fHistNTrackvsPhi->Fill(fEmbeddedSingleParticle.Phi());
      fHistNTrackvsEta->Fill(fEmbeddedSingleParticle.PseudoRapidity());
      fHistNTrackvsPhivsEta->Fill(fEmbeddedSingleParticle.Phi(), fEmbeddedSingleParticle.PseudoRapidity());
 ...
 */
void StD0EventsJetMaker::FillJetBranch()
{
  ...
  for(UInt_t ijet = 0, jetCount = 0; ijet < jets_incl.size(); ++ijet) {
    ...
    for(UInt_t ic = 0; ic < constituents.size(); ++ic) {
      ...
      if (uid == 99999) {d0Jet = kTRUE; d0index = ic; break;}
    }

    // if (d0Jet) cout << Form("%s:: Jet pT = %f;  D0 pT = %f", GetName(), jet->Pt(), constituents[d0index].perp()) << endl;
  ...
  cout << Form("%s::N_Jets_Used = %i", GetName(), fJets->GetEntries()) << endl;
 ...
 */
void StD0EventsJetMaker::FillJetConstituents(StJet *jet, std::vector<fastjet::PseudoJet>& constituents,
 ...
 */
Bool_t StD0EventsJetMaker::GetSortedArray(Int_t indexes[], std::vector<fastjet::PseudoJet> array) const
{
 ...
 */
Bool_t StD0EventsJetMaker::IsLocked() const
{
  if (fLocked) {
...
//________________________________________________________________________
Bool_t StD0EventsJetMaker::AcceptTrack(StPicoTrack *trk, Float_t B, TVector3 Vert) {
...
//________________________________________________________________________
Bool_t StD0EventsJetMaker::AcceptJetTrack(StPicoTrack *trk, Float_t B, TVector3 Vert) {
  // constants: assume neutral pion mass
  ///double pi0mass = Pico::mMass[0]; // GeV
  double pi = 1.0*TMath::Pi();
...
// Tower Quality Cuts
//________________________________________________________________________
Bool_t StD0EventsJetMaker::AcceptJetTower(StPicoBTowHit *tower, Int_t towerID) {
  // constants:
  double pi = 1.0*TMath::Pi();
...
//________________________________________________________________________
Int_t StD0EventsJetMaker::GetCentBin(Int_t cent, Int_t nBin) const
{
  Int_t centbin = -1;
...
Bool_t StD0EventsJetMaker::GetMomentum(TVector3 &mom, const StPicoBTowHit* tower, Double_t mass, Int_t towerID, Double_t CorrectedEnergy) const {
...
void StD0EventsJetMaker::FillEmcTriggersArr() {
...
Double_t StD0EventsJetMaker::GetMaxTrackPt()
{
  // get # of tracks
...
//_________________________________________________________________________________________________
Double_t StD0EventsJetMaker::GetMaxTowerEt()
{
  // get # of towers
 ...
 */
void StD0EventsJetMaker::FillJetBGBranch()
{
   // E_scheme, pt_scheme, pt2_scheme, Et_scheme, Et2_scheme, BIpt_scheme, BIpt2_scheme, WTA_pt_scheme, WTA_modp_scheme
   fastjet::RecombinationScheme    recombScheme;
   if (fRecombScheme == 0)     recombScheme = fastjet::E_scheme;
...
// set sum weights
//________________________________________________________________________
void StD0EventsJetMaker::SetSumw2() {
...
void StD0EventsJetMaker::RunEventQA() {
...
int StD0EventsJetMaker::GetMatchedBtowID(StPicoTrack *trk){

...
const Int_t StCSJetMaker::fgkConstIndexShift = 100000;

ClassImp(StCSJetMaker)

//________________________________________________________________________
StCSJetMaker::StCSJetMaker() : 
  StMaker(),
  fD0Analysis(kFALSE),
...
//________________________________________________________________________
StCSJetMaker::StCSJetMaker(const char *name, double mintrackPt = 0.20, bool doHistos = kFALSE, const char* outName = "") : 
  StMaker(name),
  fD0Analysis(kFALSE),
  ...
  fBGMode(1)
{
  ...
  fEmbeddedSingleParticle.SetPxPyPzE(0,0,0,0);
}
...
StCSJetMaker::~StCSJetMaker()
{
  ...
  if(fHistFJSigma)             delete fHistFJSigma;
...
//________________________________________________________________________
void StCSJetMaker::DeclareHistograms() {
  // constants
  ...
  fHistFJRho = new TH1F("fHistFJRho", "Underlying event energy density via FastJet", 400, 0, 100);
  fHistFJSigma = new TH1F("fHistFJSigma", "Underlying event energy density sigma via FastJet", 400, 0, 100);
...
// Write histograms
//________________________________________________________________________
void StCSJetMaker::WriteHistograms() {
...
//________________________________________________________________________
void StCSJetMaker::WriteHadCorrQAHistograms() {
...
void StCSJetMaker::Clear(Option_t *opt) {
  fJets->Clear();
  fJetsBGsub->Clear();
}
//
...
//________________________________________________________________________________
int StCSJetMaker::Make()
{

  // cout << Form("============= Class :: %s =============", GetName()) << endl;
  ...
  for (id0candidate = 0; id0candidate < d0TrackIndices.size(); id0candidate++){
    ...
    // cout << Form("%s:: Candidate # = %i \t %i \t %i \t %f", GetName(), id0candidate, iTrack1, iTrack2, mTrkD0Mom.Perp()) << endl;


    FindJetsWithD0s(iTrack1, iTrack2);

    FillJetBGBranch(id0candidate);

  }

  // if (d0TrackIndices.size() == 2){

  //   if (tmp_full_event[0].size() != tmp_full_event[1].size()) cout << Form("%s::The event inputs aren't same", GetName()) << endl;

  //   cout << " =========================================================================== " << endl;
  //   for (int i = 0; i < tmp_full_event[0].size(); i++){
  //     // if (tmp_full_event[0][i] != tmp_full_event[1][i]){
  //       // cout << Form("%i\t%.2f\t%.2f\t%.2f\t%.2f", i, tmp_full_event[0][i].e() - tmp_full_event[1][i].e(), tmp_full_event[0][i].perp()-tmp_full_event[1][i].perp(),tmp_full_event[0][i].eta()-tmp_full_event[1][i].eta(),tmp_full_event[0][i].phi()-tmp_full_event[1][i].phi()) << endl;
  //     // }

  //     // cout << Form("%i\t%.2f\t%.2f\t%.2f\t%.2f", tmp_full_event[0][i].user_index(), tmp_full_event[0][i].px(), tmp_full_event[0][i].py(), tmp_full_event[0][i].pz(), tmp_full_event[0][i].e()) << endl;

  //     // cout << Form("%i\t%i", i, tmp_full_event[0][i].user_index() - tmp_full_event[1][i].user_index()) << endl;
  //   }

  //   cout << " =========================================================================== " << endl;

  //   if(tmp_full_event[0] != tmp_full_event[1]) {
  //     cout << "Input's not the same" << endl;
  //   }
  //   else{
  //     cout << "Input's the same" << endl;
  //   }
  // }
...
void StCSJetMaker::FindJetsWithD0s(Int_t track1, Int_t track2)
{
  ...
  if((fJetType == kFullJet) || (fJetType == kChargedJet)) {
    ...
    for(unsigned short iTracks = 0; iTracks < ntracks; iTracks++){
      // get track pointer
      if (iTracks==track1 || iTracks==track2) continue;
    ...
    StPicoTrack *trk1 = static_cast<StPicoTrack*>(mPicoDst->track(track1));
    StPicoTrack *trk2 = static_cast<StPicoTrack*>(mPicoDst->track(track2));
    ...
    if (trk1 && trk2){
      ...
      if((pt > fMinJetTrackPt && pt < fMaxJetTrackPt) && (eta > fJetTrackEtaMin) && (eta < fJetTrackEtaMax) && (phi > fJetTrackPhiMin) && (phi < fJetTrackPhiMax)){
        ...
        if (fBGMode == 1) {
          ...
          double px1 = trk1->gMom(mVertex, Bfield).x();
          double py1 = trk1->gMom(mVertex, Bfield).y();
          double pz1 = trk1->gMom(mVertex, Bfield).z();
          double p1 = trk1->gMom(mVertex, Bfield).Mag();
          double energy1 = 1.0*TMath::Sqrt(p1*p1 + pi0mass*pi0mass);

          fjw.AddInputVector(px1, py1, pz1, energy1, track1);

          fastjet::PseudoJet particle_Track1(px1, py1, pz1, energy1);
          particle_Track1.set_user_index(track1);
          fFull_Event.push_back(particle_Track1);

          double px2 = trk2->gMom(mVertex, Bfield).x();
          double py2 = trk2->gMom(mVertex, Bfield).y();
          double pz2 = trk2->gMom(mVertex, Bfield).z();
          double p2 = trk2->gMom(mVertex, Bfield).Mag();
          double energy2 = 1.0*TMath::Sqrt(p2*p2 + pi0mass*pi0mass);

          fjw.AddInputVector(px2, py2, pz2, energy2, track2);

          fastjet::PseudoJet particle_Track2(px2, py2, pz2, energy2);
          particle_Track2.set_user_index(track2);
          fFull_Event.push_back(particle_Track2);
        }

        //====  matched track index ===
        int trackIndex1 = track1;
        int trackIndex2 = track2;

        int matchedTowerIndex1 = abs(GetMatchedBtowID(trk1)) - 1; // towerIndex = towerID - 1
        int matchedTowerIndex2 = abs(GetMatchedBtowID(trk2)) - 1; // towerIndex = towerID - 1
...
//=================================================================================================================
  ...
  // cout << "Size of Full Events: " << fFull_Event.size() << endl;
  fjw.Run();
}
void StCSJetMaker::FillJetBGBranch(int D0Candidate)
{
  fJets->Delete();
   // E_scheme, pt_scheme, pt2_scheme, Et_scheme, Et2_scheme, BIpt_scheme, BIpt2_scheme, WTA_pt_scheme, WTA_modp_scheme
   fastjet::RecombinationScheme    recombScheme;
   ...
   fastjet::Selector full_selector = eta_range_sel * hard_jet_sel;  // make this the main line as of March 31, 2020 - others should of been aware in past
  ...
  //  bge_rho.reset();
   ...
   for (int i = 0; i < fFull_Event.size(); i++){
      tmptotal+=fFull_Event[i].perp();
   }
   tmptotal/=(1.0*fFull_Event.size());
  ...
  //  cout << bge_rho.description() << endl;
   ...
   cout << Form("%s::N_Jets_Used = %i", GetName(), bge_rho.n_jets_used()) << endl;

  //  cout << Form("%s::Rho = %f; Sigma = %f, Mean Area = %f", GetName(), bge_rho.rho(), bge_rho.sigma(), bge_rho.mean_area()) << endl;
   ...
   fastjet::Selector notD0 = (!fastjet::SelectorUserIndex(99999));
   ...
   for(UInt_t ijet = 0, jetCount = 0; ijet < jets_incl.size(); ++ijet) {
    ...
    // if (d0Jet) cout << Form("%s:: Event Info = %i, Rho = %f; Jet pT = %f \t %f;  D0 pT = %f, Delta R = %f", GetName(), fFull_Event.size(), rho, jets_incl[ij].perp(), jet->Pt(), constituents[d0index].perp(), deltar) << endl;
   } // jet loop

  //  for (int i = 0; i < fJets->GetEntries(); i++){

  //   StJet *tmp = static_cast<StJet *>(fJets->At(i));

  //   cout << Form("%s:: Const Size Here = %i", GetName(), tmp->GetJetConstituents().size()) << endl;
  //  }
  // if (fBGMode == 2) cout << Form("%s::Jets Size = %i", GetName(), fJets->GetEntries()) << endl;
  TClonesArray *tempjets = (TClonesArray *)fJets->Clone();
  fJetsArr.push_back(tempjets);
  
  // if (fBGMode == 1){
  //   cout << "Number of Jet Clusters in Final Array = " << fJetsArr.size() << endl;
  //   for (int i = 0; fJetsArr.size(); i++){
  //     TClonesArray *tmpArray = new TClonesArray("StJet");
  //     tmpArray = fJetsArr[i];
  //     Int_t tmpnjets = tmpArray->GetEntries();

  //     cout << Form("%s:: Temp Array Entries = %i", GetName(), tmpnjets) << endl;

  //     for (int i = 0; i < tmpnjets; i++){
  //       StJet *tmpjet = static_cast<StJet *>(tmpArray->At(i));
  //       vector<fastjet::PseudoJet> constituents = tmpjet->GetJetConstituents();
  ...
  //         // get user defined index
  //         Int_t uid = constituents[ic].user_index();
  //         if (uid == 99999) {d0Jet = kTRUE;co break;}
  //       }

  //       if (d0Jet) cout << Form("%s:: Jet pT Here = %f", GetName(), tmpjet->Pt()) << endl;
...
void StCSJetMaker::FillJetConstituents(StJet *jet, std::vector<fastjet::PseudoJet>& constituents,
    std::vector<fastjet::PseudoJet>& constituents_unsub, Int_t flag, TString particlesSubName)
{
  ...
  for(UInt_t ic = 0; ic < constituents.size(); ++ic) {
    ...
    else if (uid == 999999){ // Embedded Particle Bit
      // cout << "Constituent ID = " << uid << endl;

      jet->AddTrackAt(uid, nt);

      // track variables
      double pt = fEmbeddedSingleParticle.Perp();
      double phi = fEmbeddedSingleParticle.Phi();
      double eta = fEmbeddedSingleParticle.PseudoRapidity();
      // cout << "INSIDE CONST." << endl;
      // cout << "Jet const. = " << pt << "\t" << phi << "\t" << eta << endl;
 ...
 */
Bool_t StCSJetMaker::GetSortedArray(Int_t indexes[], std::vector<fastjet::PseudoJet> array) const
{
 ...
 */
Bool_t StCSJetMaker::IsLocked() const
{
  if (fLocked) {
...
//________________________________________________________________________
Bool_t StCSJetMaker::AcceptTrack(StPicoTrack *trk, Float_t B, TVector3 Vert) {
...
//________________________________________________________________________
Bool_t StCSJetMaker::AcceptJetTrack(StPicoTrack *trk, Float_t B, TVector3 Vert) {
  // constants: assume neutral pion mass
  ///double pi0mass = Pico::mMass[0]; // GeV
  double pi = 1.0*TMath::Pi();
...
// Tower Quality Cuts
//________________________________________________________________________
Bool_t StCSJetMaker::AcceptJetTower(StPicoBTowHit *tower, Int_t towerID) {
  // constants:
  double pi = 1.0*TMath::Pi();
...
//________________________________________________________________________
Int_t StCSJetMaker::GetCentBin(Int_t cent, Int_t nBin) const
{
  Int_t centbin = -1;
...
Bool_t StCSJetMaker::GetMomentum(TVector3 &mom, const StPicoBTowHit* tower, Double_t mass, Int_t towerID, Double_t CorrectedEnergy) const {
...
Double_t StCSJetMaker::GetMaxTrackPt()
{
  // get # of tracks
...
//_________________________________________________________________________________________________
Double_t StCSJetMaker::GetMaxTowerEt()
{
  // get # of towers
...
int StCSJetMaker::GetMatchedBtowID(StPicoTrack *trk){
...
Double_t StCSJetMaker::dPhi(Double_t phi1, Double_t phi2) {
  Double_t deltaPhi;

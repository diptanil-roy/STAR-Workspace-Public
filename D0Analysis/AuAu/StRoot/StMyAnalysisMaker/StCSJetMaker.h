#ifndef STCSJETMAKER_H
#define STCSJETMAKER_H
...
class StCSJetMaker : public StMaker {
 public:
  ...
  StCSJetMaker();
  StCSJetMaker(const char *name, double mintrackPt, bool dohistos, const char* outName);
  virtual ~StCSJetMaker();
  ...
  void    FindJetsWithD0s(Int_t trk1, Int_t trk2);
  void    FindJetsWithEmbeddedTracks();
  // switches
  ...
  virtual void         GetEmbeddedParticle()            { fEmbedSingleParticle = kTRUE   ; }

  virtual void         SetBGSubtractionMode(int mode)   { fBGMode = mode; }
  ...
  std::vector<double> GetArrayofSigmaValue() {return sigmavalue;}
 ...
 protected:
  ...
  void                   FillJetBranch();
  void                   FillJetBGBranch(int D0Candidate);
  void                   InitUtilities();
  ...
  vector<fastjet::PseudoJet> tmp_full_event[10];
 ...
 private:
  ...
  StGenerateARandomTrack *mGenerateRandomParticle; //Random Object
  ...
  TLorentzVector          fEmbeddedSingleParticle;
  vector<TLorentzVector>  fEmbeddedSingleParticleVector;
...
//  Bool_t                 mTowerStatusArr[4800];
  ...
  StCSJetMaker(const StCSJetMaker&);            // not implemented
  StCSJetMaker &operator=(const StCSJetMaker&); // not implemented

  ClassDef(StCSJetMaker, 4) // Jet producing task
};
#endif

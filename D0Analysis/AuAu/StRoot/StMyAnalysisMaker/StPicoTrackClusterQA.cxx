...
// #include "runlistRun14AuAu_low_P18ih_SL20d_xrootd.h" // Run14 AuAu low-luminosity subset
...
StPicoTrackClusterQA::~StPicoTrackClusterQA()
{
  ...
  if(fHistHFTNTrackvsPt)         delete fHistHFTNTrackvsPt;
  if(fHistHFTNTrackvsPhi)        delete fHistHFTNTrackvsPhi;
  if(fHistHFTNTrackvsEta)        delete fHistHFTNTrackvsEta;
  if(fHistHFTNTrackvsPhivsEta)   delete fHistHFTNTrackvsPhivsEta;

  if (dEdXvp) delete dEdXvp;
  if (invbetavp) delete invbetavp;
  if (m2vp) delete m2vp;
  if (PicoVsProjectedTowerID) delete PicoVsProjectedTowerID;
  
  const int nPtBins = 6;
  for (int bin = 0; bin < nPtBins; bin++){
    if (PtVsPicoVsProjectedTowerID[bin]) delete PtVsPicoVsProjectedTowerID[bin];
  }
...
Int_t StPicoTrackClusterQA::Init() {
  ...
  event = 0; hfttracks = 0; tpctracks = 0;

  EventTree = new TTree("EventTree", "EventTree");
  EventTree->Branch("RunId", &runno, "RunId/I");
  EventTree->Branch("EventId", &event, "EventId/I");
  EventTree->Branch("HFTTracks", &hfttracks, "HFTTracks/I");
  EventTree->Branch("TPCTracks", &tpctracks, "TPCTracks/I");
...
void StPicoTrackClusterQA::DeclareHistograms() {
    ...
    // if(fRunFlag == StJetFrameworkPicoBase::Run14_AuAu200_low) nRunBins = 548 + 52;
    ...
    const int nPtBins = 6;
    double PtBins[nPtBins + 1] = {0.2, 0.5, 1., 2., 5., 10., 40.};
    ...
    fHistHFTNTrackvsPt       = new TH1F("fHistHFTNTrackvsPt", "HFT Ntracks vs p_{T}", 200, 0., 40.);
    fHistHFTNTrackvsPhi      = new TH1F("fHistHFTNTrackvsPhi", "HFT Ntracks vs #phi", 144, 0., 2.0*pi);
    fHistHFTNTrackvsEta      = new TH1F("fHistHFTNTrackvsEta", "HFT Ntracks vs #eta", 40, -1.0, 1.0);
    fHistHFTNTrackvsPhivsEta = new TH2F("fHistHFTNTrackvsPhivsEta", "HFT Ntrack vs #phi vs #eta", 144, 0, 2.0*pi, 40, -1.0, 1.0);

    dEdXvp = new TH2F("dEdXvp", "dEdXvp", 3000, 0, 30, 4000, -20, 20);
    invbetavp = new TH2F("invbetavp", "invbetavp", 3000, 0, 30, 4000, -20, 20);
    m2vp = new TH2F("m2vp", "m2vp", 3000, 0, 30, 5000, 0, 10);

    PicoVsProjectedTowerID = new TH2F("PicoVsProjectedTowerID", "Pico Tower ID Vs Projected Tower ID", 4801, -0.5, 4800.5, 4801, -0.5, 4800.5);
    for (int bin = 0; bin < nPtBins; bin++){
      PtVsPicoVsProjectedTowerID[bin] = new TH2F(Form("PicoVsProjectedTowerID_%i", bin), Form("Tower ID vs Projected Tower ID Pt #in [%f, %f]", PtBins[bin], PtBins[bin+1]), 4801, -0.5, 4800.5, 4801, -0.5, 4800.5);
    }
    ...
    fHistEventCounter->LabelsOption("v");  // set x-axis labels vertically
    ...
    if(fRunFlag == StJetFrameworkPicoBase::Run14_AuAu200_MB)  { runMin = 15050000.; runMax = 15200000.; }
    // if(fRunFlag == StJetFrameworkPicoBase::Run14_AuAu200_low) { runMin = 15050000.; runMax = 15200000.; }
...
void StPicoTrackClusterQA::WriteHistograms() {
  ...
  fHistHFTNTrackvsPt->Write();
  fHistHFTNTrackvsPhi->Write();
  fHistHFTNTrackvsEta->Write();
  fHistHFTNTrackvsPhivsEta->Write();

  dEdXvp->Write();
  invbetavp->Write();
  m2vp->Write();
  PicoVsProjectedTowerID->Write();
  const int nPtBins = 6;
  for (int bin = 0; bin < nPtBins; bin++){
    PtVsPicoVsProjectedTowerID[bin]->Write();
  }
...
{
  // zero out these global centrality variables
  ...
  // 	- below are example calls to various print out functions of the picoDst branches
/*
...
*/
  ...
  int RunId_Order = mBaseMaker->GetRunNo(fRunFlag, fRunNumber);
  ...
  // MB30 and not HT events!! - fill trigger histo
  ...
  // HT and not MB30 events!! - fill trigger histo
  ...
  if (!doppAnalysis){
    ...
    for(int i = 0; i < sizeof(arrMB_Run14)/sizeof(*arrMB_Run14); i++) {
      if(mPicoEvent->isTrigger(arrMB_Run14[i])) fHaveMBevent = kTRUE;
      if(fHaveMBevent) break;
    }
  }

  // Run Track Tower QA for all MB Events
  if (fHaveMBevent){
  ...
  if( (!doppAnalysis && fHaveMB30HT2HT3 && fTriggerToUse == StJetFrameworkPicoBase::kTriggerMB30HT2HT3 && !fDoTowerQAforHT)  || 
      ( doppAnalysis && fHaveMBHT2HT3   && fTriggerToUse == StJetFrameworkPicoBase::kTriggerMBHT2HT3 && !fDoTowerQAforHT) ) {  // FIXME - April28, 2021 noticed pp without !fDoTowerQAforHT
    FillEventTriggerQA(fHistEventSelectionTrg);
    RunEventQA();
    RunFiredTriggerQA();
    RunBEmcPidTraitsQA();
...
/*
  // print track/ trig/ tower/ clusters and trigger thresholds
...
void StPicoTrackClusterQA::RunTrackQA()
{
  ...
  int RunId_Order = mBaseMaker->GetRunNo(fRunFlag, fRunId);
  ...
  runno = mPicoEvent->runId(); 
  event = mPicoEvent->eventId();
  hfttracks = 0;
  tpctracks = 0;
  ...
  for(int iTracks = 0; iTracks < ntracks; iTracks++){
    ...
    if (trk->isHFTTrack()){
      fHistHFTNTrackvsPt->Fill(pt);
      fHistHFTNTrackvsPhi->Fill(phi);
      fHistHFTNTrackvsEta->Fill(eta);
      fHistHFTNTrackvsPhivsEta->Fill(phi, eta);

      hfttracks++;
    }
    fProfEventTrackPt->Fill(RunId_Order + 1., pt);

    int btowid = trk->bemcTowerIndex();
    int projbtowid = abs(GetMatchedBtowID(trk)) - 1;

    double btow = btowid;
    double projbtow = projbtowid;

    

    int ptbin = -99;
    if (pt >= 0.2 && pt < 0.5) ptbin = 0;
    else if (pt >= 0.5 && pt < 1.0) ptbin = 1;
    else if (pt >= 1.0 && pt < 2.0) ptbin = 2;
    else if (pt >= 2.0 && pt < 5.0) ptbin = 3;
    else if (pt >= 5.0 && pt < 10.0) ptbin = 4;
    else if (pt >= 10.0 && pt < 40.0) ptbin = 5;
    if (ptbin < 0) continue;

    // cout << "Pt = " << pt << " Tow ID = " << btow << "\t" << projbtow << "\t" << trk->isBemcMatchedTrack() << endl;
    PicoVsProjectedTowerID->Fill(btowid, projbtowid);
    PtVsPicoVsProjectedTowerID[ptbin]->Fill(btow, projbtow);
...
void StPicoTrackClusterQA::FillPidHistograms(StPicoTrack *trk){
  ...
  if (toftrack){
    ...
    double m2 = p*p*(invbeta_from_tof*invbeta_from_tof - 1);
...
float StPicoTrackClusterQA::GetTofBeta(StPicoTrack *trk){
...
//
// BEmcPidTraits QA functions
//	- function is kinda incomplete, as in it just doesn't do much
//	- we don't really have a use for BEMcPidTraits in Jet-Corr after April 2020 with StPicoDst updates 
//________________________________________________________________________
void StPicoTrackClusterQA::RunBEmcPidTraitsQA() {
...
void StPicoTrackClusterQA::SetSumw2() {
  ...
  fHistHFTNTrackvsPt->Sumw2();
  fHistHFTNTrackvsPhi->Sumw2();
  fHistHFTNTrackvsEta->Sumw2();
  fHistHFTNTrackvsPhivsEta->Sumw2();
...
int StPicoTrackClusterQA::GetMatchedBtowID(StPicoTrack *trk){
...
Bool_t StPicoTrackClusterQA::AcceptTrack(StPicoTrack *trk, Float_t B, TVector3 Vert) {
  ...
  if(phi < 0.0)    phi += 2.0*pi;  // force to be 0-2pi
  if(phi > 2.0*pi) phi -= 2.0*pi;  // force to be 0-2pi
...
TH1 *StPicoTrackClusterQA::FillEventTriggerQA(TH1 *h) {
  ...
  // Run14 AuAu 200 GeV
  // if(fRunFlag == StJetFrameworkPicoBase::Run14_AuAu200 || fRunFlag == StJetFrameworkPicoBase::Run14_AuAu200_MB || fRunFlag == StJetFrameworkPicoBase::Run14_AuAu200_low) {
...
void StPicoTrackClusterQA::RunHadCorrTowerQA()
{
  ...
  // towerStatus array
  double mTowerMatchTrkIndex[4800][7] = { -1 };
  int mTowerStatusArr[4800] = { 0 };
  ...
  for(unsigned short iTracks = 0; iTracks < ntracks; iTracks++){
    ...
    // track variables + fill QA histograms
    ...
    //cout<<"matchedTowerIndex: "<<matchedTowerIndex<<"  mTowerStatusArr[]: "<<mTowerStatusArr[matchedTowerIndex]<<"  trackIndex: "<<trackIndex<<endl;
    if(matchedTowerIndex < 0) continue; // no matches
  ...
  for(int itow = 0; itow < nTowers; itow++) {
    ...
    if(towerPhi < 0.0)    towerPhi += 2.0*pi;  // force to be 0-2pi
    if(towerPhi > 2.0*pi) towerPhi -= 2.0*pi;  // force to be 0-2pi
    ...
    if(towerEunCorr == 0) { 
      counter++;
      cout<<counter<<"   wtf happened here..."<<endl;
    }
    ...
    // August 15, 2020: if *have* 1+ matched trk-tow AND uncorrected energy of tower is at least your tower constituent cut, then CONTINUE 
    ...
    // print
    //cout<<"itow: "<<itow<<"  towerID = "<<towerID<<"  towerPhi = "<<towerPhi<<"  towerEta = "<<towerEta<<"  towerADC = "<<towerADC<<"  towerE = "<<towerE<<"  towerEunCorr = "<<towerEunCorr<<"  mIndex = "<<mTowerMatchTrkIndex[towerIndex]<<endl;
...
void StPicoTrackClusterQA::FillTriggerIDs(TH1 *h) {
  ...
  int nLoopMax = 0;
  if(fRunFlag == StJetFrameworkPicoBase::Run12_pp200)       nLoopMax = nRun12IDs;
  if(fRunFlag == StJetFrameworkPicoBase::Run14_AuAu200)     nLoopMax = nRun14IDs;
  if(fRunFlag == StJetFrameworkPicoBase::Run14_AuAu200_MB)  nLoopMax = nRun14IDs;
  // if(fRunFlag == StJetFrameworkPicoBase::Run14_AuAu200_low) nLoopMax = nRun14IDs;
  ...
  for(int i = 0; i < nLoopMax; i++) {
    ...
    // if(fRunFlag == StJetFrameworkPicoBase::Run14_AuAu200_low) h->GetXaxis()->SetBinLabel(i+1, Form("%i", triggersRun14[i]));
  }
  ...
  for(unsigned int i = 0; i < mytriggers.size(); i++) {
    ...
    if(mytriggers[i] > 1000) {
      for(int j = 0; j < nLoopMax; j++) {
        ...
        // if(mytriggers[i] == triggersRun14[j] && fRunFlag == StJetFrameworkPicoBase::Run14_AuAu200_low) h->Fill(j + 1);
      } // loops over ID's
    }   // non-test trigger
  }     // loop over triggers
...
void StPicoTrackClusterQA::RunEventQA() {
  ...
  int fRunId      = mPicoEvent->runId();
  int RunId_Order = mBaseMaker->GetRunNo(fRunFlag, fRunId);// + 1;

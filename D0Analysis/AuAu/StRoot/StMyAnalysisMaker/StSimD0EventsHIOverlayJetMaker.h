#ifndef StSimD0EventsHIOverlayJetMaker_H
#define StSimD0EventsHIOverlayJetMaker_H
...
class StSimD0EventsHIOverlayJetMaker : public StMaker {
 public:
  ...
  StSimD0EventsHIOverlayJetMaker();
  StSimD0EventsHIOverlayJetMaker(const char *name, double mintrackPt, const char *d0taggername, bool dohistos, const char* outName);
  virtual ~StSimD0EventsHIOverlayJetMaker();
  ...
  // StPicoTrack* SmearMom(TVector3 p, int pid);



  void    FindRecoJetsWithD0s(Int_t D0);
  // switches
  ...
  // common setters
  void         SetD0Analysis(bool d0)                     { fD0Analysis    = d0; }
  ...
  void         SetDropTracksForEfficiency(Bool_t drop)    { fDrop             = drop  ; }
  ...
  std::vector<TClonesArray *> GetJetsBGsub()                   { return fJetsBGsubArr; }
 ...
 protected:
  ...
  Bool_t                 fDrop ;                  // Drop some tracks based on efficiency
  ...
  std::vector<TClonesArray *> fJetsBGsubArr;
 ...
 private:
  ...
  std::vector<TVector3 >          d0RecoMomenta;
  std::vector<std::vector<int> >  droppedTrackIndices;

  std::vector<std::vector<double> > trackssmearedwithfastsim;

  std::vector<std::vector<double> > trackdatapicodst;
  std::vector<std::vector<double> > towerdatapicodst;
...
//  Bool_t                 mTowerStatusArr[4800];
  ...
  StSimD0EventsHIOverlayJetMaker(const StSimD0EventsHIOverlayJetMaker&);            // not implemented
  StSimD0EventsHIOverlayJetMaker &operator=(const StSimD0EventsHIOverlayJetMaker&); // not implemented

  ClassDef(StSimD0EventsHIOverlayJetMaker, 4) // Jet producing task
};
#endif
